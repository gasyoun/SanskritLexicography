...
Status: 🟡 provisional pick — encode as a per-lemma variant, NOT a rule; §54 weakly favours `stem_final` but n=2. **Adjudicated 26-08-2026 (H3538): INCONCLUSIVE at ruling strength** — the only corpus evidence is n=2 (Tier 2, derived from the §54 validation pass), below what any verdict can stand on; the per-lemma-variant encoding stands as the safe representation. Discriminating probe: extend the [RECIPES §1](https://github.com/gasyoun/SanskritLexicography/blob/master/RECIPES.md) Whitney accent validation to every derivative ī/ū-stem gen.pl token in the accented RV corpus — the same pass that closes [GAPS §1](https://github.com/gasyoun/SanskritLexicography/blob/master/GAPS.md).
