...
Status: ✅ **ruled and applied (H3538 adjudication, applied 07-09-2026 by H3960): the text wins over the label; kāṇḍas 6–7 are the critical edition, relabeled, text kept.** Two independent Tier-2 measurements agreed — the committed concordance (kāṇḍa 7: 2,688/2,690 verses pair with the DCS critical text at identical `sarga.verse`, 95.5 % scoring 1.0; kāṇḍa 6: 99.8 % identity — against 1.2–3.0 % for kāṇḍas 1/2/3/5) and the segment census (0 `ru` segments, [FINDINGS §481](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md)) — while the "Southern" reading rested on a filename/constant label alone, Weak-tier evidence. MG ruled on the [registry-contradictions sheet](https://gasyoun.github.io/vote/sheets/uprava_registry_contradictions_02-09-26.html) 02-09-2026: relabel and keep the text. **Applied:** `SOUTHERN_FILES` in [`build_ramayana_concordance.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/build_ramayana_concordance.py) now holds only kāṇḍas 1/2/3/5; kāṇḍas 6–7 moved to a new `CRITICAL_KEYED_FILES` constant. The committed [`ramayana_southern_critical_concordance.tsv`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/ramayana_southern_critical_concordance.tsv) was re-run: the 7,116 kāṇḍa-6/7 rows now carry class `self_aligned_critical` (never `matched`/`fuzzy`/`moved`) — a byte-for-byte diff confirms only that one column changed, no verse text touched. `COVERED_TEXTS_RU.md` and this row updated to match; [issue #822](https://github.com/gasyoun/SanskritLexicography/issues/822) closed against this change.

Residuals not in this fix's fence: (b) `build-gorresio`'s kāṇḍa-6/7 Gorresio↔"Southern" verse map still keys against the critical-edition numbering under a "Southern" label — flagged in `COVERED_TEXTS_RU.md`'s kāṇḍa-6 paragraph, not re-keyed (out of scope, H3960 fence); (c) how a future Russian uttarakāṇḍa should be keyed stays open ([GAPS §13](https://github.com/gasyoun/SanskritLexicography/blob/master/GAPS.md)), per MG's ruling not to touch it here.
> **Source:** [H1705](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1705-Opus_SanskritLexicography_ramayana-bombay-book7-etext_26.07.26.md) · [H3538](https://github.com/gasyoun/Uprava/blob/main/docs/CONTRADICTIONS_ADJUDICATION_WAVE1_26-08-2026.md) · [H3960](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3960-Sonnet_SanskritLexicography_ramayana-kanda67-critical-relabel_02.09.26.md) · [integrity issue #822](https://github.com/gasyoun/SanskritLexicography/issues/822) · 07-09-2026 · Claude Code Sonnet 5 (`claude-sonnet-5`)

### §10. Union headword total: 323,422 vs 323,422
...
| Source | Value | Evidence loc |
|---|---|---|
| SanskritLexicography surfaces ([FINDINGS §29](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md), [HEADWORDLISTS_DEEP_MANUAL.md](https://github.com/gasyoun/SanskritLexicography/blob/master/docs/manuals/HEADWORDLISTS_DEEP_MANUAL.md), [ch03](https://github.com/gasyoun/SanskritLexicography/blob/master/Digital_Sanskrit_Lexicography-BOOK/chapters/ch03_headword_inventory.md)) + kosha's own [datasets.json](https://github.com/gasyoun/kosha/blob/main/data/manifest/datasets.json) `union-headwords` row | **323,422** | post-fold `build_union.py` output |
| [kosha/README.md](https://github.com/gasyoun/kosha/blob/main/README.md) (twice) + three archived handoffs (H054, H105, H106) | **323,422** | kosha `lemmas` table load |
Status: ✅ **RULED 26-08-2026 (H3538) — both true under different scopes; the header row is the whole gap.** Tier 1, measured directly on the canonical asset: [union_headwords.tsv](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/union/union_headwords.tsv) holds **323,422 physical lines**, of which **line 1 is a column header** (`slp1 · iast · n_dicts · dicts · gender · fem_fold`) and the file ends in a newline — so **data rows = 323,422**. Every 323,422 witness counts headwords; every 323,422 witness counts file lines (or reflects a header-inclusive load). The row's own off-by-one hypothesis is CONFIRMED. **The headword count of record is 323,422.** The kosha-side prose (README, H054/H105/H106) should read "323,422 headwords (323,422 file lines incl. header)"; that wording fix belongs to a kosha `[integrity]` pass, not this repo. Residual probe (optional, kosha-side only): `SELECT COUNT(*) FROM lemmas;` on a rebuilt kosha.db, to check whether the header was actually ingested as a row or merely counted. Local ruling per the §8 precedent — no `D##` graduation.
Blocks: any paper citing "the union" out of kosha inherits a figure that disagrees with the canonical asset by 1; A55's "323,422 rows" title-adjacent claim.
...
✅ **Ruled 07-09-2026 (MG, via the [registry-contradictions vote sheet](https://gasyoun.github.io/vote/sheets/uprava_registry_contradictions_02-09-26.html), executed as [H3961](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3961-Sonnet_SanskritLexicography_zenodo-governance-match-live-archival_02.09.26.md)): the repo-level Zenodo archival is the intended posture.** [DATA_LICENSE.md](https://github.com/gasyoun/SanskritLexicography/blob/master/DATA_LICENSE.md) and [data/FAIR_RELEASE_1.md](https://github.com/gasyoun/SanskritLexicography/blob/master/data/FAIR_RELEASE_1.md) are amended to state that posture (superseded §, not deleted). The curated file-level deposit does **not** become redundant: the repo-level `.zenodo.json` record is a whole-repo `software`/MIT record whose own description disclaims dataset-level DOI coverage, so the curated CC-BY, dataset-typed deposit of the two TSV files is still wanted for its own DOI. FAIR release #2's hold is lifted (rights posture is settled; the deposit itself is not yet minted — no DOI was minted or release cut by this ruling).
...
Status: ✅ ruled — MG's decision (not agent-rulable by evidence alone, since both positions were deliberate human-authored policy statements about redistribution rights, not measurements). What Tier 1 evidence had already narrowed: a GitHub release archive carries only the tracked git tree at the tag, so the untracked scan PDFs `DATA_LICENSE.md` worried about were never actually swept in, and the whole-repo record's own description states "source dictionary texts are corrected upstream in `csl-orig` and are not redistributed here." That observation made the live posture defensible; MG's ruling made it official.

Blocked (now lifted): FAIR release #2 (Q2 WS2.5 of [ROADMAP_STATISTICS_ORG_CENSUS_2026_2027.md](https://github.com/gasyoun/SanskritLexicography/blob/master/ROADMAP_STATISTICS_ORG_CENSUS_2026_2027.md)) was held because minting DOIs for a second dataset set while release #1's rationale was unsettled would have published the inconsistency. That hold is lifted — the rights posture is ruled. Release #1's own upload (the two `@DO` rows in [data/FAIR_RELEASE_1.md](https://github.com/gasyoun/SanskritLexicography/blob/master/data/FAIR_RELEASE_1.md) § Status) is still not made; nothing here mints it.
