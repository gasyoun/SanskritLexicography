...
### Fixed
- **The blind A/B vote sheet was unreviewable — anatomy colouring + linked citations restored** (29-07-2026, Opus 5 1M `claude-opus-5[1m]`): [`build_ab_review_sheet.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/h1210/build_ab_review_sheet.py) rendered raw CDSL markup as escaped text with dead `<ls>` citations — the third generator in a row to re-introduce the defect H1646 (csl-atlas) and H1808 (here) already settled. Both panels (RU rendering + DE source) now go through `csl_pyutil.anatomy.highlight()` with a one-per-sheet legend, and every citation resolves to its scan through the repo's ported [`ls_resolver.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/ls_resolver.py) — **1,430 linked citations**; abbreviation-only citations stay plain rather than becoming dead links. LANG_PARITY entry `h1210_ab_arm_scaffold` re-affirmed (the fix is language-neutral; the GAP is unchanged).
