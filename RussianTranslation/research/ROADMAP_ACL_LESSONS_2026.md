...
- **Leonchenko Sinonimy as a W2 evidence lane**: `VisualDCS/derived-data/Sinonimy` (xlsx: Глагольные синонимы, Значения, S_P_D_F, Works-Share-Syn) → digitize to jsonl → another per-sense evidence source alongside grin12/grin3 ("which senses does Leonchenko's synonymy support"). Local data, no rights blocker. **✅ Digitization done (H1491, 26-07-2026):** [research/sinonimy/sinonimy.jsonl](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/research/sinonimy/README.md), 47,273 rows. Finding: the four named groups reduce to **three** distinct datasets — S_P_D_F and Works-Share-Syn turned out to hold confirmed duplicate/re-exported copies of the two top-level workbooks + `Значения.xlsx`, not new content (see sinonimy/README.md for the dedup table). **✅ Wired into `corpus_gate.py` (H2410, 09-08-2026, Sonnet 5 `claude-sonnet-5`):** `load_sinonimy_index()` reads the jsonl directly (no separate build script), self-excludes/dedups `synonym_group_lemma`/`synonym_group_gloss` members and feeds two new `build_card()` fields — `sinonimy_synonyms` (Sa-side corroboration, rule 5 analog) and `sinonimy_senses` (soft EN sense signal, rule 8 analog); wired into the `4_korpus_proverka.txt` verdict prompt as rule 9. "Sinonimy as its own published crosswalk" below stays open (out of scope for H2410).
