...
- Pre-filter отсечет ~25% карточек (тривиально-OK) → экономия ~$700
...
Случай когда opaque-тег `<ab>X.</ab>` был **дропнут** в RU и заменен
...
4. **tag_dropped** — отдельный класс ошибок: opaque-тег <ab>X.</ab>
   ...
   дропнут в RU и заменен русским текстом. Например EN содержит
...
```
- **Без связки «of» между сохраненными тегами** (например `<ab>N.</ab>
...
1. **anglicism** — калька с английского ("ходить вперед" вместо
...
НЕ считай ошибкой:
- сохраненные английские аббревиатуры в <ab>X.</ab> тегах
...
- **Полный mw_ru через Yandex** (если решим делать) — берем slim+async (~$225-300)
- **Pilot/проверка** — берем full sync (там цена не критична, нужно
...
- ❌ `levenshtein_ratio(en, ru) > 0.85` — cyrillic vs latin дает почти
...
**Гибрид v4** — самый расчетливый вариант: Yandex-slim делает первый
проход (находит anglicism/grammar дешево), Opus делает целевой проход
