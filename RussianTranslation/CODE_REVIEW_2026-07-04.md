...
| F5 | [audit_window_en.py:252](src/pilot/audit_window_en.py) | Advertised HARD `DUP` gate never emitted — the only within-record duplicate signal was the soft `SAME-GLOSS` (gated on `≥3` content words), so two senses with identical English (e.g. `to go`/`to go`) passed `--strict` clean. A parity GAP vs the RU `sense_dupes` gate. | Emit a real HARD `DUP(=tag)` flag for two senses in one record sharing identical normalized English, independent of word count; add `DUP` to the `HARD` set so `--strict` fails. Pinned by `window_selftest.test_en_gate_dup_has_teeth`; parity entry `en_dup_hard_gate_20260704` (GAP-being-closed → SHARED). |
| F6 | [audit_window.py:71](src/pilot/audit_window.py) | RU gate recovered `translation`/`coverage`/`sense_dupes` verdicts by regex-scraping the child auditors' prose stdout (`\| flagged:` / `rendered by:`); any wording drift → parser returns `[]` → flagged cards silently dropped from the requeue while the gate reports clean, child returncode never consulted. | Child auditors now emit a strict machine-readable `FLAGGED_JSON: […]` verdict line; the parent parses it strictly and returns `None` (never `[]`) on a missing/malformed line → requeues the whole window and marks the gate `unparseable-verdict` (fails loud). Dead `parse_nws` removed. Pinned by `window_selftest.test_ru_gate_fails_loud_on_unparseable_child`. |
...
- ✅ **Advertised HARD `DUP` gate never emitted** — [audit_window_en.py:252](src/pilot/audit_window_en.py). **Fixed (F5, H169).** Real HARD `DUP` flag now fires regardless of gloss length; `--strict` fails on it.
- ✅ **RU gate recovers verdicts by regex-scraping subprocess stdout** — [audit_window.py:71](src/pilot/audit_window.py). **Fixed (F6, H169).** Child auditors emit a strict `FLAGGED_JSON:` verdict line; the parent fails loud + requeues the whole window on any unparseable verdict instead of silently dropping flagged cards.
