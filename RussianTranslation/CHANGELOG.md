...
## [1.144.155] - 2026-09-07

- **H3960 — Rāmāyaṇa kāṇḍas 6–7 relabeled Baroda critical edition, text kept.** `SOUTHERN_FILES` in `src/build_ramayana_concordance.py` now holds only kāṇḍas 1/2/3/5; kāṇḍas 6–7 moved to a new `CRITICAL_KEYED_FILES` constant (H3538 adjudication, applied). The committed `src/ramayana_southern_critical_concordance.tsv` was re-run: the 7,116 kāṇḍa-6/7 rows now carry class `self_aligned_critical` instead of `matched`/`fuzzy`/`moved` — a byte-for-byte diff confirmed only that one column changed, no verse text touched. `pwg_ru/COVERED_TEXTS_RU.md` corrected.
## [1.144.151] - 2026-09-07
...
- **FINDINGS §612 amended — w09’s sidecars are half-recoverable, and the half that is not reveals the cards are substantively STALE, not merely unvalidatable (Opus 5 (`claude-opus-5`), 29-08-2026).** Regenerating through `_pilot_gen_merged.py` restores every `.raw.txt` byte-exactly; 4 of 5 `.portrait.json` still mismatch, and the whole diff is `"layers": ["pw"]` → `["nws", "pw"]`. Control: `kast_ur_i`, the one head with no `nws` sub-card, reproduces byte-for-byte. The source GAINED a supplement layer after the window was planned (`inputs[key].nws == 0` in the manifest). So promoting those cards would store knowingly incomplete entries — the stale-input gate was protecting content, not just provenance — and a re-run is now strictly better than the original window rather than a consolation, since it sees `nws + pw`.

- **Upaniṣad `<ls>` census: 820 refs / 314 rows, and Syrkin is mined but never aligned (Opus 5 `claude-opus-5`, 29-08-2026).** Prompted by "if `U.` is for Upanishads, have we aligned Syrkin?" — asked off H3663's `jar_ayu` card. **The premise needed correcting first:** that card's `<ls>Uṇ. 1,4</ls>` is the **Uṇādisūtras** (a grammatical text, cited right after `(von 1. {#jar#})`), not an Upaniṣad; Upaniṣads are spelled `UP.` and a standalone `U.` does not occur as an `<ls>` source at all — `UP.` **820**, `Uṇ.` **4**, `U.` **0**. Neither resolves through repo data: `pwg_ab.py` covers `<ab>` not `<ls>`, and [`ls_source_map.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/ls_source_map.json) holds 45 entries with **no Upaniṣad among them**, so the `Uṇ.` reading is inferred from citation shape, not verified — that table is the missing prerequisite. **Surface:** 820 of 82 957 `<ls>` refs (~1 %) across 314 rows (~2.7 %), heavily concentrated — Chāndogya 312, Bṛhadāraṇyaka 108, Muṇḍaka 70, Śvetāśvatara 66, Kauṣītaki 56, Taittirīya 44, Aitareya 18 — i.e. almost exactly Syrkin's corpus. **Answer: mined, not aligned.** `syrkin_tom_1_utf` is a running-text mining source ("running scholarly prose, terms scattered — lower, noisier": 41 term-bearing rows, 35–36 useful), which harvests term equivalents but cannot say what Syrkin renders at a given passage; passage-level lookup is **Track A, recorded BLOCKED** in [REUSE_MAP.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/REUSE_MAP.md) — "none of MG's ~12 parallel texts exist in verse-aligned jsonl". Existing caveat: the «Syrkin» file set already proved impure (a non-Syrkin `jabala-up` 2025 inside it, two edition years retired), so any alignment censuses the file set first. **Why it outweighs 1 %:** those 314 rows are among the few places where a canonical published Russian rendering exists for the exact passage, so a G5 reviewer could compare against Syrkin instead of adjudicating from the German alone — the cheapest quality signal in the store, and it lands on review, not generation. Two deterministic prerequisites, no paid window: extend `ls_source_map.json` over the nine Upaniṣads so `CHĀND. UP. 3,19,2` parses into (work, chapter, section, verse), then convert Syrkin to verse-addressed jsonl (the blocked Track A work). Note: [research/UPANISHAD_LS_CENSUS_SYRKIN_ALIGNMENT_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/research/UPANISHAD_LS_CENSUS_SYRKIN_ALIGNMENT_29-08-2026.md).
- **H3679 staged — the 7 held Lane A keys are re-translation-ready; the paid window is gated on the c1 ration and a store-lineage reconciliation (OxAlpha — GLM 5.3 Flash (`glm-5.3-flash`), 29-08-2026, ZERO paid calls).** All free work verified offline: inputs regenerated 7/7 with `_pilot_gen_merged.py` (the H3627 recipe; safe-name→SLP1 decode pinned — `_s_ulin`=`SUlin`, `ut_ta`=`utTa` uttha, `ut_t_apana`=`utTApana` utthāpana — and `uTta`/`uTtApana` are absent from PWG, `T`=tha not ṭ), copied to the durable `pwg-ru-data/raws/`, and four `--budget=1` byte-mode chunk manifests built with `ut_t_apana` isolated alone (measured `N cards in N batches`, `[1,…]`). Two traps pinned for the fire session: requeue/gen harness flags are **equals-form only** (`--out <path>` as two tokens is silently ignored and writes the default harness), and without the five binding flags the manifest stamps **v1** which production refuses (H3677). The ration gate (H3658/H3663: ≤2 attempts per UTC day) is measured exhausted for 29-08 — H3659's window (close PRs 05:56–07:29Z) and H3663's 16-key lane (close PRs 09:47Z) both ran that UTC day — so the earliest legal fire is 30-08 00:00Z, from the box holding the c1 roster (the Mac estate has no `max_orchestrator.sqlite`, no profile `--config-dir`, no `PWG_EVIDENCE_DIR`; H3659's `D:\ClaudeTools\…` evidence root names the Windows box). **Load-bearing discovery, gated before any promotion:** the only store on this disk is 11 462 rows (H3627's 28-08 mirror refresh, sha `19fcf5258e5e…`) — H3654's and H3663's promotions to 11 519 (`3022239c63ac`) exist on NO durable surface here, so the fire session must reconcile the store base (sync the Windows store into `pwg-ru-data`, or requeue the 9 lost cards honestly) before `--merge`. Packet [pwg_ru/h3679/H3679_LANE_A_TAIL_7KEY_C1_RERUN_STAGED_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3679/H3679_LANE_A_TAIL_7KEY_C1_RERUN_STAGED_29-08-2026.md).

## [1.144.127] - 2026-08-29
- **FINDINGS §612 fixed — a window’s INPUT sidecars are now parked beside its manifest at plan time, fail-closed (Opus 5 (`claude-opus-5`), 29-08-2026).** §612 found that the durable evidence root kept everything a paid window PRODUCED and nothing it CONSUMED: the `.raw.txt` / `.portrait.json` pair lives in the checkout-relative gitignored `src/pilot/input/`, dies with the planning worktree (§603’s class), and `audit_window.py` hashes exactly those files — which is how `no_pwg_w09`’s two audit-clean cards became permanently unpromotable after 8 priced calls. New `no_pwg_scale_plan.park_input_sidecars()` copies both sidecars for every `selected_key` into `<artifact_dir>/input/` next to `execution_manifest.<root>.json`, records a sha256 pair per key in the plan’s `headless_meta.input_sidecars`, and **raises at plan time if any sidecar is absent** — before a single call is paid for, because discovering it after the window has run is the unrecoverable case. Two small files per key put the consumed half of a window on the same durable path every other artifact already travels. Pinned by `window_selftest.test_h3659_input_sidecars_parked_beside_manifest` (copy fidelity + the fail-closed refusal, which must never degrade to a warn); 219/219 green, and the 39 LANG_PARITY entries pinning `window_selftest.py` were re-verified (the added test is language-neutral, so every SHARED / INTENTIONAL-DIVERGENCE verdict stands).

- **The `{#…#}` span-drop null class is CLOSED on a real paid window; `hasita` is a clean card — H3677 (Opus 5 `claude-opus-5`, gen `claude-sonnet-5`, 29-08-2026).** `no_pwg_w10` on c1: **5 keys, 5 calls, `null_keys: []`, zero retries / heals / refusals** against a `--max-calls 13` ceiling (§604's ≥2.5× floor), versus H3659's 8 calls for 3 verdicts and one `translation-fidelity-reject`. `target_anchor_invocations: 5` / `target_anchor_repairs: 5`, all `repaired` — and falsifiable, because the counter increments BEFORE the attempt, so `1 invocation / 0 repairs` would have read *reached-and-refused* rather than §604's ambiguous zero. **`hasita~~h0_zz_pw`, requeued as unfixable across H858 / H3144 / H3157 / H3659, is audit-clean**, and the repair is semantically right rather than count-matching: sense 2 carries `<ls>GAUT.</ls>` at the *tail* of the Russian, exactly where the German parallel puts it — the trailing-citation placement `german_anchor` structurally cannot do. Audit: **4 clean** (`darv_i`, `gl_ana`, `hasita`, `jaw_ayus`), **1 defect** (`kast_ur_i`, `markup_wrapper_dropped` + `untranslated_braced_german_gloss` ×2) — read off `requeue.defect.keys.txt`, not the `PASS: 5/5 clean` unit line, per §611. **Two corrections recorded as §615.** H3675 justified the target repair by *"the same sense's german still carries every token in order (guaranteed by the guard's own precondition)"* — that precondition held on **zero** of the five cards: `german_anchor` fired on all five first, so the repair anchored against a machine-repaired german. Still sound (`german_anchor` reinjects from the source skeleton and verifies), but not for the reason on record. Both repairs reinjected the **identical** token set per key and **every** key dropped `{T1}`, so the drop is a property of the CARD, not of one field. And the live `hasita` dropped **T1, T8, T11** where H3675's offline replay reconstructed `{T7}` — a replay proves a repair works, never what it will be asked to repair. Spend 5 calls, 16 827 output / 119 225 cache-creation / 121 765 cache-read tokens, cost NOT evaluable (§597/§602). §612 pre-empted: all ten input sidecars were staged into the durable evidence root **before** the run, so these cards stay promotable after the worktree dies. **Nothing promoted yet** — the narrowed `--merge` (defect blocked by list, no `--force`, no `--override-reviewed`) is pending. Packet [pwg_ru/h3677/H3677_NO_PWG_W10_RESULT_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3677/H3677_NO_PWG_W10_RESULT_29-08-2026.md).

- **The predicted repair recovered 1 defect card of 8, and the measurement explains why — H3663 residual + FINDINGS §614 (Opus 5 `claude-opus-5`, 29-08-2026).** H3663's packet predicted its 8 defects would largely fall to the deterministic repairs that cleared `_atura` in H3658 Lane B, since both are dominated by `markup_wrapper_dropped`. **Wrong.** Applying `fix_d1` + `fix_d3` + `apply_no_yo` to all 8 fired **6 ё removals and zero wrapper repairs**: across their **92 senses there are zero `«…»` spans**, so `fix_d3` correctly reported `eligible=False` everywhere. `_atura`'s wrappers had *drifted* to guillemets and were 1:1 rewrappable; these were **never emitted** — `jar_ayu` 13/13, `_sr_avaka` 13/13 and `_s_ulin` 6/6 wrappers wholly absent from the Russian, which would mean guessing gloss boundaries inside translated prose, exactly what PR #789 refused. And **four of the eight** (`r_ama_wa`, `ut_ta`, `y_atu`, `v_as_a`) have `dropped = 0`: their wrappers are intact and the flag was never their reason for being held back. So one flag name covers three situations — guillemet drift (repairable), wholesale omission (not), and not-a-wrapper-problem-at-all — and the cheap diagnostic that separates them is counting `{%…%}` in DE vs RU and `«…»` in RU **before** promising a repair class. **Recovered `v_iqu`**, flipped clean by the ё fix alone: promoted `--merge` against a union block list rebuilt from the re-audit dirs, **store 11 515 → 11 519** (4 sense rows). Gates green: mirror `ed8265065ddf` → `3022239c63ac`, `audit_store_gates` **11 519** rows `only_src=0 only_mirror=0 changed_ru=0` exit 0, `placement_axis_check` OK. **Standing after both passes: 8 of 16 promoted, 7 defect, 1 transient** — the 7 need re-translation or human review, and no deterministic rule in this repo addresses them.

## [1.144.126] - 2026-08-29
- **FINDINGS §612 — the durable evidence root keeps a paid window’s outputs but not its inputs, so `no_pwg_w09`’s two clean cards are permanently unpromotable (Opus 5 (`claude-opus-5`), 29-08-2026).** `audit_window.py` refuses the narrow promote of `darv_i` + `gl_ana`: all ten `.raw.txt`/`.portrait.json` sidecars are missing from the checkout (935 present, zero for these keys), from the surviving H3659 worktree, and from the evidence root — which stores out/status/calls/manifest/preflight/receipt/envelopes, every output and nothing consumed. The manifest keeps `skeleton`/`portrait` as strings while the audit hashes files; `--allow-stale` is forensic-only and using it here would be guard-tunnelling; replanning cannot guarantee byte-identity. Fix: copy `input/<key>.raw.txt` + `.portrait.json` for every `selected_key` into the evidence root at manifest-build time — two files per key. Third recurrence of §603, first unrecoverable one.

- **FINDINGS §611 — the window audit's "units clean" line is not the promotable verdict, and `--merge` is not dry-run (Opus 5 `claude-opus-5`, 29-08-2026).** Two H3663 traps, both producing plausible-looking wrong results. `audit_window.py`'s `PASS: n/n units clean` / `FLAGGED_JSON: []` is the **markup-fidelity unit gate** only; the audit's `requeue.defect.keys.txt` is what `promote_final_cards.py`'s defect guard loads, and on H3663 the two disagreed **15-of-16 vs 7 clean / 8 defect / 1 transient**. Per-chunk runs write per-chunk requeue dirs, so a multi-chunk window needs their **union** — one chunk's file is not the batch's block list. Corollary: `high-confidence risks: 0` held on every chunk while 8 keys were still defects, so "no high-confidence risks" is not evidence of promotability. Second trap: `--apply`'s help gates only `--ready-partial-report`, so a plain `--merge` **writes the store**; H3654's recipe carried `--apply` and every downstream reader inherited the assumption that it is the write switch. The automatic `.premerge.*.bak` and the absence of `--override-reviewed` are what made the resulting unintended write recoverable and content-correct. Both are why H3663 narrowed its promotion **input** instead of using `--force`, which bypasses the guard for every key rather than the one being excused.

- **Gate 3 of a paid `no_pwg` window is closed: the planner now stamps a production-eligible v2 manifest — H3677 (Opus 5 `claude-opus-5`, 29-08-2026).** [`gen_opt_harness2.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/gen_opt_harness2.py) emits `pwg.headless_execution_manifest.v2` only when given a profile binding, and `bounded_staged_run` refuses v1 in production — but [`no_pwg_scale_plan.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/no_pwg_scale_plan.py) never passed the five flags [`coordinator.prepare`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/coordinator.py) passes to the same generator, so **every window the planner ever prepared was born unrunnable** and H3659 had to rebuild its manifest by hand before it could spend. The failure is silent in the worst place: preparation prints `… manifest …v1` and **exits 0** with the lease registered, the cost gate passed and the harness written, so the refusal arrives at `bounded_staged_run` — *after* a human has cleared billing authorization and the cost guard. The cheapest of the three gates was discovered last, every time. New `binding_args()` builds the coordinator's exact list and `prepare_window` forwards it; `register_prepared_lease` records `profile_slot`/`config_dir`/`executor_lane` as `prepare` already did; `--profile-slot`/`--config-dir` are refused unless supplied together. **Verified end-to-end, not asserted:** the patched planner emits v2 carrying `config_dir_fingerprint 9321e2c138f0…`, byte-identical to the binding H3659 hand-built for `no_pwg_w09`, and selects exactly the five sub-cards the handoff names (`darv_i`, `gl_ana`, `hasita`, `jaw_ayus`, `kast_ur_i`). `windows100_selftest._test_planner_binding` pins the list against `coordinator.py`'s **source** rather than a copied literal, so the two paths cannot drift apart again (the H2254 copied-constant failure mode one subsystem over). Also confirmed free of charge, because §604's lesson is that a `0` repair counter is unfalsifiable: `target_anchor` is genuinely **reachable** on the live path in BOTH lanes — the Python call sits inside the `count_card_field` branch that produced `hasita`'s `translation-fidelity-reject`, and the JS harness increments `TARGET_ANCHOR_INVOCATIONS` *before* attempting the repair, so "never reached" and "reached and refused" are now distinguishable. `window_selftest` **218/218**; `lang_parity_check` 104 entries no drift, four entries re-stamped after re-deriving SHARED (all 93 added lines grepped for a language-keyed token, zero hits). FINDINGS §613. **Gates 1 (human billing authorization) and 2 (the fail-closed cost guard, whose `--allow-unbounded` escape is a human's call — H2851) remain OPEN and were deliberately not routed around: ZERO paid calls, nothing promoted, no store write.** Packet [pwg_ru/h3677/H3677_NO_PWG_PLANNER_V2_BINDING_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3677/H3677_NO_PWG_PLANNER_V2_BINDING_29-08-2026.md).

## [1.144.125] - 2026-08-29
- **All 16 never-attempted keys translated; 7 promoted, and a killed window proved the salvage path has an external-kill gap — H3663 (Opus 5 `claude-opus-5`, gen `claude-sonnet-5`, 29-08-2026).** H3658's unrun Lane A, executed after MG ruled "run those two windows". **The scoping step H3663 called unsolved was already tooled:** the 16 are the *transient* (never-ran, null) bucket of the H3627/H3654 window, and [`requeue_from_audit.py <root> --transient`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/requeue_from_audit.py) exists for exactly that — its docstring calls it "only null cards (cheap re-run)" and it deliberately keeps `--tm=auto` *because nothing was ever cached*. `coordinator.py prepare-requeue --transient` wraps it, but there were **0 coordinator leases** (H3627 drove the worker off a hand-built manifest), so the script was called directly; `root=h3627-reingest`, `nominal=true`, both read off the sealed manifest's `meta`. **A 16-key background window was KILLED at 15 min having spent 7 calls with 6 finalized, and `out.h3663.json` was never written.** Timing is the diagnosis — started 11:17, died 11:32, exactly when the session turn ended after a Stop-hook cycle: background tasks did not survive turn end. That exposes a real gap: **1.144.114's salvage covers the worker's OWN abort, not an external kill** — it holds results in memory and writes `--output` only on clean completion or its own salvage path, and a SIGKILL gives it neither, reproducing the H3627 all-or-nothing loss by a route the salvage never claimed. Containment was clean (no orphan processes, store untouched, no partial write). **Mitigation: five short chunks**, each writing its own output, with the turn kept alive by polling, and **`_sr_avaka` isolated alone** so the key that killed H3627 could not take others down. **All 16 returned, `null_keys: []` in every chunk** — and `_sr_avaka` translated **cleanly on the first attempt in 134.6 s**, no retry, no heal, no `structured_output_retry_exhausted`; the schema-park half of 1.144.114 was never even exercised. **The audit verdict is NOT the unit gate.** `audit_window.py`'s `PASS: 4/4 units clean` is markup fidelity only; its `requeue.defect.keys.txt` is the promotable verdict the promoter guards on, and reading the first gave a false "15 of 16 clean". True split: **7 clean · 8 defect · 1 transient** — clean are `ar_sas` `h_uti` `k_anana` `kzu_d_a` `majj_a` `men_a` `satt_a`; **no defect carried a high-confidence risk**, and the dominant classes (`markup_wrapper_dropped`, `gloss_wrapper_became_guillemet`) are exactly the ones H3658 Lane B cleared deterministically with `fix_d3` + `apply_no_yo`, so those repairs are the obvious next move on output already paid for. Promotion narrowed the input rather than `--force`ing a mixed batch (which bypasses the guard for *every* key): whole clean chunks verbatim, the mixed chunk rewritten without its flagged key, a **union** block list from all five chunks — guard reported `no intersection with incoming keys`. `--merge` without `--override-reviewed`: **store 11 482 → 11 515**, 7 cards / 33 sense rows, rows `ai_translated` and therefore out of the citable edition until G5. **Trap pinned: `--merge` is NOT dry-run by default** — `--apply` gates only `--ready-partial-report`, so a plain `--merge` writes the store; H3654's recipe carried `--apply` so the asymmetry never surfaced. Gates after the write: `refresh_tm_mirror.py` G1/G2/G3 all PASS, mirror `5ed8a96c0d62` → `ed8265065ddf` with the backup stamped **`h3663`** (the first refresh to carry its own provenance, since H3658 made `--handoff` required instead of defaulting to `H3627`); `audit_store_gates.py` rows **11 515**, `only_src=0 only_mirror=0 changed_ru=0`, exit 0; `placement_axis_check.py` **OK** (6 320 sidecar rows, 633 placed), exit 0; the 3 `SAN-LOSS` rows are pre-existing and none is a promoted key. Gate provenance: **GATE-0 health PASS** (22 318 ms vs 80 000) + **CANARY GO** (one call, 12.1 s), fired because the durable probe log held **0 entries for 29-08** and its last c1 GO was 17.9 h old — H3659's same-day receipt was worktree-local and died with the worktree, so an earlier "1 of 2 attempts used" reading was wrong and a receipt that cannot survive its worktree cannot serve as the ration record. **Spend: 35 calls, cost NOT evaluable** (`unknown_gateway`, `observed_cash_usd: null` on all 7 runs — never `$0` per §597); 403 331 output / 976 569 cache-creation / 2 658 928 cache-read / 4 038 964 subagent tokens, of which the 7 killed-window calls are pure re-payment. Packet [pwg_ru/h3663/H3663_LANE_A_16KEY_C1_WINDOW_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3663/H3663_LANE_A_16KEY_C1_WINDOW_29-08-2026.md).

## [1.144.124] - 2026-08-29
- **FINDINGS §610 — the target-side span drop is repaired, not requeued: `hasita` is promotable (Opus 5 (`claude-opus-5`), 29-08-2026, H3675, zero paid calls).** §608 left the fork open and **MG ruled `reanchor`**. New [`src/target_anchor.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/target_anchor.py), the twin of `german_anchor`, repair-then-verify at the `count_card_field` guard in both lanes. **Why this is not the [PR #789](https://github.com/gasyoun/SanskritLexicography/pull/789) boundary guess, which stands:** a `{Tn}` token is opaque and atomic — no boundary to find, only a position — and the SAME sense's `german` still carries every token in order (guaranteed: reaching this guard requires the german count to have matched), with senses pairing 1:1 by card schema. The translation is never parsed; a token is restored to the position its own parallel names — present evidence, not inference. **The rule:** per sense, each missing token goes next to its nearest surviving neighbour measured in that sense's german, with the sense START and sense END as virtual anchors — so a headword span goes to the head, a trailing span to the tail (`{T1} невозмутимый {T2}`, never `{T1} {T2} невозмутимый`), and a leading-but-not-first span before its successor. `german_anchor` cannot do this: it anchors against the card-wide SOURCE skeleton and has no per-sense end to measure to. **Measured:** `hasita~~h0_zz_pw` — requeued as unfixable across H858 → H3144 → H3157 → H3659, the card a paid window was spent on — is repaired offline, `{T7}` (`<ls>GAUT.</ls>`) landing exactly where the german parallel puts it. **A trap under both lanes, fixed:** `restore_card`/`restoreCard` unmask IN PLACE, so the german branch was consuming the `{Tn}` tokens the target repair needs as its anchor; both now restore a copy, and a german repair is re-stamped after the target repair re-restores or its provenance vanishes on a card dropped on both sides. **What the store may receive is unchanged** — only cards counting exactly right on both sides; a non-drop still rejects, now as `translation-fidelity-reject: target-anchor <reason>` (prefix preserved so every existing reader matches). New telemetry `target_anchor_repairs`/`_invocations`/`_detail`/`_outcomes`. Registered `target_anchor_repair_h3675` **SHARED** (target field is a parameter, never a literal; asserted mechanically). Window selftest 218/218, headless PASS, target_anchor_test 22 checks, parity 104/104. **Nothing promoted** — re-running the window is paid and §602/§604 still gate it. Packet [pwg_ru/h3675/H3675_TARGET_ANCHOR_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3675/H3675_TARGET_ANCHOR_29-08-2026.md).
- **FINDINGS §609 — `classify_run` reports an undersized `--max-calls` as `code-failure`, sending the reader after a harness bug that is not there (Opus 5 (`claude-opus-5`), 29-08-2026).** `no_pwg_w09` classifies `code-failure` on 3 null cards, but only `hasita` is a real gate flag — `jaw_ayus` and `kast_ur_i` are `budget_exceeded:max_calls`, never attempted, because `--max-calls 8` was 1.6× a five-card window instead of ≈ 2.5×. `classify()` counts nulls without asking why, sets `tripped` from `budget_stops` (16), and makes `code-failure` the fall-through, so /pwg-window-close then instructs the reader to debug a healthy harness and blocks the requeue that would finish the two unrun keys. Same conflation shape as §498; `budget_exceeded:*` is already separable in the `failures` map, so a `budget-truncated` verdict needs no new telemetry.
...
- **FINDINGS §608 — the rejected card is never saved, `german_anchor` would refuse it anyway, and `german_anchor_repairs: 0` is no longer unfalsifiable (Opus 5 (`claude-opus-5`), 29-08-2026, H3665, zero paid calls).** §605's code-shape verdict is **confirmed** with exact lines ([`headless_worker.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/headless_worker.py) L879 admits the repair on the german-only count; L902–906 raises `translation-fidelity-reject` on the target field and is reachable only when L879 already passed). Two things it could not know: (a) `normalize_batch` sets `card = None` on that reject and `write_failed_envelope` fires only on a process-level failure, so **`hasita~~h0_zz_pw`'s card is on disk nowhere** and §604's prescribed replay was not runnable as written; (b) rebuilding the card's control-flow-proven state from the manifest skeleton and running the real module returns **`nothing-missing`** — the repair would **decline** this card even if wired to it, so "cannot fire" becomes "cannot fire, and would decline if it did", and re-gating the call site would fix nothing. **Against §605 consequence (1):** the counter is read from a *summary*, not from two docstrings — which is how §604 was written at the cost of a paid window. Both lanes now emit `german_anchor_invocations` + `german_anchor_not_reached` (headless also `german_anchor_outcomes`), making the three states three distinct summaries: clean `0/0/[]`, german-side drop `1/1/[]`, target-side drop `0/0/[key]` — previously rows 1 and 3 were byte-identical. `repaired-then-rejected` closes a fourth hole: a repaired-then-nulled card loses its stamp with `card = None`. Pinned by `test_h3665_german_anchor_counter_is_falsifiable` (asserts the invariant, not just fixtures) and `german_anchor_test.js` CONTROL 2b/2c through the real emitted harness. **The target-side drop still has no repair** — net-new work, a human should decide between a target-side re-anchor and an explicit requeue ruling. Window selftest 216/216, headless selftest PASS, parity ledger 103/103 no drift. Packet [pwg_ru/h3665/H3665_GERMAN_ANCHOR_NOT_REACHED_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3665/H3665_GERMAN_ANCHOR_NOT_REACHED_29-08-2026.md).
