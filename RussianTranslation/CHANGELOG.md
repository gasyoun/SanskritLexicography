...
### Fixed
- **LANG_PARITY drift re-verify (H2243, 03-08-2026, Sonnet 5 `claude-sonnet-5`):** independently landed the same fix as [H2209 (#1060)](https://github.com/gasyoun/SanskritLexicography/pull/1060) — 9 ledger violations across 4 entries. `coordinator_claim_expiry_and_atomic_jsonl` + `h1386_resume_recovery_and_medium50` re-derived SHARED — `coordinator.py`/`max_account_orchestrator.py` drift is H2229's OPT-8 kitchen-collision banner (`emit_collision` bolted onto existing refusal paths) plus H2154's fail-closed `occupied_keys()` guard, neither reads a target-language field. `h1209_controller_worker_rig` + `h1210_ab_arm_scaffold` restored to SHARED: H2226/OPT-4 already field-parameterized `wf_template.js`/`wf_template_ab.js`/`control_template.js` for EN reuse and correctly re-stamped the ledger, but the later H2227/OPT-2 merge (`72c8311d`) accidentally reverted these two ledger entries to their pre-H2226 GAP verdict/hashes while leaving the parameterized code untouched — a **ledger-doc merge regression**, not fresh code drift, confirmed by direct re-hashing of the working-tree files against H2226's originally-stamped hashes. Root-cause pattern logged as [FINDINGS §516](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md#516-a-later-prs-stale-base-merge-can-silently-revert-an-earlier-prs-ledger-doc-only-re-stamp-while-leaving-that-earlier-prs-code-change-fully-intact). `lang_parity_check.py`: 0 violations. `window_selftest.py`: 201/201 passed.
