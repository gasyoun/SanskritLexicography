...
## W-series adversarial re-verification — EXECUTED 09-07-2026 (H425)

Re-ran every offline check behind the W2/W3 verdicts against the **live store**
(`src/pwg_ru_translated.jsonl`, 11,261 rows) on `master` `9ac9ed1`, adversarial
stance: try to make the machinery emit unsupported evidence or report
`silent`/negative for something merely unmeasured. Driven by Sonnet 5
(`claude-sonnet-5`); the checks themselves are deterministic, LLM-free.

**Verdict: clean — no fabrication, no status-conflation.** Every W2 count
reproduces exactly; both fabrication spot-checks reproduce 20/20; the
corpus-gate status machinery distinguishes absence from negative with explicit
markers. Two W3 notes below (a −2 store-surface drift and the still-`PARTIAL`
0-populated `government` field) are consistent with this doc's own W3 verdict,
not regressions.

| check | audit-stated | re-measured (H425) | verdict |
|---|--:|--:|:--|
| W2 rows with ≥1 evidence | 2,269 (20.1%) | **2,269 (20.1%)** | ✅ exact |
| W2 koch provides/supports/contradicts/silent | 155 / 578 / 17 / 74 | **155 / 578 / 17 / 74** | ✅ exact |
| W2 kna / fri / smirnov / kow / grin* / apte_hi / vedic / kosha_syn / meulenbeld / corpus | (per §W2 table) | **all 12 sources identical** | ✅ exact |
| koch xref: entries / crosswalk / bare / resolved / unresolved | 29,177 / 27,447 / 3,471 / 3,208 / 263 | **29,177 / 27,447 / 3,471 / 3,208 / 263** | ✅ exact (post-H409) |
| fri xref resolved | 111 (32.9%) | **111**; `--selftest` OK | ✅ |
| koch fabrication spot-check (`random.seed(42)`, n=20) | 20/20 clean | **20/20 clean** — every resolved gloss exists in koch under the target key1 AND its headword matches the `см. X` redirect target | ✅ reproduced |
| fri fabrication spot-check (`random.seed(42)`, n=20) | 20/20 clean | **20/20 clean** — every resolved gloss exists in fri, headword matches the `v.`/`cf.` target | ✅ reproduced |
| W3 raw-source census: markers / entries / senses | 3,853 / 1,476 / 3,222 | **3,853 / 1,476 / 3,222** | ✅ exact |
| W3 backfill parses `de`, never `ru` | spec | **confirmed** ([`annotate_government.py:63`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/annotate_government.py) + docstring) | ✅ |
| W3 no LLM in the government path | spec | **confirmed** — no model import; cases copied verbatim from the `de` span (e.g. `(<ab>acc.</ab>)`), never invented | ✅ |
| W3 store-surface (rows whose `de` yields ≥1 marker) | 510 | **508** | ⚠️ −2 drift (ongoing store regeneration; extractor unchanged — raw census still 3,853 exactly) |
| W3 stored `government` field populated in the live store | PARTIAL / 0 populated (§ summary table) | **0 rows carry the field** | ✅ consistent — the backfill machinery is correct but has never been run against the committed store; `annotate_government.py --check` shows stored `[]` vs 508 re-extractable (a not-yet-applied deliverable, **not** a fabrication or silent loss) |
| corpus-gate status markers `ok`/`db_absent`/`db_error`/`skipped_short_term`/`evidence_unavailable` all explicit | FL7 spec | **confirmed** — `corpus.db` present ⇒ `corpus_examples_with_status('gam')` returns `ok`; never silently degrades to LLM-only | ✅ |
| `#ИМЯ?`/`#ЗНАЧ?` spreadsheet-error guard (`_SPREADSHEET_ERR_RE`, H409) | strips before tokenizing | **confirmed** — `ru_tokens('#ИМЯ?')` = `∅`; the embedded `имя` never leaks as a gloss token | ✅ |

**On the two W3 notes.** (1) The 510→**508** store-surface figure is a −2 drift
from the store being regenerated by ongoing promote runs since the 08-07 audit;
the extractor is byte-identical (raw-source census still returns 3,853 exactly),
so no logic regressed. (2) The live store's `government` field is **0-populated**
— which matches this doc's own W3 summary verdict (**PARTIAL** — "schema slot
exists, 0 populated"). The H335/H338 census + extractor + `annotate_government.py`
backfill are all correct and deterministic; the backfill simply has not been run
against the committed store. Because H425's fix-PR trigger is a *proven
fabrication or status-conflation* (neither found), running the backfill is out of
this re-audit's scope — flagged here as the outstanding W3 delivery step, not a
defect corrected in this pass.

_Auto-generated table by Sonnet 5 (`claude-sonnet-5`), H425._
