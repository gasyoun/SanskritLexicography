...
3. **CI guard unblocked for every future audit.** [`review_changelog_guard.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/review_changelog_guard.py) required a literal `RussianTranslation/CHANGELOG.md` touch, but since H3355 direct `[Unreleased]` bullets are hook-blocked and entries go to `changelog_queue/` — so any queued review or audit doc failed the «RussianTranslation gates» job (this PR's first run did). The guard now accepts a `changelog_queue/*.md` file in the same diff; selftest extended.
