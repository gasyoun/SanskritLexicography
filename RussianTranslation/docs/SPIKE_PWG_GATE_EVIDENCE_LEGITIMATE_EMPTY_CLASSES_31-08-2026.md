...
|---|---|---|---|---|
| `lane_spotcheck_freshness` | `C2-2` | *(none)* | Not because emptiness is impossible, but because this gate **never passes on it**. With no candidate report the verdict is `inconclusive`, not `pass` — the family's own exit-code contract ("a surveillance job that dies must not read as a pass", `lane_spotcheck_tick` docstring) — and `nonstop_scheduler` fails closed on auto-promote. A declared-empty PASS here would launder a refusal into a green light. | — |
