...
| **A6** residual ledger append from defect requeue | **SHIPPED** — `requeue_from_audit` → `no_pwg_residual_ledger.append_from_audit_report` (opt-out `--no-residual`); O(n²) append fixed H1940 (git-verified 03-08-2026, commit `a75eaa17`; this memo originally miscited H1811) | Verify only |
