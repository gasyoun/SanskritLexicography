...
**The complement of the 11:06 reading ([H2174](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/RESULTS_LOG.md)).**
Two hours earlier all three candidate gate numbers *failed* (96 520 / 77 966 / 69 137 vs
65 000); here all three *pass* (60 845 / 40 623 / 36 508) — so the clock question (settled on
wall, H2160 option A) is again not what stood between this pipeline and a window.

Five measured attempts on 02-08: **PASS → NO-GO → NO-GO → NO-GO → PASS**
(43 815 → 75 561 → 96 520 → 66 291 → 60 845 ms wall) — **2/5**. This does not weaken H2174's
second-pass verdict; it instantiates it. That pass fitted the ceiling against the full series
(**2/8 = 25 %** implied pass rate, **median − ceiling = +11 988 ms**) and concluded the gate is
*expected* to fail ~75 % of the time, so the remedy is re-deriving the ceiling **value**
...
not re-rolling for a lucky reading. This reading is one of the 25 %, taken 18 minutes after
the 12:46 NO-GO on the same profile and prompt.

The practical sting: **a GO cannot be scheduled, and this one expired unusable.** Reaching it
cost 4 NO-GOs and ~$2.9 in probes across the day, and the one thing it gates — the canary —
could not be fired for the reason in §3.
