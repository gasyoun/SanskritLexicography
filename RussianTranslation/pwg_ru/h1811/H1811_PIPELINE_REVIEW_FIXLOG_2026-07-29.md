...
| H2b budget note preserve | 🟡 IMPLEMENTED 31-07-2026 (H1940 Phase 2, OpenAI GPT-5.6 Sol `openrouter/openai/gpt-5.6-sol`), pending merge — when a whole-card retry is refused by `budget_exceeded:*`, `resolve_group` now preserves the earlier per-key content note instead of replacing every pending key with the batch-wide budget stop. The preservation flag is conditional on the typed budget prefix, so ordinary retry errors keep their historical last-error behavior. Explicitly separate from H2a: this is the whole-card translate lane, not presplit fragment-to-base propagation | pending | `headless_worker_selftest.test_h2b_translate_budget_preserves_attempt_content_note`, verified RED on pre-H2b `origin/master`: attempt 1 recorded `translation-fidelity-reject`, retry refusal overwrote it with `budget_exceeded:translate` |
