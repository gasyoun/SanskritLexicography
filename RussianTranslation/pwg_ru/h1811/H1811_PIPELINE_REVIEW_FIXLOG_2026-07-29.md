...
_Created: 29-07-2026 · Last updated: 31-07-2026_
...
| H1 manifest crash | ✅ DONE 31-07-2026 (H1940 Phase 2), merged `62993b6b` — the manifest `open`/`read`/`sha256`/`json.loads` moved inside `main()`'s pre-existing configuration `try`, and `KeyError`/`TypeError` joined its `except` tuple. Every one of the five failure shapes now writes a status file with `classification: configuration` and exits 2 instead of escaping as a bare traceback. **The worker half alone would NOT have stopped the retries** — an acceptance review caught that overclaim, and the orchestrator was measured rather than assumed: `run_claimed` hashes + `json.load`s the manifest BEFORE launching the worker, unguarded, so a missing file or invalid JSON escaped `run_claimed` entirely (and left the job stuck `in_progress`, which `_claim_tx` then treats as the account already owning a job — one bad manifest wedged the account); and `fail_or_retry` keys purely on `attempts < max_attempts`, so a `configuration` verdict returned to `pending` (measured on master: `attempts=1, max_attempts=3` → `'pending'`). So H1 also guards that pre-launch read and adds a separate `fail_terminal()` for the deterministic classes (`configuration`, `manifest_drift`) — deliberately NOT a branch inside `fail_or_retry`, so transient classes keep their retry budget untouched — and puts `configuration` into the readiness report's hard-failure set, since a class that permanently kills a job must not vanish from it. `manifest_hash` is pre-bound to `None`, which both removes the `UnboundLocalError` the move would otherwise have introduced and keeps a never-read manifest from being attested with a fabricated hash — `null` is the absent-hash shape `bounded_staged_run`'s `headless.get('manifest_sha256')` and `emit_call_events`' `or 'call'` fallback already handle. When bytes WERE read their hash is retained exactly as before, so an invalid-JSON status still carries real evidence of what was rejected. `KeyError`/`TypeError` are type-qualified in the error detail (a bare `'inputs'` names no cause); the pre-H1 types keep their exact wording, which matters because `max_account_orchestrator` feeds `status['error']` to `parse_reset` on the rate-limit path | [PR #904](https://github.com/gasyoun/SanskritLexicography/pull/904) | `headless_worker_selftest` ×4, all verified RED against pre-H1 master where the exception escapes and no status file is written: unreadable manifest (exit 2 · configuration · no output/spawn · `manifest_sha256` null), invalid JSON (hash of the ACTUAL malformed bytes retained, computed independently of the code under test), structural `KeyError` via a manifest with no `inputs` (raised in `build_prompt`, an argument to `HeadlessEngine.call`, so it precedes any reservation or spawn), structural `TypeError` via a scalar `fragment_tm` group (raised in `_validate_fragment_tm`, before the config-dir check and the profile claim). Plus `max_account_orchestrator_selftest` ×4 driving the REAL `run_claimed` with `max_attempts=3`, so `failed` proves the class ended the job and not exhaustion — **3 master-failing** (missing manifest → escapes; invalid JSON → escapes; worker `configuration` → `'pending'`) and **1 deliberate regression guard, green on master by construction** (a `process` failure must still return to `pending`; it exists to prove retry was not globally disabled). Stated rather than rounded up to 4/4 RED, per the H2a precedent |
...
| H2b budget note preserve | ✅ DONE 31-07-2026 (H1940 Phase 2, OpenAI GPT-5.6 Sol `openrouter/openai/gpt-5.6-sol`), merged `9a5bddbc` — when a whole-card retry is refused by `budget_exceeded:*`, `resolve_group` now preserves the earlier per-key content note instead of replacing every pending key with the batch-wide budget stop. The preservation flag is conditional on the typed budget prefix, so ordinary retry errors keep their historical last-error behavior. Explicitly separate from H2a: this is the whole-card translate lane, not presplit fragment-to-base propagation | [PR #906](https://github.com/gasyoun/SanskritLexicography/pull/906) | `headless_worker_selftest.test_h2b_translate_budget_preserves_attempt_content_note`, verified RED on pre-H2b `origin/master`: attempt 1 recorded `translation-fidelity-reject`, retry refusal overwrote it with `budget_exceeded:translate` |
| H3 fsync checkpoints | ✅ DONE 31-07-2026 (H1940 Phase 2, Opus 5 `claude-opus-5[1m]`) — `flush()` + `os.fsync(f.fileno())` added before the pre-existing `os.replace` in all three writers (`headless_worker.atomic_json`, `bounded_supervisor._write_checkpoint`, `cohort_engine._save_checkpoint`). **Landed as option A (inline), NOT the spec's "route through the fsynced atomic writer" — the call and its reason, per the ruling that required measuring first.** [`src/pilot/h3_byte_probe.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/h3_byte_probe.py) (committed, so the measurement is reproducible) shows route-through is **not** byte-neutral: `window_common.atomic_write_text` passes no `newline=` to `os.fdopen`, so it emits **CRLF on Windows** and drops the trailing newline — 246 bytes vs 232 on the probe payload, diverging at **offset 1**. Those bytes are hash-bound, and the same shared writer also emits the execution manifest whose `sha256_path` digest IS `manifest_sha256` ([`no_pwg_scale_plan.py:519`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/no_pwg_scale_plan.py) → `:312`) plus the preflight evidence — so pinning `newline=` there is a hash migration across several gate-pinned artifacts, not a durability fix, and is explicitly out of scope for a "surgical fixes only" item. Recorded instead as [Uprava FINDINGS §262](https://github.com/gasyoun/Uprava/blob/main/FINDINGS.md). Note the sketch's stated motive for option A was *wrong* in the opposite direction: it feared losing `atomic_json`'s `.tmp.<pid>` suffix, but `atomic_write_text` uses `tempfile.mkstemp` (O_EXCL, unique per call), which is strictly stronger than a pid suffix — the real blocker is the newline, not the tmp name. Inline mirrors the canonical writer exactly: file-fsync only, since its own directory fsync is `os.name != 'nt'`-guarded and lives in `fsync_existing_path` | this branch | three pins, one per site, each **verified RED on pre-H3 master** (`got [('replace', …)]` — no fsync at all): `headless_worker_selftest.test_h3_atomic_json_fsyncs_before_replace`, `bounded_supervisor_selftest` **(q)**, `cohort_engine_selftest` pin **10**. The probe instruments `os.fsync`/`os.replace` and calls `os.fstat(fd)` inside the fake fsync, which raises on a closed descriptor — so none of the three can pass against an implementation that fsyncs a stale fd. Each also asserts the on-disk bytes are unchanged (no CRLF, trailing newline intact), pinning the byte-neutrality that justified option A |
| H4 dup lease-id | ✅ DONE 31-07-2026 (H1940 Phase 2, Opus 5 `claude-opus-5[1m]`) — `coordinator.claim` now refuses a `lease_id` already present in `state['leases']`, the exact guard `register_prepared_lease` has always carried, placed immediately after the id is computed and **before** the artifact-dir `makedirs` / `leases.append` / `save_state`, so a refused claim leaves no directory, no lease, no saved state and no registry event and `with DirLock(...)` releases on the way out — clean by construction, the H8 pattern. Applied to the **auto-generated** id too, not only an explicit `--lease-id`: `make_lease_id` embeds a second-resolution UTC stamp *and* `os.getpid()`, so a collision there needs the same kind+lane+target within the same second from the same pid, which is itself a genuine duplicate. Two claims for the same **target** stay legal — they get distinct ids and never reach the guard. Why the old behaviour was worse than "an extra row": every lookup goes through `lease_by_id`, which returns the FIRST match, so the second lease was unreachable-but-persisted — still consuming a preparation slot and still appearing in `active_targets`/reserved-key scans while breaking the single-id CAS assumption the rest of the coordinator rests on | this branch | `coordinator_hardening_selftest.test_h4_claim_rejects_duplicate_lease_id`, **verified RED on pre-H4 master** (the second claim simply succeeds — no `SystemExit` raised at all). The fixture seeds **two** runnable roots deliberately: `verb_candidates` filters out `active_targets(state)`, so with a single root the second claim would die at `no runnable verb candidate` *before* reaching the guard and the pin would have passed on master for entirely the wrong reason. It also asserts the refused claim emits no registry event and mutates no existing lease, that the `DirLock` is reacquirable, and that a **distinct** id still claims — the guard refuses duplicates without wedging the lane |
| H7 drain spin | ✅ DONE 31-07-2026 (H1940 Phase 2, Opus 5 `claude-opus-5[1m]`) — `make_run_window`'s per-lease drain loop now computes a `(pending, done_unrecorded, done)` progress signature per pass; an unchanged signature sleeps `DRAIN_IDLE_POLL_SECONDS` (3 s, the exact bound `mao.STAGED_RUN_IDLE_POLL_SECONDS` already uses for the staged C4 backstop) and counts the pass, and `DRAIN_NO_PROGRESS_PASSES` (20) **consecutive** dead passes fail closed naming the lease and all three counters. Four design points that are not free choices: (1) the counter is consecutive, not cumulative, so a window that stalls-recovers-stalls still completes; (2) `done` TOTAL is in the signature, not just the unrecorded slice — a requeue landing a new pending job in the same pass another completes leaves the other two counters equal and would read as a stall; (3) the check sits **ahead** of the `if pending:` branch, so it also covers a done-but-unrecordable job — the stall shape C4 does **not** cover, since C4 guards only inside its own pending branch; (4) it sits **after** the clean-completion `break`, so the H1386 C2 resume path still breaks on pass 1 and never reaches the backstop (tests (l)/(m) green). `max_drain_iterations` is untouched — the new cap is an earlier, more specific stop, deliberately not derived from a ceiling that legitimately covers many *productive* passes. Cost of the old behavior, since it was worse than "no sleep": each dead pass also re-dispatched, re-recorded and spawned another `promote-ready` subprocess, so the default ceiling meant up to ~1000 pointless spawns before an error message that named a counter instead of the stall | this branch | `bounded_staged_run_selftest` ×3 driving the REAL `make_run_window` against a real sqlite db + admitted account: **(s)** zero-claim stall stops on the backstop (6 dispatch passes, 5 polls, message carries `pending=1`), **(s2)** progress resets the CONSECUTIVE counter (4 no-progress passes total against a cap of 3, across two separate stalls — a cumulative counter would have raised), **(s3)** done-but-unrecordable stall stops too and never dispatches. **(s) and (s3) verified RED on pre-H7 `origin/master`** by a scratch harness using only master's API (`max_drain_iterations=30`): master ran the full 30 passes with **zero** `time.sleep` calls and died `exceeded 30 drain iterations`, versus 20 passes / 19 polls / stall named. (s2) is master-failing too, but only on its poll assertion (master never sleeps) — stated rather than claimed as a third behavioural RED, per the H2a precedent |
| H8 preflight timeout | ✅ DONE 30-07-2026 (H1940 Phase 2) — the claim-path `perf_preflight` `run_cmd` in `verb_candidates` now carries `timeout=PREPARE_TIMEOUT_SECONDS`, the same bound the requeue-preparation and harness-generation preflight calls already used, and `subprocess.TimeoutExpired` becomes a deterministic `SystemExit`. `subprocess.run` kills and reaps the child, and the raise lands ahead of the artifact-dir `makedirs` / `leases.append` / `save_state`, so a timed-out claim releases the global `DirLock` with no lease, no artifact dir, no saved state and no registry row. Moving the candidate computation and store/worklist scans out of the lock remains the deferred larger refactor | [PR #903](https://github.com/gasyoun/SanskritLexicography/pull/903) | `coordinator_hardening_selftest` pins `test_h8_claim_preflight_timeout_is_bounded` (the claim path must receive `timeout=600`, not `None`) and `test_h8_claim_preflight_timeout_unwinds_clean` (a **real** hanging child under a short injected timeout: bounded termination, child pid dead, nothing persisted, `DirLock` reacquirable) — both verified RED against pre-H8 master, where the child runs to completion and a lease is claimed anyway |
...
| O(n²) residual-ledger append | ✅ DONE 31-07-2026 (H1940 Phase 2, Opus 5 `claude-opus-5[1m]`) — the waste-list item. `append_residual` re-read the ENTIRE append-only ledger on every call, so `append_from_audit_report` was O(keys × ledger bytes) on the hot path (called per audited window against a ledger that only grows). It now takes an optional `latest=None` snapshot: omitted, it re-reads exactly as before, so every existing caller is untouched; a batch caller reads once and threads it through. The load-bearing subtlety is the **write-back** — on a successful append the new row is written into `latest`, so a later key in the same batch sees it without re-reading; without that, a batch containing the same key twice would append it twice, and the batched dedupe would NOT equal the per-call re-read. `backfill_documented` had byte-for-byte the same shape and the same two-line fix, so it was fixed too rather than left to be re-filed later (the fixlog named only `append_from_audit_report`) | this branch | `no_pwg_residual_ledger.selftest` extension, **verified RED on pre-fix code**: a 12-key batch (plus one deliberate duplicate) re-read the ledger **13 times** against **1** after the fix. It also asserts the duplicate is written once, that the ledger holds exactly 12 rows, and that the returned key list is unchanged — so the pin covers dedupe equivalence, not merely the call count |
