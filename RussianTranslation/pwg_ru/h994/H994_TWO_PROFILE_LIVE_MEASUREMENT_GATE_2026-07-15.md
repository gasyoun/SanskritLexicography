...
> **Handoff note (corrected 16-07-2026):** the canonical handoff for this work is **[H963](https://github.com/gasyoun/Uprava/blob/main/handoffs/H963-Opus_SanskritLexicography_pwg-ru-four-profile-live-ladder-acceptance_15.07.26.md)** (MG's, minted 15-07 14:36). This report was written under **H994**, a handoff I minted later the same day off a **stale local clone** that had not yet fetched the H963 mint — i.e. a **duplicate**. H994 is now archived and this measurement is **consolidated into H963's Execution log**. This report keeps its `pwg_ru/h994/` path (stable published URL); it is the detailed record for H963's partial execution.
...
- c4's status rests on the probe + a two-call diagnostic, not a full generation: it is **responsive**
  (returns valid structured output on a natural prompt) but **~30–53 s under a representative payload →
  latency NO-GO** (see the Rung 2 UPDATE — the earlier "fast/sub-30 s" read was an `'x'`-padding artifact).
  Responsiveness is a precondition for — not proof of — a clean card generation.
...
_Auto-generated by Opus 4.8 (`claude-opus-4-8[1m]`) executing the H963 ladder (this report was generated
under the duplicate H994 handoff, now archived + consolidated into H963), Ultracode; two-profile live
