...
4. **Build the three-taxonomy genre crosswalk with inter-taxonomy κ** — the traditional lane is largely derivable from the per-work genre in [`ls_source_map.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/ls_source_map.json); DCS text-type lives in `dcs_freq_dims.json`; only the library-science layer is fully net-new. Measuring agreement empirically answers "can we use several?" — yes, if the disagreement cells are informative. *[backlog #4]*
...
**Claim.** DCS computational, traditional Sanskrit, and library-science genre taxonomies crosswalk above chance, and disagreement concentrates on boundary genres (`itihāsa`/`purāṇa`, `śāstra`/`tantra`). **Data.** Work list from `ls_source_map.json` (traditional genre already per work) × `dcs_freq_dims.json` genre (DCS) × a new library-science labelling. **Method.** Pairwise inter-taxonomy κ ([Artstein & Poesio 2008](https://aclanthology.org/J08-4004/)); confusion matrix per pair. **Figure.** Three-way chord / confusion matrix. **Research read:** empirically settles "use several?" — the disagreement cells *are* the finding (contested texts). **Learner read:** genre is a lens, not a fact. **Nearest ID + delta:** RENOU's 20-register lattice is *one* taxonomy; E6 is the multi-taxonomy crosswalk with κ *between* taxonomies. **Readiness:** two of three lanes derivable; library-science labelling + κ is the work. Reconcile the traditional lane with `csl-atlas` semantic-field work to avoid a parallel taxonomy.
...
- **Library-science taxonomy choice** (UDC / LCSH / a bespoke Indological subject spine) — a genuine open decision, mirrored to GTD as `@DECIDE`.
