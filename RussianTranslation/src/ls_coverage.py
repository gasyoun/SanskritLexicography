...
`DHĀTUP.` -- four numbers:
  ...
  full_text_enriched carry the recognized full text (Spr. (II): the saying in the
                     Indische Sprüche corpus. DHĀTUP.: the Palsule artha-index
                     record for the root at that gaṇa,serial coordinate — H1333)
...
GH = sibling_root(HERE)    # GitHub/ (csl-orig is a sibling repo)
    ...
    for n_attr, vis in pairs:
        ...
        if cls == 'Spr. (II)':
            if spr.second_ed_num(n_attr, vis) is not None and spr.saying(spr.second_ed_num(n_attr, vis)):
                c['enriched'] += 1
        if cls == 'DHĀTUP.':
            # H1333. `coord` counts the citations that CAN be Palsule-keyed at all
            # (a gaṇa-only `DHĀTUP.` cannot); `enriched` counts those the committed
            # concordance actually resolves. Reporting both keeps the partial
            # crosswalk visible instead of rounding it up to the class total.
            if dhp.coord(n_attr, vis) is not None:
                c['coordinate'] += 1
                if dhp.record(n_attr, vis):
                    c['enriched'] += 1
        if cls == 'P.':
...
def md_table(per, pfull, source_label):
    ...
    for c in CLASSES:
        d = per[c]
        enr = ('%d' % d['enriched']) if c in ('Spr. (II)', 'DHĀTUP.') else '— (n/a)'
    ...
    d3 = per['DHĀTUP.']
    st = dhp.stats()
    L.append('- **`DHĀTUP. x,y` → Palsule (H1333):** %d / %d citations carry a gaṇa,serial '
             'coordinate; %d of those (%.1f%%) resolve to a Palsule artha-index record via the '
             'committed concordance `src/data/dhatup_palsule.json` (%s of the %s distinct '
             'coordinates PWG cites, %s%% — the rest are coordinates Böhtlingk lists under two '
             'root spellings, dropped rather than guessed, or roots absent from Palsule\'s artha '
             'index; %s = %s%% link without the two disambiguation filters). **Accuracy, measured '
             'separately:** over the %s coordinates where PWG itself prints the dhātupāṭha artha '
             'beside the citation — an independent witness — our record names it %s times exactly '
             '(%s%%), %s allowing citation-form variation (%s%%). A record is ROOT-level: Palsule\'s '
             'index cannot separate the homonyms Böhtlingk numbers apart. Palsule has no online '
             'edition, so the datum is tooltip text carrying the printed page (`P167`), never an '
             'href; the gaṇa-level Westergaard link is unchanged.'
             % (d3['coordinate'], d3['total'], d3['enriched'],
                pct(d3['enriched'], d3['coordinate']),
                st.get('coords_linked', '?'), st.get('coords_cited', '?'),
                st.get('match_rate', '?'),
                st.get('coords_linked_without_filters', '?'),
                st.get('match_rate_without_filters', '?'),
                st.get('inline_artha_examined', '?'), st.get('inline_artha_agree', '?'),
                st.get('inline_artha_agreement_rate', '?'),
                st.get('inline_artha_agree_loose', '?'),
                st.get('inline_artha_agreement_rate_loose', '?')))
    L.append('')
