...
## Специализированные глоссарии имён — `build_glossaries.py`

Помимо пяти общих словарей выше, [build_glossaries.py](build_glossaries.py) читает
**сырые** файлы `Data/*.txt` SamudraManthanam (индексы имён при печатных изданиях
Рамаяны — они НЕ проходили через `corpus_builder/jsonl`, в отличие от источников
`build_src.py`). Это текстово-специфичные словари ИМЁН, не общая лексика — держатся
...
- `Словарь Потаповой.txt`, `Эрман-Темкин.txt`, `Словарь Гринцера из Бада
  ...
  русской транскрипции) не существует и в репозитории её нет — строить
...
python build_glossaries.py [путь-к-SamudraManthanam] # 2 специализированных глоссария имён
