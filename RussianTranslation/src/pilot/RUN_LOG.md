...
**v1 run (prior Opus session, 9 agents, 645,849 subagent tok, 39m24s): the rig's own gates lied.** Workflow self-report 3/3 `would_promote`; canonical audit **1/3** — `nakzatra`/`sakft` fidelity-REJECT (`<ls>` 76/79 and 67/74, `{#` 30/34 and 28/32, identical deficits in german AND russian). Root cause, deterministically proven (every missing token and every count deficit exactly accounted for): the v1 in-JS gate HARD-required sense-count EQUALITY against the naive `senses` glyph count (7/4/10 — counts PW cross-reference ordinals; the H960-hardened `source_senses` is 3/2/3), so the retry feedback steered workers to FIT the wrong count by displacing PW/SCH supplement records into `card.notes` — not a restored field (C-01), mask-stripped at render (H858) ⇒ **source content silently lost while every rig gate showed green**. Incident `H1209_SLICE_V1_2026-07-18`, classified `gate-bug`, in [LAUNCH_FUCKUPS.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/LAUNCH_FUCKUPS.md). An independent adversarial review confirmed `canonical_audit.py` mirrors `accept()` on 7/7 checked dimensions (restore semantics, both HARD fidelity gates — regex-dialect equivalence tested empirically on all 168 field strings — TNMASK multiset, SAN-LOSS direction, ordering); no verdict flippable.

**Fix (v2 gates — direction-aligned with the canonical battery; the transferable lesson: a non-canonical gate in a side harness is WORSE than none, it manufactures the defect it cannot see):** HARD `{Tn}`-multiset fidelity over `record.grammar`+`sense.german` AND over `sense.russian` (mask-level supersets of `accept()`'s two HARD rejects — needs no placeholder payloads in-JS); sense gate SHORTFALL-ONLY vs `source_senses` (canonical SAN-LOSS direction — over-emission never flagged, a faithful split may yield MORE senses); retry feedback explicitly forbids notes-parking.

**v2 run (`wf_e858f3cf-6af`, this session, 8 agents, 544,056 subagent tok, 44m10s): canonical 3/3 PASS** — 79/79 34/34 · 78/78 34/34 · 74/74 32/32, schema ok, zero soft flags, **self-report == canonical**. Attempts 3/1/1 — `nakzatra`'s two bad attempts were caught and fixed by the FREE deterministic retry (controller calls stayed 1/card); it now carries the PW/PWKVN/SCH supplements as separate sense rows (10 emitted ≥ 3 source), notes = commentary only. `sakft` drew 2 genuinely useful MINOR non-blocking controller notes (homonym-index placement; a dropped `[Page7-0508]` page marker) — the Opus controller adds review value beyond the free gates.

**Slice metrics vs the H1209 measure table (n=3, promote-DRY — honest caveats, no silent caps):** audit-clean 3/3 = 100% (планка ≥80%; H911 baseline ~62% — but n=3 and ALL 3 cards are complexity-flagged (16.43/16.93/12.66, threshold 4.0), so this validates the RIG, not yet the production rate); calls/clean = 8/3 ≈ 2.67 (планка ≤1.75 — over on this deliberately all-COMPLEX slice, 1 card burned 2 free retries; medium50 measures the real rate); controller overhead = 3/8 calls = 37.5% of calls (the Workflow journal records no per-role TOKEN split — token-level overhead unmeasurable at this granularity, named as a gap); retries 2/5 worker calls, escalations 0, review-sheet 0; complexity-trigger false-flag rate unmeasurable on an all-complex slice (needs the medium50 mix). Promote-DRY only — NOTHING was promoted to the store.

**Deferred (unspent, queued):** medium50 RU batch (the планка metrics) and mini-EN ≤15 (H1070/H1152 guards + `audit_window_en` + first `promote_en`); the rig is RU-lane — GAP entry `h1209_controller_worker_rig` in [LANG_PARITY.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/LANG_PARITY.md), tracked by H1209's mini-EN step. Committed artifacts under [`src/pilot/h1209/`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/h1209/canonical_audit.py): the four rig scripts, the manifest, both as-run Workflow scripts (`wf_run.js`/`wf_run2.js`, payloads embedded), both slice results and both canonical audit reports.
