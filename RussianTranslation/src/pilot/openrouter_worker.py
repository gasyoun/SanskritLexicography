...
PROVIDERS = {
    ...
    'openrouter': {'base': 'https://openrouter.ai/api/v1',
                   ...
                   # OpenRouter id may lag first-party; override with --model when needed.
                   'default_model': 'deepseek/deepseek-chat'},
}

# First-party Flash 0731 list prices (USD / 1M tok) — keep in lockstep with deepseek_arm.
...
SAME frozen sample (`sample_manifest.json`), same prompts, same free det_gate, same
blinded Opus + det_gate protocol (judge never sees the arm label). Model B default:
`deepseek-v4-flash` (H2439 retarget; older H1210 arm B used `deepseek-chat`).
...
def selftest():
    ...
    assert PRICE_CACHE_MISS_IN == 0.14 and PRICE_CACHE_HIT_IN == 0.0028
    assert PRICE_OUT == 0.28
