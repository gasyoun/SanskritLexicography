export const meta = {
  name: 'h1210-ab-arm-a-claude-native',
  description: 'H1210 A/B arm A (Claude-native): Opus controller + Sonnet workers on the 100-card stratified PWG slice — H1209 v2 rig verbatim, card loop parallelized',
  phases: [{ title: 'Translate' }, { title: 'Control' }],
}

// H1210 delta vs the H1209 v2 template — exactly ONE change, named here so the A/B is
// auditable: the per-card chain below is IDENTICAL (same gates, same <=2 free retries,
// same sticky controller rejection, same 300 s agent deadline), but the CARD LOOP is
// scheduled with parallel() instead of `for (const c of CARDS)`. H1209 locked a strictly
// serial loop because it was a 3-card canary; at 100 cards serial execution is ~8 h of
// wall clock for a measurement that has no cross-card state. Scheduling only — no gate,
// prompt, model, or retry semantics differ, so arm A's numbers stay comparable to the
// H1209 slice and to arm B (which runs the same chain card-parallel in Python).
...
// H1210 delta 2 (measured, not guessed): H1209 set 300 s for a 3-card canary. The 28-07
// probe run on `nakzatra` (12.5 KB card_block) burned all three attempts and ended
// `worker-null-death` because single worker calls on a large card legitimately take ~5 min
// — a TIMEOUT being scored as a generator defect would silently bias arm A's clean rate
// downward. Raised to 900 s: still a hard backstop against a hung call (B07's whole point),
// but above the measured honest-completion time for the biggest cards in this slice.
...
// parallel() is a barrier, which is exactly right here: the report needs every card's row
// before it can be written, and there is no downstream stage to pipeline into. A thunk that
// throws resolves null; a null row is kept as an explicit 'card-thunk-null' REJECT rather
// than vanishing from the denominator (no silent caps).
const raw = await parallel(CARDS.map(c => () => runCard(c)))
const results = raw.map((r, i) => r || {
  key1: CARDS[i].key1, complexity: CARDS[i].complexity, attempts: 0, self_report: null,
  det: null, controller: null, controller_calls: 0, final_status: 'card-thunk-null', card: null,
})
