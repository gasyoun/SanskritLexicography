...
CLASS_RU = {
    ...
    'wrong_entry': ('чужая статья — случайный двойник',
                    'Слова НЕ родственны (advan «essend» ≠ adhvan «дорога»): их связала только коллизия уплощения d/D и т.п. Инжест взял постороннюю статью по уплощённому ключу.'),
    'wrong_entry_xref': ('чужая статья — родственная отсылка',
                         'Попавшая карточка сама печатает целевую лемму (anukampa «s. anukampā» — пара m./f.; asru «s. aśru»): это родственный стаб, указывающий на цель. Механизм тот же, но пара связана.'),
...
def main():
    ...
    config = {
        ...
        'filters': [('wrong_entry_dup', 'дубликация'),
                    ('wrong_entry', 'случайный двойник'),
                    ('wrong_entry_xref', 'родственная отсылка'),
                    ('junk_key1', 'мусорный key1')],
