...
### §8b. Fitting a continuous locus index for the **full Mahābhārata** against any free bulk e-text — ✅ REOPENED & DONE for ALL 18 PARVANS (the "no free vulgate" premise was refuted, 12-07-2026)
🟢 ✅ **Porting the Harivaṃśa fitted-index method (H488) to the MBH to resolve PWG/MW `MBH. book,verse` loci — done for the whole epic: a free bulk Nīlakaṇṭha vulgate WAS obtained.**
**Reversal (H761 book 7, then generalized to all 18, 12-07-2026, Opus 4.8 `claude-opus-4-8`).** The blocking premise below is **false as of 11-07-2026**: a full Nīlakaṇṭha-vulgate MBH e-text (83,971 shlokas, 18 parvans, per-adhyāya P/U/A/S addressing) is **free** at [sanatana.in](https://sanatana.in/mahabharata/) (scraped into CommentaryStrategies [PR #83](https://github.com/gasyoun/CommentaryStrategies/pull/83); independently re-harvested csl-atlas-native by [`f8_mbh_harvest.py`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/scripts/forensic/f8_mbh_harvest.py)) — not D3-gated Manipal, not OCR. The f8 fitted-index fit + held-out gate runs exactly as f7 did. **First book 7 (H761):** held-out MW 48.1 % (90/187) vs 0.0 % null; exemplar `MBH. 7,9283` → vulgate 7.200.24 (*…droṇaputram athābravīt*). **Then all 18 parvans ([`f8_mbh_resolve.py`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/scripts/forensic/f8_mbh_resolve.py), [csl-atlas PR #261](https://github.com/sanskrit-lexicon/csl-atlas/pull/261)):** pooled held-out MW **55.2 % (2,234/4,048) vs 1.4 % null (≈ 40×), every parvan passing individually**; book 7 reproduces the 90/187 exactly (two independent harvests of the same source). Note-locus census: 409 corroborated / 787 displaced / 1,270 absent. A parva whose digitization diverged from PWG's Calcutta numbering would **fail** its own gate — none did. Evidence: [`MBH_CITATION_RESOLUTION_CENSUS.md`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/forensic/MBH_CITATION_RESOLUTION_CENSUS.md) §2/§4 (all 18) + [`MBH_DRONA_FITTED_INDEX_CENSUS.md`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/forensic/MBH_DRONA_FITTED_INDEX_CENSUS.md) (book-7 detail). The retrieval-lane result below stands as the complementary (BORI reading-evidence) verdict; the *fitted-index deferral* is fully lifted.

_Historical record (the deferral as recorded at H610, retained for provenance):_
🔴 ✍️ **Porting the Harivaṃśa fitted-index method (H488) to the MBH to resolve PWG/MW `MBH. book,verse` loci.**
Failed because: the Harivaṃśa port succeeded only because a clean, freely downloadable **vulgate** e-text existed (Kinjawadekar). For the full MBH **no free bulk Nilakantha-vulgate Sanskrit text exists** — every bulk-downloadable full MBH (GRETIL/Tokunaga, DCS, sanskritdocuments, bombay.indology.info) is the **BORI critical** recension, against which a per-parvan continuous index is the same §8 structural dead end (BORI systematically dropped vulgate passages, so the Calcutta-`N`↔BORI-count offset is not a per-parvan constant). The only Nilakantha-vulgate machine text (Manipal Sastri-Vavilla, 95,286 verses) is a D3-gated private SPA API — spot-check only, no bulk; the cited Calcutta 1834–39 is scans (OCR). So the MBH **locus census is deferred, not run against an untrusted index** (H610 exit b). Reachable instead: reading-evidence — **956 of 2,466 Böhtlingk correction notes (39 %)** have their quoted pratīka attested in the BORI text by locus-free character-fuzzy retrieval (validation case `MBH. 7,9226` *yenāvibruvatā praśnam* → BORI `07,170.032`, confirmed), ≈ the Harivaṃśa 37.7 %.
Evidence: [`csl-atlas/data/forensic/MBH_CITATION_RESOLUTION_CENSUS.md`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/forensic/MBH_CITATION_RESOLUTION_CENSUS.md) §2 (H610, 11-07-2026); scripts [`f8_mbh_census.py`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/scripts/forensic/f8_mbh_census.py) + [`f8_mbh_verify.py`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/scripts/forensic/f8_mbh_verify.py).
...
> **Source:** [H610](https://github.com/gasyoun/Uprava/blob/main/handoffs/H610-Opus_csl-atlas_mbh_citation_census_11.07.26.md) · [csl-atlas MBH census](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/forensic/MBH_CITATION_RESOLUTION_CENSUS.md) · 11-07-2026 · `claude-opus-4-8`
...
Don't retry unless: you adjudicate the citation against **the vulgate itself**, never against the critical edition (that part is now settled — see the executed result below). The shared-*error* hypothesis for this pool is a **measured null**, so the rare-`HARIV.` pool is exhausted — reopen only with a *different* citation form whose number is demonstrably wrong against the vulgate in both dictionaries.

**Executed (H488, 10-07-2026).** The Kinjawadekar (Chitrashala 1936) vulgate e-text was harvested at **15,364 verses = 93.8%** of 16,374 (the `bhavishyaparva/` plain series is far fuller than the `_mpr` series the 71.1% estimate sampled, so the "thin P3" worry dissolves). A per-adhyāya continuous index was fitted on 14,471 PWG anchors and **validated on 815 held-out MW anchors — 68.4% land within ±3 of their cited śloka vs a 2.1% shuffled-N null (≈33×), so the index is trustworthy.** On the 565 shared refs: **206 (37.7%) corroborate at the exact cited śloka vs a 0.5% null (≈75×)** — a shared, verse-level, *verifiably-correct* apparatus — but the shared-**erroneous**-citation count is **null**: "displaced" candidates fall *below* their shuffled-N null (79 vs 200) with no clustering. The airtight shared-mistake upgrade is not obtained; against the correct edition the shared citations verify as *correct*. Datasets: [`harivamsa_shared_citation_resolution.csv`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/forensic/harivamsa_shared_citation_resolution.csv), census [§6](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/forensic/HARIVAMSA_CITATION_RESOLUTION_CENSUS.md), [csl-atlas PR #239](https://github.com/sanskrit-lexicon/csl-atlas/pull/239).
...
> **Source:** [csl-atlas](https://github.com/sanskrit-lexicon/csl-atlas) A10 · executed [H488](https://github.com/gasyoun/Uprava/blob/main/handoffs/H488-Opus_csl-atlas_harivamsa_vulgate_citation_resolution_10.07.26.md) · [10-07-2026](https://github.com/gasyoun/SanskritLexicography/commits/master?since=2026-07-10&until=2026-07-11) · `claude-opus-4-8`
