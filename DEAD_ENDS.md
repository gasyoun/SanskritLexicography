...
### §8b. Fitting a continuous locus index for the **full Mahābhārata** against any free bulk e-text — ✅ REOPENED & DONE for book 7 (the "no free vulgate" premise was refuted, 12-07-2026)
🟢 ✅ **Porting the Harivaṃśa fitted-index method (H488) to the MBH to resolve PWG/MW `MBH. book,verse` loci — now runnable: a free bulk Nīlakaṇṭha vulgate WAS obtained.**
**Reversal (H761, 12-07-2026, Opus 4.8 `claude-opus-4-8`).** The blocking premise below is **false as of 11-07-2026**: a full Nīlakaṇṭha-vulgate MBH e-text (83,971 shlokas, 18 parvans, per-adhyāya P/U/A/S addressing) was scraped **free** from [sanatana.in](https://sanatana.in/mahabharata/) into CommentaryStrategies ([PR #83](https://github.com/gasyoun/CommentaryStrategies/pull/83)) — not the D3-gated Manipal API and not OCR of scans, so the "no free bulk Nilakantha-vulgate text exists" claim no longer holds. The f8 fitted-index fit + held-out gate now runs exactly as f7 did: for **Droṇaparva (book 7)** the **held-out MW gate PASSES** — 48.1 % (90/187) within ±3, δ peaked at 0, vs a 0.0 % shuffled-N null; exemplar `MBH. 7,9283` → vulgate 7.200.24 (*…droṇaputram athābravīt*) end-to-end; 847 corroborated (29.6 % vs 0.7 % null). The other 17 parvans generalize by re-running per book (each parva is its own held-out gate — a parva whose digitization diverges from PWG's Calcutta numbering **fails** its gate, a real per-parva negative). Evidence: [`csl-atlas/data/forensic/MBH_DRONA_FITTED_INDEX_CENSUS.md`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/forensic/MBH_DRONA_FITTED_INDEX_CENSUS.md), [`f8_mbh_drona_fitted_index.py`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/scripts/forensic/f8_mbh_drona_fitted_index.py) ([csl-atlas PR #260](https://github.com/sanskrit-lexicon/csl-atlas/pull/260)). The retrieval-lane result below stands as the complementary (BORI reading-evidence) verdict; only the *fitted-index deferral* is lifted.
