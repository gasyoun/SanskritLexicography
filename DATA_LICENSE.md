...
not the whole repository, and not the large primary-source scans (e.g. the
untracked PDF reproductions occasionally staged in this working tree), whose
rights are third-party and unresolved. A repo-wide Zenodo/GitHub archival
integration is deliberately **not** used here for that reason; FAIR Release #1
is a curated, manually-deposited dataset release of the two files named
above plus a cross-link to the citation-graph dataset owned by
