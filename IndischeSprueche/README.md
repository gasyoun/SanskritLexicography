...
Treat this file as a **bulk-query / derived-data source**, never as the citable
edition. For anything authoritative or citation-facing, route to
[`boesp2`](https://github.com/sanskrit-lexicon-scans/boesp2) /
[`boesp1`](https://github.com/sanskrit-lexicon-scans/boesp1) instead.

**Do NOT use it for:**

- **The authoritative text.** It is 76 verses short of `boesp2`'s verified
  7,613 and comes from an Excel transcription of unproofed rigor — use the
  scan-backed editions above when correctness matters.
- **`Spr. N` citation resolution.** Already shipped:
  [`csl-orig/v02/pwg/pwg.txt`](https://github.com/sanskrit-lexicon/csl-orig/blob/main/v02/pwg/pwg.txt)
  carries 10,366 `<ls>`-wrapped `Spr.` citations with live hrefs generated by
  `csl-websanlexicon`'s `basicadjust.php`
  ([PWG#87](https://github.com/sanskrit-lexicon/PWG/issues/87), closed). Do not
  rebuild the crosswalk from this file.
- **Russian or English text.** Only `translation_de` exists; there is no RU/EN
  field (`subhashita_ramayana`'s Russian pairing was deliberately excluded — see
  Provenance).

**Good fits (its offline/bulk value):**

1. **Sanskrit–German gnomic parallel corpus.** 7,537 `iast`/`deva` →
   `translation_de` pairs from a single expert translator = a clean gold Sa→De
   translation memory / DE-gloss validation set. The
   [RU translation kit](https://github.com/gasyoun/SanskritLexicography/tree/master/RussianTranslation)
   (`mw_ru`/`pwg_ru`) has no German sibling; this is the natural seed for one.
2. **Karaoke verse feed.** The one live consumer:
   [`SanskritKaraoke`](https://github.com/gasyoun/SanskritKaraoke) already imports
   curated anuṣṭubh verses via
   [`tools/import_subhashita.py`](https://github.com/gasyoun/SanskritKaraoke/blob/main/tools/import_subhashita.py)
   with German as the labelled translation. This file is the pool to expand from
   (⚠ the standing gap is **audio**, not text).
3. **Meter / scansion datasets.** `iast` + `deva` present, predominantly
   anuṣṭubh — a ready fixture for śloka/vipulā detection.
4. **Teaching example bank.** Short self-contained verses + translation +
   apparatus suit
   ...
   reader (subhāṣita-of-the-day content).
5. **Devanāgarī↔IAST parity fixtures.** Both fields hand-aligned across 7,537
   rows = free round-trip golden vectors for
