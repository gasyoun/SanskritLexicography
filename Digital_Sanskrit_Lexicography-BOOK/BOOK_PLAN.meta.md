...
|---|---|---|---|
| 1 | **Ch. 3 ← A40** — write the CDSL-headword-inventory + corpus-grounding chapter from data | one of two chapters not yet in book form; A40 is data-only (no article prose to convert) — genuine first-drafting atop [HeadwordLists/NOW_VS_THEN.md](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/NOW_VS_THEN.md) + union_headwords.tsv + dcs_cdsl_xref.tsv; **mandatory** McEnery & Brezina corpus-absence/representativeness reframe | ✅ **done 18-07-2026** ([H1240](https://github.com/gasyoun/Uprava/blob/main/handoffs/H1240-Fable_SanskritLexicography_m01-ch03-a40-ch11-a50-data-chapter-prose_18.07.26.md), Fable 5 `claude-fable-5`) — [ch03_headword_inventory.md](https://github.com/gasyoun/SanskritLexicography/blob/master/Digital_Sanskrit_Lexicography-BOOK/chapters/ch03_headword_inventory.md); A40 had reached full prose (H675), so this was a journal→book conversion; the corpus-absence reframe rides on ch02 §6 by citation |
| 2 | **Ch. 11 ← A50** — write the `<ls>` citation-frequency-graph chapter from data | A50 is a readiness-2/5 **skeleton** (prose 1/5); its §4 tradition-profiles rest on an **inferred map, 0/119 human-reviewed** that must NOT be asserted as fact yet; needs the review gate cleared or the section written as explicitly inferred; **mandatory** significance-at-N=828K (effect-sizes-not-p-values) reframe | ✅ **done 18-07-2026** ([H1240](https://github.com/gasyoun/Uprava/blob/main/handoffs/H1240-Fable_SanskritLexicography_m01-ch03-a40-ch11-a50-data-chapter-prose_18.07.26.md), Fable 5 `claude-fable-5`) — [ch11_citation_frequency_graph.md](https://github.com/gasyoun/SanskritLexicography/blob/master/Digital_Sanskrit_Lexicography-BOOK/chapters/ch11_citation_frequency_graph.md); A50 had reached full prose (H677), so conversion not first-drafting; §4 ships as **explicitly inferred** (0/119 reviewed stated in running text and table) — the review gate remains uncleared and stays a submission gate for the journal A50 |
...
|---|---|---|
| 18-07-2026 | **Backlog #1/#2 closed — 14 of 14 chapters in book form.** Ch. 3 (← A40) and Ch. 11 (← A50) landed as book-form conversions — both articles turned out to carry full prose (H675/H677), so the backlog's "data-only, first-drafting" premise was stale; the Part II/IV bridge ⚠️ obligations (the #5 residue) resolved against the merged chapters in the same pass; two attestation/absence semantics inversions caught and fixed (ch02 §6.2/§6.4 + BRILL_PROPOSAL described VEI 69.8 %…SKD 14.1 % as *absence* rates — they are **attestation** rates per A40 §4.4); ch11 reconciles the two committed `<ls>` extraction conventions (bare vs attribute-bearing tags, ch10 census vs graph builder) and states the tradition map as inferred (0/119 reviewed) | H1240, Fable 5 `claude-fable-5` |
