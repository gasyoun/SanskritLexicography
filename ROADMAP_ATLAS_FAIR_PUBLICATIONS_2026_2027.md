...
- [ ] **Community review pool v1**: convert the three blocked packets (R2 ×10, H4 ×105, xref ×50) into a student-facing review UI (atlas already renders packets; add submission + double-keying + κ report). Recruit 5–10 students (samskrtam.ru network, Sanskrit students' lists). *The review tasks are the pedagogy*: each packet teaches dictionary reading. No submission/double-keying UI found on csl-atlas origin/main as of 24-09-2026 (grep over html/js/py for double-keying / second-annotator / submission surfaces: 0 hits; packets render from committed JSON, e.g. [data/citations/h1684_tradition_adjudication_packet.json](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/citations/h1684_tradition_adjudication_packet.json)) — the submission layer remains **not found**.
