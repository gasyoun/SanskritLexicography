...
| G2 | **Standards exports: pilot delivered, scope partial** *(status re-probed 24-09-2026 — fixes the 02-09 drift)*: the TEI Lex-0 pilot **exists** — 258 `*.lex0.xml` files in [`csl-standards/data/pilot/tei-lex0/`](https://github.com/sanskrit-lexicon/csl-standards/tree/main/data/pilot/tei-lex0) plus curated TEI at [`data/pilot/curated/tei/`](https://github.com/sanskrit-lexicon/csl-standards/tree/main/data/pilot/curated/tei), ahead of its Q4 2026 slot — but coverage is MW/PWG/PWK only: no SKD/indigenous sample, no committed schema-validation artifact found on origin/main, and no ELEXIS linkage. Lexicography reviewers will still ask "where is the TEI?" — the answer now exists, incomplete. | High→Medium *(downgraded 24-09-2026: pilot exists; residual is indigenous coverage + validation proof)* | One pilot: MW + one indigenous dict (SKD) → TEI Lex-0, validated against the Lex-0 schema; OntoLex later. *(Remaining 24-09-2026: add the SKD sample + a committed validation run.)* |
...
3. **TEI Lex-0 pilot (G2)** in csl-standards: MW sample + SKD sample exported and schema-validated; documents what the indigenous apparatus needs that Lex-0 lacks (itself publishable as a note). *(Status 24-09-2026: pilot EXISTS, ahead of its Q4 slot — 258 [`data/pilot/tei-lex0/`](https://github.com/sanskrit-lexicon/csl-standards/tree/main/data/pilot/tei-lex0) Lex-0 exports + curated TEI, but MW/PWG/PWK only; SKD sample and a committed schema-validation run: not found — see G2.)*
...
4. **Book writing begins** against the contract; new chapters 1, 2, 7, 10 drafted (everything else is revision of P1–P6). *(Status 24-09-2026: not found — no new-chapter drafts (1, 2, 7, 10) exist; correctly gated on the contract; the proposal draft exists (see Q4).)*
