...
_Created: 12-06-2026 · Last updated: 25-09-2026_

> **Truth-pass 24-09-2026** (H5300, OxAlpha `opencode/z-ai/glm-5.3-flash`). **Verdict: live-with-residual.**
> Every Part IV line re-probed against `origin/main` of `csl-atlas`/`csl-standards`/`csl-observatory`/`VisualDCS`
> and [Uprava ARTICLES.md](https://github.com/gasyoun/Uprava/blob/main/ARTICLES.md) (fetched 24-09-2026).
> Corrections to the 02-09 note below: `.zenodo.json` exists only in csl-atlas + csl-observatory — none was ever
> committed in csl-standards/VisualDCS (git history: empty); ORCID iDs are sparse, not "most" (atlas 2, VisualDCS 1,
> observatory 1, standards 0 per CITATION.cff); the G3 PROV-O crosswalk **does exist in draft**
...
> labels → PROV-O + TEI `@cert`/`@resp`); per-dataset licence fields now present in 18/175 atlas data JSONs
> (census of the rest owed); the learner reading layer **v1 page exists** on csl-atlas origin/main (DCS frequency
> bands × lemma coverage; live site is up at [sanskrit-lexicon.github.io/csl-atlas](https://sanskrit-lexicon.github.io/csl-atlas/)
> but the learner page is not yet in the deployed build — 404); H5315 (maker-proposal draft) and H5338 (maintainer
> runbook) were minted 23-09-2026. Per-line statuses dated 24-09-2026 are inline below; nothing ticked without a
> linked artifact.
...
| G2 | **Standards exports: pilot delivered, scope partial** *(status re-probed 24-09-2026 — fixes the 02-09 drift)*: the TEI Lex-0 pilot **exists** — 298 `*.lex0.xml` files in [`csl-standards/data/pilot/tei-lex0/`](https://github.com/sanskrit-lexicon/csl-standards/tree/main/data/pilot/tei-lex0) (re-probed live 24-09-2026; the pass window first saw 258 — origin moved mid-pass), of which **47 are SKD samples** (`skd-*.lex0.xml` — the indigenous-sample claim below was itself probed and corrected this pass), plus curated TEI at [`data/pilot/curated/tei/`](https://github.com/sanskrit-lexicon/csl-standards/tree/main/data/pilot/curated/tei), ahead of its Q4 2026 slot — but coverage is MW/PWG/PWK + partial SKD only: no committed schema-validation artifact found on origin/main, and no ELEXIS linkage. Lexicography reviewers will still ask "where is the TEI?" — the answer now exists, incomplete. | High→Medium *(downgraded 24-09-2026: pilot + SKD samples exist; residual is validation proof + full indigenous coverage)* | One pilot: MW + one indigenous dict (SKD) → TEI Lex-0, validated against the Lex-0 schema; OntoLex later. *(Remaining 24-09-2026: committed validation run; widen indigenous coverage beyond the 47-file SKD pilot.)* |
| G3 | **In-house evidence vocabulary is unmapped**: `observed/derived/inferred/reviewed` is excellent but private. *(Status 24-09-2026: a DRAFT crosswalk now exists — [`csl-standards/docs/EVIDENCE_LABEL_CROSSWALK.md`](https://github.com/sanskrit-lexicon/csl-standards/blob/main/docs/EVIDENCE_LABEL_CROSSWALK.md) maps the labels onto W3C PROV-O + TEI `@cert`/`@evidence`/`@resp`; it names G3 explicitly, stays draft, ships no exporter.)* | Medium→Low *(downgraded 24-09-2026: crosswalk drafted; exporter open)* | Publish a one-page crosswalk to W3C PROV-O / TEI `@cert`/`@resp`; costs a day, makes the labels citable. *(Remaining 24-09-2026: finalize the draft + ship an exporter.)* |
| G4 | **Single-reviewer bottleneck = methodological weakness, not just a queue**: 10 R2 checkpoint rows, 105 H4 rows, 50 xref rows blocked on one person. With n=1 there is no inter-annotator agreement, so reviewed claims have unmeasurable reliability. | High | The community-review decision fixes this *if* every packet is double-keyed and κ is reported (OBS-T has the κ *tooling* staged — `validation/gold_sample.csv` + `obs_t_gold.py` — but no measured κ yet: the second-annotator column is empty; the earlier "κ=0.42" claim here was unsupported and is retracted, see [FINDINGS §87](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md)). *(Re-probed 24-09-2026: a cross-model κ IS now measured for the OBS-T gold sample — 0.906 [0.872–0.938], n=390, location axis, H1385 21-07-2026, per [ARTICLES.md A12](https://github.com/gasyoun/Uprava/blob/main/ARTICLES.md) — but the atlas R2/H4/xref packets themselves remain n=1.)* |
...
- **Definition typology**: synonym gloss vs equivalent vs encyclopedic description per dictionary — the single most classical microstructural dimension. **◐ first-pass 24-07-2026 (H1483):** operational rubric + 15-dict / 926k-record distribution + gold sample 63/79=79.7% — [DEFINITION_TYPOLOGY_WS2_4_2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/data/DEFINITION_TYPOLOGY_WS2_4_2026.md). Double-keyed 300×7 pool still open (item 6 below).
...
| P4 | *When zero means nothing: recovering the indigenous microstructure of Śabdakalpadruma and Vācaspatya* | M4/M7, anubandha key, zero-meaning methodology, H6 register | **IJL** or WSC 2027 or *Vimarśinī* (third option added 26-09-2026, [H5510](https://github.com/gasyoun/Uprava/blob/main/handoffs/H5510-OxAlpha_SanskritLexicography_sanskritlibrary-publications-census_26.09.26.md) — SL Journal, peer-reviewed, scope explicitly covers Vyākaraṇa + digital humanities/computational linguistics; free online by registration + Lulu print; ed.-in-chief Scharf; no public CFP deadline on its pages; [venue profile](https://github.com/gasyoun/SanskritLexicography/blob/master/SANSKRITLIBRARY_PUBLICATIONS_CENSUS_26-09-2026.md)) | Q1–Q2 2027 |
| P5 | *Fifty thousand corrections: an error typology of twelve years of collaborative dictionary maintenance* | OBS-T (52,498 corrections released snapshot, two-axis typology; **κ pending for A31's origin axis — A12's location-axis κ measured 21-07-2026, 0.906 cross-model, H1385; status note 24-09-2026**) + the [A31 origin-axis census](https://github.com/gasyoun/SanskritLexicography/blob/master/papers/A31_fifty_thousand_corrections_error_origin_typology.md) | **Lexikos** | Q2 2027 |
...
- [x] **FAIR sprint (G1, G3, G6)**: Zenodo DOI for atlas data release v1; CITATION.cff in csl-atlas, csl-standards, VisualDCS, csl-observatory; ORCID registered and added to papers; PROV-O crosswalk page for evidence labels; per-dataset licence fields. **Repo-side closed (A02, 24-09-2026), owner residual stands:** the `.zenodo.json` gap is now closed — added to csl-standards ([PR #143](https://github.com/sanskrit-lexicon/csl-standards/pull/143), merged) and VisualDCS ([PR #146](https://github.com/gasyoun/VisualDCS/pull/146), merged), mirroring the ones already in [atlas](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/.zenodo.json) and [observatory](https://github.com/sanskrit-lexicon/csl-observatory/blob/main/.zenodo.json); `CITATION.cff` now confirmed present in all four repos with matching `.zenodo.json`. **Still owed, all human-gated or a larger lift — repo-side action stops here:** connecting csl-atlas to Zenodo and minting the atlas-data DOI itself needs a human with Zenodo account access (atlas README's [Pending-author-actions note](https://github.com/sanskrit-lexicon/csl-atlas#readme) still stands; first org-side DOI remains A12's dataset [10.5281/zenodo.21346705](https://doi.org/10.5281/zenodo.21346705), minted 16-08-2026 — VisualDCS's own DCS-2026 DOI [10.5281/zenodo.22853985](https://doi.org/10.5281/zenodo.22853985) predates this sprint and is unrelated to the atlas release); ORCID stays sparse (atlas 2, VisualDCS 1, observatory 1, standards 0) — adding more requires each co-author's own ORCID iD, not something an agent can register on their behalf; the G3 PROV-O crosswalk exists in draft (see G3); per-dataset licence fields remain at 18/175 atlas data JSONs (e.g. [etymology-oracle.json](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/etymology-oracle.json)) — the full licence census is a separate, larger unit, not attempted here.
...
- [ ] **Community review pool v1**: convert the three blocked packets (R2 ×10, H4 ×105, xref ×50) into a student-facing review UI (atlas already renders packets; add submission + double-keying + κ report). Recruit 5–10 students (samskrtam.ru network, Sanskrit students' lists). *The review tasks are the pedagogy*: each packet teaches dictionary reading. *(Status 25-09-2026: the "0 hits" reading above is now STALE — same-day work (23–24-09-2026) landed a full design of record, [docs/REVIEW_POOL_V1_DESIGN.md](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/docs/REVIEW_POOL_V1_DESIGN.md) (H5307, epic E014), decomposing the item into build items B1–B9 with an explicit rollout (P1–P6); [docs/REVIEW_POOL_STUDENT_GUIDE.ru.md](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/docs/REVIEW_POOL_STUDENT_GUIDE.ru.md) (H5310) and a κ-tool PR (H5309) and a recruitment-post draft (H5311) followed. This pass (A02) landed **B1** — [PR #516](https://github.com/sanskrit-lexicon/csl-atlas/pull/516), merged: `scripts/validate_review_decisions.py`'s H4 expected-row set was silently empty (H1621 flipped all rows off `needs-review`), so every H4 export — including the pool's double-keyed exports — failed validation; fixed with a fallback mirroring the sheet builder's own, 2 new tests, 9/9 green. A second concurrent A02 pass independently reached the same B1 fix and stood down once [H5308](https://github.com/gasyoun/Uprava/blob/main/handoffs/H5308-OxAlpha_csl-atlas_review-pool-packets-to-double-keyed-sheets_23.09.26.md) ([PR #519](https://github.com/sanskrit-lexicon/csl-atlas/pull/519), merged) landed **B2–B6 and B9** minutes later in the same wave: pool mode (`--pool`/`--annotator` slicing), blind rendering, per-annotator sheet emission, the R2 sheet, the rotating-offset assignment builder (`data/review/pool_assignment.json`), and property tests, exercised by a two-annotator dry run over all three packets (R2 10/10, H4 89/89, xref 40/40 — synthetic canary data, not real student keys). **B7/B8 landed 25-09-2026 (this pass, A02):** [PR #525](https://github.com/sanskrit-lexicon/csl-atlas/pull/525) (auto-merge armed, CI running at time of writing) implements `scripts/merge-review-pool-decisions.py` (B7 — the five-case truth table over two independent keys, resolved against the packet's own machine-proposed label, note-scrub refusal for contact-bearing tokens, adjudication requires an explicit `adjudicatedBy` + rationale so a tie never defaults to the machine label) and `scripts/lexico/review_pool_kappa.py` (B8 — Krippendorff's α headline, Cohen's κ per dyad at n≥30, seeded bootstrap CI, Landis–Koch bands, `data/review/pool_kappa_report.json` + generated `docs/REVIEW_POOL_KAPPA_REPORT.md`), both exercised end to end against the real H5308 R2 dry-run canary (10/10 rows → `reviewed-ok`; α correctly `null` — the all-approve synthetic canary is a single-category degenerate case, labelled machinery verification not a measurement) with 29 new tests wired into `npm run verify`. This closes H5483 and every build item B1–B9 of the design. **Owner residual, not agent-doable:** recruiting 5–10 students is outreach/social, not an engineering task — H5311's draft post is ready but sending it and enrolling students needs a human; without real annotators no real κ number exists. Checkbox stays unticked: the repo-side machinery is now complete, but the roadmap item as written ("recruit 5–10 students… κ report") cannot close until that human step runs.)*
- [ ] **Submit P1 to IJL** (gold-sample results folded in). Re-probed 24-09-2026 ([ARTICLES.md A16](https://github.com/gasyoun/Uprava/blob/main/ARTICLES.md)): still **5/5, still NOT submitted** — ready-to-send pending MG's read-and-sign (citation dossier H1077 17-07-2026 + author-pass [PR #300](https://github.com/sanskrit-lexicon/MWS/pull/300), 06-09-2026, SIGNOFF_A16_author_pass.md); queued as **SUBMIT-15**.
- [ ] Clear H5 maker proposal (divaraTa → diviraTa) to Cologne makers. Not sent as of 24-09-2026 — no record of transmission to Cologne found; a draft handoff [H5315](https://github.com/gasyoun/Uprava/blob/main/handoffs/H5315-OxAlpha_csl-atlas_h5-divarata-divirata-maker-proposal-draft_23.09.26.md) (maker-proposal draft) was minted 23-09-2026 and is open — the maker-facing proposal itself remains **not found**.

### Q4 2026 (Oct–Dec) — *Sense inheritance + learner's layer v1*
1. **P2 written and submitted** (R2 data is restored and reproducible; checkpoint rows now adjudicated by the pool). *(Status 24-09-2026: written, NOT submitted — [ARTICLES.md A02](https://github.com/gasyoun/Uprava/blob/main/ARTICLES.md) 4/5, Lexicographica, SUBMIT-15; author-pass executed 06-09-2026 ([PR #443](https://github.com/sanskrit-lexicon/csl-atlas/pull/443)); awaiting MG read-and-sign + SKD adjudication. "Checkpoint rows now adjudicated by the pool": not found — no pool exists.)*
2. **Learner's reading layer v1** in csl-atlas: DCS frequency bands (VisualDCS adapter contract: `dcs_lemma_summary.json`) joined to lemma-lookup + dossier; per-lemma card = frequency band, best-attested senses (survival-ranked from P2 data), Whitney root + gaṇa, paradigm-browser link. Ship as a public page. *(Status 24-09-2026: v1 EXISTS on csl-atlas origin/main, ahead of its Q4 slot — page [`src/tools/learner-reading-layer.md`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/src/tools/learner-reading-layer.md) fed by [`scripts/build-learner-index.mjs`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/scripts/build-learner-index.mjs) (lemma coverage × DCS frequency bands, band 5 = learn first → [`src/data/learner/learner-index.json`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/src/data/learner/learner-index.json), en/ru locales); the atlas site is live at [sanskrit-lexicon.github.io/csl-atlas](https://sanskrit-lexicon.github.io/csl-atlas/) but the learner page is not yet in the deployed build (404 at `/tools/learner-reading-layer/`). Survival-ranked sense join from P2 data: not found in v1.)* *Audience personas (adopted 02-07-2026 from the DharmaMitra workshop intake form — [analysis](https://github.com/gasyoun/VisualDCS/blob/main/non-derived/Pre-Workshop_Questionnaire_ANALYSIS.md)): professional/published translator · academic/researcher · student · practitioner translating for practice · hobbyist/independent learner — the practitioner and hobbyist segments are first-class users of this layer, not an afterthought; actual segment sizes pending DharmaMitra's aggregated form results (MG asks at the workshop, GTD @DO).*
3. **TEI Lex-0 pilot (G2)** in csl-standards: MW sample + SKD sample exported and schema-validated; documents what the indigenous apparatus needs that Lex-0 lacks (itself publishable as a note). *(Status 24-09-2026: pilot EXISTS, ahead of its Q4 slot — 298 [`data/pilot/tei-lex0/`](https://github.com/sanskrit-lexicon/csl-standards/tree/main/data/pilot/tei-lex0) Lex-0 exports + curated TEI, incl. **47 SKD samples** (`skd-*.lex0.xml`, live tree probe 24-09-2026); a committed schema-validation run: not found — see G2.)*
4. **Book proposal drafted** (synopsis + P1/P2 as sample chapters). *(Status 24-09-2026: DRAFTED — ten-chapter synopsis [`PROPOSAL_SYNOPSIS_TEN_CHAPTER_PLAN_LSM_BRILL_23-09-2026.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/Digital_Sanskrit_Lexicography-BOOK/PROPOSAL_SYNOPSIS_TEN_CHAPTER_PLAN_LSM_BRILL_23-09-2026.md) (H5323, merged 23-09-2026) with real readiness per chapter, crosswalked to the 14-chapter M01 [BRILL_PROPOSAL](https://github.com/gasyoun/SanskritLexicography/blob/master/Digital_Sanskrit_Lexicography-BOOK/BRILL_PROPOSAL.md); which ToC is sent stays `@DECIDE`.)*

### Q1 2027 (Jan–Mar) — *The indigenous turn*
1. **Megastructure study**: front/back matter of all 9 narrative dicts catalogued as data (extends the anubandha-key method); feeds P3 and P4. *(Status 24-09-2026: not found — no dedicated megastructure study or artifact on csl-atlas origin/main; nearest existing artifact is A30's front-matter anubandha key ([A30 paper](https://github.com/gasyoun/SanskritLexicography/blob/master/papers/A30_skd_vcp_microstructure_note.md)).)*
2. **P3 submitted** (citation registers). *(Status 24-09-2026: NOT submitted — [ARTICLES.md A08](https://github.com/gasyoun/Uprava/blob/main/ARTICLES.md) 4/5, SUBMIT-15, awaiting MG read-and-sign + the §4 figure revision owed after the H4075 stale-figures re-diff; the Dictionaries-side variant A18 is 3/5 GUIDE with the band-3 hand-verify open.)*
3. **P4 drafted** (indigenous microstructure); decide IJL vs WSC 2027 vs *Vimarśinī* by deadline calendar. *(Status 24-09-2026: drafted ×2, NOT submitted — [A04](https://github.com/gasyoun/Uprava/blob/main/ARTICLES.md) 4/5 (verbal-root kośa) + [A30](https://github.com/gasyoun/Uprava/blob/main/ARTICLES.md) 3/5 (ŚKD/VCP), both author-passed 06-09-2026, awaiting MG read-and-sign; IJL-vs-WSC venue `@DECIDE` open — *Vimarśinī* added as third option 26-09-2026 ([H5510](https://github.com/gasyoun/Uprava/blob/main/handoffs/H5510-OxAlpha_SanskritLexicography_sanskritlibrary-publications-census_26.09.26.md)), the choice itself stays human.)*
4. **Kośa chapters**: ARMH/ABCH stubs → real chapters; first macrostructural model of versified synonymic dictionaries in the atlas (Ch.7 material; nothing comparable exists in English). *(Status 24-09-2026: beyond stubs — dict pages [armh.md](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/src/dicts/armh.md) / [abch.md](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/src/dicts/abch.md) plus corpus-scale kośa fusion data ([r2_kosa_fusion.json](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/lexico/r2_kosa_fusion.json)) shipped; the first macrostructural model of versified synonymic dictionaries: not found.)*
5. **Book proposal submitted** to de Gruyter/Brill. *(Status 24-09-2026: not found — no submission record; proposal drafted 23-09-2026 (see Q4), ToC choice `@DECIDE`.)*
6. Review pool round 2: definition-typology annotation (the missing classical dimension) — 300 entries × 7 dicts, double-keyed. *(Status 24-09-2026: not found — pool v1's submission UI itself does not exist yet, see Q3 2026.)*

### Q2 2027 (Apr–Jun) — *Students, evaluation, book contract*
1. **Learner's layer v2 + user study (G7)**: task-based evaluation with the review-pool students (find-the-right-sense tasks, timed, vs plain Cologne lookup); results feed P6. Protocol written 24-09-2026, before any data: [USER_STUDY_PROTOCOL_LEARNER_LAYER_G7_24-09-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/docs/USER_STUDY_PROTOCOL_LEARNER_LAYER_G7_24-09-2026.md) (H5336) — within-subject, 2-group Latin square, n = 24 target, 360 eligible items measured on live data, 152-ФЗ regime; open asks are the Russian consent text and the MW sense index. *(Status 24-09-2026: user study not found — the protocol above is preparation, the study itself has no artifact; layer v1 exists on origin/main (see Q4) — v2 n/a.)*
2. **P5 submitted** (error typology — the OBS-T dataset is unique in the field; no dictionary project has 12 years of correction telemetry). *(Status 24-09-2026: NOT submitted — [ARTICLES.md A12](https://github.com/gasyoun/Uprava/blob/main/ARTICLES.md) 4/5, LREC-COLING, SUBMIT-15: location-axis κ measured 0.906 cross-model (H1385, 21-07-2026), dataset DOI [10.5281/zenodo.21346705](https://doi.org/10.5281/zenodo.21346705) minted 16-08-2026, awaiting MG read-and-sign + the §4.3 gold-provenance wording `@DECIDE`; [A31](https://github.com/gasyoun/Uprava/blob/main/ARTICLES.md) 3/5, Lexikos, awaiting its origin-axis κ (agent-clearable via the H1385 protocol).)*
3. **P6 submitted** (learner's layer + study). *(Status 24-09-2026: not found — [ARTICLES.md A32](https://github.com/gasyoun/Uprava/blob/main/ARTICLES.md) is 1/5, idea only, no artifact of its own. **Update 25-09-2026 (H5337):** A32 now has its own artifact — the paper skeleton [papers/A32_frequency_graded_reading_layer_learners.md](https://github.com/gasyoun/SanskritLexicography/blob/master/papers/A32_frequency_graded_reading_layer_learners.md), readiness 1→2/5: claim, 13-row data inventory, the join as method (from the csl-atlas [learner layer v1 spec](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/docs/LEARNER_LAYER_V1_SPEC.md), merged), related-work map seeded from repo holdings, ablation table, and a user-study section that is **empty by design** until the G7 protocol runs. Not submitted; the v1 dataset build ([csl-atlas PR #521](https://github.com/sanskrit-lexicon/csl-atlas/pull/521)) was still open on 25-09-2026.)*
4. **Book writing begins** against the contract; new chapters 1, 2, 7, 10 drafted (everything else is revision of P1–P6). *(Status 24-09-2026: not found — no new-chapter drafts (1, 2, 7, 10) exist; correctly gated on the contract; the proposal draft exists: [`PROPOSAL_SYNOPSIS_TEN_CHAPTER_PLAN_LSM_BRILL_23-09-2026.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/Digital_Sanskrit_Lexicography-BOOK/PROPOSAL_SYNOPSIS_TEN_CHAPTER_PLAN_LSM_BRILL_23-09-2026.md), see Q4.)*
5. Co-maintainer named for atlas data builds (G8). *(Status 24-09-2026: no co-maintainer named; a maintainer-runbook handoff [H5338](https://github.com/gasyoun/Uprava/blob/main/handoffs/H5338-OxAlpha_csl-atlas_atlas-data-build-maintainer-runbook_23.09.26.md) was minted 23-09-2026 and is open.)*
