...
| L5 | `ls_resolver.py` + `nws_ls_markup.py` (H1809) | `<ls>` citation → scanned-edition page-URL resolver, plus a companion normalizer that maps the NWS-layer’s own siglum convention (Roman-numeral maṇḍala, optional `(Sā)` recension marker) into PWG’s Arabic/comma form before resolving | `resolve("Spr. 1") → the boesp scan-page URL`; `nws_ls_markup.py census` → candidate spans + resolvable/residue split | 07/26 | Re-implementing citation→scan mapping, or re-discovering the NWS convention mismatch · [RussianTranslation/src](https://github.com/gasyoun/SanskritLexicography/tree/master/RussianTranslation/src) |
