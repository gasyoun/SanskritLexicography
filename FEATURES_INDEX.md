...
_Created: 04-07-2026 · Last updated: 03-09-2026 (H3948: L17 PWG four-tier enumeration reader + store-impact meter — turned «should we re-segment the store?» into a measured 12–3,212-row range nobody has to guess; prior: H3959: L16 PWG `<ab>` abbreviation bucketer — the three disjoint sets + the census gate that makes "none remain German" mechanical; prior: H3878: E25 refreshed — the aorist/perfect conflation it warned about was resolved inside the indicative by H1486, and the residual 8,726 non-indicative past tokens are unformationed upstream, not query-hidden; prior: H3790: E53 curated per-work dating table for PWG's 45 cited works — sourced ranges, two dating-invalid sigla, eleven decisions on the vote hub; prior: H3750: L15 pwg_ru provenance census — measured/asserted/absent classifier + the git-reconstruction probe that refuses the backfill; H3344: crosslink audit — P4 `public/exercises` repointed to its renamed `public/lila`; H2943: P12 samskrtam.ru crossword + bandha explorers; H2874: F47 citation-count provenance resolved + the two `<ls>` count tables added as sidecars; H2408: F48 defgen eval benchmark + D20 corrected French/second-reference role; H2361: L12 citation_tm.py parallel-verse lookup + its measured EN-lane misalignment; H2110: H8 Systema in-cabinet reading packs; H1909: L5 nws_ls_markup.py bare-citation discriminator)_
...
**At a glance:** 44 dictionaries · 23 interfaces (17 live) · 47 data assets · 17 tools · 4 external stacks · 12 learner-facing drill/practice sets (P1–P12) · 30 catalogued methods/algorithms (Q1–Q30).
...
| ⚪ A7 | `cyrillic_proper_noun_slp1.tsv` | Validated Cyrillic→SLP1 **proper-noun lookup** — every key derived from an IAST witness printed beside the Cyrillic name in a Russian index; zero rule-derived keys (tiers: onomasticon 446 / lexicon 50 / IAST-witness-only 38) | 534 rows · 90 KB | `Кадамбари	kAdambarI	Kādambarī	onomasticon` | 09/26 | [h3985_cyr_slp1_table.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/tools/h3985_cyr_slp1_table.py) |
...
| 🟡 D20 | `heritage_dico_gloss.tsv` | Heritage DICO lemma glosses — SLP1-keyed, gloss prose in **French** (Huet), not English. Doubles as the **independent second reference** for the defgen eval (F48) | 24.5k · 4.3 MB | `akAra   DICO/1.html#akaara   [ ( a ) - kāra ] m. le son ou la lettre 'a'.   kaara` | 07/26 | [HeadwordLists](https://github.com/gasyoun/SanskritLexicography/tree/master/HeadwordLists) |
...
| ID | Asset | What it is | Size | Example (real sample) | Intro | Home |
|---|---|---|---|---|---|---|
| 🟡 E25 | VisualDCS DCS ingest + lemma summary | Canonical CoNLL-U → SQLite build + `dcs_lemma_summary.json`. `Tense=Past` is split into Aorist/Perfect per token via DCS `Formation` (H1486); the 8,726 non-indicative past tokens carry no `Formation` upstream and are quotable by mood only (H3878) | ~15.9k lemmas · 3.4 MB | `{ "A": { "freqBand": 5, "attested": true } }` | 06/26 | [import_dcs_conllu.py](https://github.com/gasyoun/VisualDCS/blob/main/src/DCS-data-2026/import_dcs_conllu.py) |
...
| ⚪ E49 | definition typology (synonym / equivalent / encyclopedic) | Classical microstructural axis over **all 44** csl-orig dicts (1,496,157 `<L>` records): operational rubric + per-dict distribution + stratified gold sample; closes ROADMAP_STATISTICS L1 / WS2.4 and ATLAS_FAIR Part I "○ not started" as ◐ first-pass (H1483; double-key 300×7 pool still open) | 44 dicts · ~1.5M records | `mw   286525   2.1% syn / 89.6% eqv / 0.6% enc` (after linear citation strip) | 07/26 | [report](https://github.com/gasyoun/SanskritLexicography/blob/master/data/DEFINITION_TYPOLOGY_WS2_4_2026.md) · [TSV](https://github.com/gasyoun/SanskritLexicography/blob/master/data/definition_typology_per_dict.tsv) · [script](https://github.com/gasyoun/SanskritLexicography/blob/master/data/definition_typology_classifier.py) |
| 🟡 E50 | Rāmāyaṇa edition-alignment family (Gorresio · Bombay · Southern↔critical) | Everything PWG's three-edition `R.` citation scheme needs, in one place (H1656 + H1689 + H1705). **Gorresio**: structural inventory (672 sargas) + a full IAST e-text (19,852 vv — Google text layer for vols 1/3/5, tesseract-5 `san` OCR for the image-only 2/4/uk) + a CONTENT-BASED Gorresio↔Southern verse map that keys live citation reuse. **Bombay 1859**: structural inventory for all 7 kāṇḍas from the scan-viewer per-page index, **no OCR** — and the measured verdict that its uttarakāṇḍa (111 sargas + 13 interpolated) is NOT ≈1:1 with the 100-sarga corpus text, so no book-7 map is owed. **Southern↔critical**: verse-level map for kāṇḍas 1/2/3/5/6/7 — ⚠️ kāṇḍas 6–7 align to themselves ([#822](https://github.com/gasyoun/SanskritLexicography/issues/822)) | 4 TSVs + 1 JSONL · 658 + 672 sargas · 19,852 vv | `7   111   11   III   810   812   153b   154b   index_typo_111` (Bombay inventory row) | 07/26 | [`ramayana_bombay_inventory.tsv`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/ramayana_bombay_inventory.tsv) · [`ramayana_gorresio_inventory.tsv`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/ramayana_gorresio_inventory.tsv) · [`gorresio_etext.jsonl`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/gorresio_etext.jsonl) · [builder](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/build_ramayana_concordance.py) |
...
| ⚪ E51 | nominal case×number grid per declension class | The **nominal** counterpart of E46: the 8-case × 3-number grid per declension class over 2,263,192 cased NOUN+ADJ tokens of the DCS master, with attested surface endings, top attested forms and a real corpus example in every cell (14 classes × token gender × 24 cells). Reuses the Sangram G2 class taxonomy (E-sibling below) rather than re-deriving it and reconciles against it exactly — 57,144 lemma_ids, 0 disagreements. ⚠️ `feat_case='Cpd'` (724,676 tokens, no case) is excluded from the grid and counted separately; declension class is a citation-form heuristic, never a corpus tag (H1472) | 14 classes · 620 KB | `-an · Masc · Nom.Sing → 17,881 tokens, ending -ā (rājā 5,346)` | 07/26 | [VisualDCS visual/paradigm_nominal.json](https://github.com/gasyoun/VisualDCS/blob/main/visual/paradigm_nominal.json) + [dashboard](https://github.com/gasyoun/VisualDCS/blob/main/sanskrit_nominal_dashboard.html) + [build report](https://github.com/gasyoun/VisualDCS/blob/main/reports/paradigm_nominal_build.md) |
| ⚪ E52 | attested declension cells per lemma (Sangram G2) | Per **lemma** (not per class): which of the 24 case×number cells each of 57,144 noun lemmas attests in the pinned DCS master, as a `cells_bits24` bitstring + gender/stem/frequency breakdowns. Median 1 cell/lemma; only 10.44% of the lemma×cell space ever attested — the corpus-scale measurement of paradigm overgeneration. Built H1048, catalogued here 27-07-2026 during the H1472 propagation pass (it had no row). Its class taxonomy is the shared one E50 reuses | 57,144 lemmas · 3.1 MB | `70467,rājan,Masc,an,15212,20,111111101101101111101111` | 07/26 | [SanskritGrammar sangram/data/declension_cell_coverage/](https://github.com/gasyoun/SanskritGrammar/tree/main/sangram/data/declension_cell_coverage) |
| ⚪ E53 | curated dating table for PWG's cited works (C2 phase 2) | A **range** plus the scholarly source per date for all 45 works in [`ls_source_map.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/ls_source_map.json) — the provenance phase 1's bare point dates never had. Graded 9 anchored · 20 consensus · 14 contested · **2 dating-invalid**: `Spr.` is Böhtlingk's own 1863–1873 anthology and `ŚKDR.` a Calcutta compilation of 1821–1858, so neither dates anything, yet between them they set a bound on 11,771 of 43,990 phase-1 windows. Seven further map dates fall outside their sourced range (`AMAR`, `GĪT. GOV`, `KATHĀS`, `RĀJAN`, `SĀH. D`, `VOP`, `Spr`), recorded as corrections without touching the committed map. Eight claims are quoted from Vogel 1979 with its printed page, that volume being held in this repo; the rest cite the standard handbook and are marked `reference-only`. Gated by `c2p2_dating_table.py --check` — a date with no citation is a hard failure. The eleven datings no evidence can settle are one published sheet, [pwg_c2p2_work_dating_11](https://gasyoun.github.io/vote/sheets/pwg_c2p2_work_dating_11.html) (H3790) | 45 works · 33 sources · 11 decisions · 41 KB | `MED · Medinīkośa · 1200–1275 CE · anchored · Vogel 1979: 347 («somewhere between 1200 and 1275»)` | 09/26 | [`work_dating_table.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/work_dating_table.json) · [memo](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/research/C2P2_WORK_DATING_TABLE.md) · [gate](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/research/c2p2_dating_table.py) |
...
| ⚪ F47 | PWG literary-source scan-index campaign registry | The 2025–26 volunteer campaign that page-indexed the printed editions PWG cites, as committed data: per-source index status, volunteer, dates, page count, coordinating GitHub issue, and scan directory — cross-validated against `pwgbib` and the full-dictionary `<ls>` extraction. Sidecars: the verbatim 4-tab sheet snapshot, a ranked e-text candidate queue, a dated audit of all 37 `sanskrit-lexicon-scans` directories, and (H2874) the two `<ls>`-per-abbreviation count tables the campaign's numbers rest on. ✅ `citation_count` **provenance resolved 16-08-2026** — the 2024-09-11 `lsextract_all.txt` work-family rollup (`ALL` 739,056), 66/67 rows exact; the old 1.2×–433× "disagreement" was a comparison against cleaned-*string* counts, a different object. ⚠️ consume `citation_count_safe`, fold by `in_pwgbib` before summing, divide only by the same snapshot's `ALL` | 82 works · 43 cols · 38 KB (+11 KB queue, +10 KB audit, +2 × ~90 KB count tables) | `ls_code=RAGH., ls_code_raw=RAGH., ls_code_base=RAGH., book_no=1a, title=RAGHUVAM̃SA 1832 Stenzler ed., status=done, status_gloss=index built, reviewed, and posted, citation_count=8752, total_pages=385, volunteer=@grigoriyt1, started=2025-01-29, finished=2025-01-30, issue_repo=PWG, issue_no=100, issue_url=https://github.com/sanskrit-lexicon/PWG/issues/100, index_posted=2025-02-21, scan_dir=raghuvamsa, scan_dir_canonical=raghuvamsa, scan_spelling_ok=yes, scan_url=https://github.com/sanskrit-lexicon-scans/raghuvamsa, ` | 07/26 | [csl-observatory/data/pwg_scan_index_tracker](https://github.com/sanskrit-lexicon/csl-observatory/tree/main/data/pwg_scan_index_tracker) |
| 🟢 F48 | Sanskrit **definition-generation benchmark** (defgen) + gloss-grounded WSD pilot | The org's eval set for "write this headword's gloss": a frozen 500-headword MW sample (seed 730, stratified 3×3 frequency × polysemy, ≥3 DCS attestations each) + 5 frozen system arms (2 DeepSeek variants, a reasoner, an in-session Fable arm, and a seeded-derangement floor) + deterministic chrF/token-F1 + a blinded LLM judge with **gates** (floor separation, judge~chrF ρ, human subsample still owed). Extended H2408 with an **independent second reference** — Huet's French glosses (D20) on the 333-headword overlap: the arm ranking is **reference-invariant**, and the MW-familiarity premium is real but small (+0.13…+0.25 on 0–5, CI excludes 0 for every system arm, largest for the no-context memorisation arm). ⚠️ MW 1899 is in every model's pretraining data — these numbers measure *reproduction + grounding*, never generation from corpus evidence alone; and cross-lingual surface metrics are near-degenerate, so the judged score is the measurement | 500 items × 5 arms + 333-item second-reference subset · ~1.4 MB | `AyA  āyā  2.Ā.  mono  high  rank 641  →  gen "to come near, approach, arrive at…"  chrF-MW 23.16 / chrF-FR 12.09 / judge-FR 4.538` | 08/26 | [MW protocol](https://github.com/gasyoun/kosha/blob/main/docs/DEFGEN_MW_GLOSS_EVAL_PROTOCOL.md) + [Heritage second reference](https://github.com/gasyoun/kosha/blob/main/docs/DEFGEN_HERITAGE_SECOND_REFERENCE_EVAL.md) · [data](https://github.com/gasyoun/kosha/tree/main/data/eval/defgen) |
| 🟡 F49 | ЛЭС-1990 static dump (`les-1990`) | «Лингвистический энциклопедический словарь» (Ярцева, 1990, общелингвистический) as a harvested static HTML mirror — byte-identical to [tapemark.narod.ru/les/](https://tapemark.narod.ru/les/index.html), mirrored 23-08-2026 off samskrtam.ru by walking the `index.html` 1408-link manifest (0 failures). Two-commit history: raw dump as diff baseline, then the inlined Yandex.Metrika counter stripped from all 1,409 HTML files. Serves as the source of truth for the samskrtam.ru binding programme (build script + JSON-LD + search Phases 2–4) and the future target of BookIndex's 36 broken `les-1990/?s=` glossary links. Rights: MG accepted legal risk 10-07-2026 — do not re-raise | 1,408 articles + index.html + a.css · 26.5 MB | `009a.html` … `about.html` / `authors.html` / `recenzija.html` (article pages behind the ToC) | 08/26 | [gasyoun/les-1990](https://github.com/gasyoun/les-1990) |
...
| G3 | [kosha — translator-first dictionary](https://github.com/gasyoun/kosha) | Multi-dict collapse (MW+PWG+AP90), scan-anchored crawlable `/w/{slp1}` (SSR + D4 static head N=11,148 / 10,370 pages, H1590); strict C-SALT `/dicts/*` face (six fields + `csl`, H2768) while `/api/v1` retains `kosha`; SPA twin; exit packet for live deploy checks | 🟡 Beta | collapse MW + PWG + AP90 for one headword; inspect the strict Salt face | 08/16 | FastAPI · SQLite · static HTML · [kosha](https://github.com/gasyoun/kosha) · [Salt-face decision](https://github.com/gasyoun/kosha/blob/main/docs/DECISION_H2768_SALT_FACE_EXTENSION_CONTRACT.md) · [exit packet](https://github.com/gasyoun/kosha/blob/main/docs/P5_WORD_PAGE_EXIT_PACKET.md) |
...
| I14 | [VisualDCS — nominal paradigm](https://github.com/gasyoun/VisualDCS/blob/main/sanskrit_nominal_dashboard.html) | Case × number grid per declension class for nouns and adjectives — the nominal twin of the verb paradigm browser (I10) | 🟡 Local | 8 cases × 3 numbers × 14 stem classes over 2,263,192 cased tokens; click a cell for endings, attested forms and corpus examples | 07/26 | Standalone HTML · [VisualDCS](https://github.com/gasyoun/VisualDCS) |
...
| L10 | KeySwap (in `sanskrit-util`) | Scholar IAST typing toolkit (AHK/PWA/Apple) + Cologne check; **v3 opt-in plugins** `offline_fuzzy` (V3-2) + `network_autocomplete` (V3-7 offline-first) | `python tools/KeySwap/typing_check.py --plugin network_autocomplete --hud "rAm"` | 07/26 | Rebuilding a second typing tray / always-on net autocomplete · [KeySwap](https://github.com/sanskrit-lexicon/sanskrit-util/tree/main/tools/KeySwap) · [v0.8.9](https://github.com/sanskrit-lexicon/sanskrit-util/releases/tag/v0.8.9) |
...
| L5 | `ls_resolver.py` + `nws_ls_markup.py` (H1809, H1909) | `<ls>` citation → scanned-edition page-URL resolver, plus a companion normalizer that maps the NWS-layer’s own siglum convention (Roman-numeral maṇḍala, optional `(Sā)` recension marker) into PWG’s Arabic/comma form before resolving, plus (H1909) a discriminator that tells genuine bare citations apart from author/year provenance-note fragments (929-span sample, 195 marked, 0 measured false positives) | `resolve("Spr. 1") → the boesp scan-page URL`; `nws_ls_markup.py census` → candidate spans + resolvable/residue split; `classify_general_bare_citation()` → citation vs. provenance-note verdict | 08/26 | Re-implementing citation→scan mapping, re-discovering the NWS convention mismatch, or re-deriving the bare-citation vs. provenance-note split · [RussianTranslation/src](https://github.com/gasyoun/SanskritLexicography/tree/master/RussianTranslation/src) |
...
| L11 | pwg_ru headless control plane (H1618) | Offline multi-profile **cohort engine** + **max-agents starvation refuse** + **C-49 residual ledger** for PWG→RU | `python src/pilot/cohort_engine_selftest.py` (7/7 offline); omit `--max-agents` on multi-key windows | 07/26 | Re-deriving multi-account barrier / re-burning known residual keys · [cohort_engine.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/cohort_engine.py) · [no_pwg_residual_ledger.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/no_pwg_residual_ledger.py) · [headless_worker.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/headless_worker.py) |
| L12 | pwg_ru TM integrity kit (H3500) | Render-time-aware defect scanner (`h3500_defect_scan.py --check` gate), keep-best store repairer (`h3500_store_repair.py`, dry-run default + ledger), canonical entry assembler (`pwg_ru_entry_join.assemble_entry` — collapses PWG-homograph duplicate blocks at join, never merges distinct senses), plus the `merge_store_rows` incoming-dup guard | `python src/h3500_defect_scan.py src/pwg_ru_translated.jsonl --check`; `assemble_entry(ru_parts)` instead of any bare per-key1 join — B090 proof: 207→103 chars on `vasin` | 08/26 | Re-deriving benchmark-vs-renderer false positives, re-introducing byte-identical dup rows via promote tails, or bare-joining homograph sections · [RussianTranslation/src](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src) · [H3500 report](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/reports/H3500_PWGRU_TM_DEFECT_FIX_25-08-2026.md) |
| L13 | Gate-evidence contract (H3748, W1) | Every gate builds its verdict THROUGH a `GateEvidence` record — hashed inputs, predicate evaluation + hit counts, JSON sidecar; a **zero-work PASS is a hard FAIL**, and legitimate emptiness is declared by pre-registered name, never silent | `python src/pilot/gate_evidence.py --selftest` · `python src/pilot/gate_evidence_selftest.py` (12 RED pins) · `gate_evidence.py --require <gate_id>` in CI | 08/26 | Adding a tenth gate whose green line means nothing · [gate_evidence.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/gate_evidence.py) · [gate_evidence_selftest.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/gate_evidence_selftest.py) · [the spike](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/docs/SPIKE_PWG_GATE_EVIDENCE_LEGITIMATE_EMPTY_CLASSES_31-08-2026.md) |
| L14 | PWG printed-locus resolver (H3751) | `~~h<N>` in a pwg_ru sub-card key is an **`enumerate` index, not a homonym number** — [`pwg_homonym.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_homonym.py) is the one sanctioned reading (positional resolution + printed `<h>`/`<pc>` off the record header), with a census, a ledgered store-rewrite driver and a RED-pin replay beside it | `python src/pwg_page_index.py --selftest` · `python src/h3751_homonym_census.py --report OUT.md` | 08/26 | Re-deriving a card’s printed column from `key1` alone, or comparing `~~h<N>` against the source `<h>` ([#1801](https://github.com/gasyoun/SanskritLexicography/issues/1801)) · [pwg_page_index.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_page_index.py) · [h3751_homonym_census.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/h3751_homonym_census.py) · [h3751_apply_homonym_locus.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/h3751_apply_homonym_locus.py) |
| L15 | pwg_ru provenance census (H3750) | Read-only classifier that answers, per store row, whether its pipeline provenance is **measured** (the `prompt_sha`/`glossary_sha`/`script_sha` triple), **asserted** (`backfilled: true` + version strings) or **absent** (no input identity) — plus a git-reconstruction probe that scores any proposed backfill method against the rows that already carry the truth, and a corroboration pass against the surviving `wf_output` sidecars. Live verdict: 10,773 / 737 / 9 of 11,519, reconstruction 1 of 9 stamp classes | `python src/provenance_census.py --reconstruct --corroborate` · `--json` · `--selftest` | 08/26 | Re-deriving the asserted share, or attempting a git-history backfill of the pre-04-07-2026 era ([GAPS §19](https://github.com/gasyoun/SanskritLexicography/blob/master/GAPS.md), [FINDINGS §621](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md)) · [provenance_census.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/provenance_census.py) · [census report](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/reports/PWG_RU_PROVENANCE_CENSUS_31-08-2026.md) · [backfill spec](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/docs/SPEC_PWG_RU_PROVENANCE_BACKFILL_31-08-2026.md) |
| L16 | PWG `<ab>` abbreviation bucketer (H3959) | The RU-column abbreviation policy as three **explicit, disjoint** sets in [`pwg_ab_ru.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_ab_ru.py) — `RU_MAP` (122 Bucket-A tokens → Russian, all values Cyrillic), `BUCKET_B` (138 grammatical sigla that stay international Latin), `RESIDUE` (14 declared-undecided, each with a reason). `census` is the gate: it classifies every `<ab>` token in the store and **exits 1** on any token outside all three sets or any non-Cyrillic `RU_MAP` value. Live split over 12,194 occurrences / 272 distinct: B 53.0 % · A-mapped 46.8 % · residue 0.1 % · **A-unmapped 0.0 %**. Ruling: MG 02-09-2026, «some remain Latin, none remain German, most German become Russian and do not become Latin» | `python src/pwg_ab_ru.py census` · `lookup "v. l."` · `coverage` | 09/26 | Guessing a token's bucket, re-deriving the frequency table, or inferring "not in `RU_MAP`" = Bucket B (the defect the ruling outlawed) ([CONTRADICTIONS §4](https://github.com/gasyoun/SanskritLexicography/blob/master/CONTRADICTIONS.md)) · [pwg_ab_ru.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_ab_ru.py) · [ABBREVIATIONS_RU.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/ABBREVIATIONS_RU.md) |
| L17 | PWG four-tier enumeration reader + store-impact meter (H3948) | PWG numbers senses on **four** distinct printed enumeration tiers, not two; [microstructure.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/microstructure.py) is the one sanctioned reading of all four, and [pwg_four_tier_store_impact.py](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_four_tier_store_impact.py) measures what corrected segmentation *would* cost the existing store **without writing a row**: 3,212 of 11,462 `pwg_ru` rows affected (28.02 %) — 12 precisely resolvable · 990 unaffected · 2,210 unresolvable, i.e. a **range of 12–3,212 rows, not a figure**. Three unresolved marker classes are declared rather than guessed. | `python RussianTranslation/src/microstructure_four_tier_selftest.py` · `python RussianTranslation/src/pwg_four_tier_store_impact.py` · `python RussianTranslation/src/pwg_enum_tier_census.py` | 09/26 | Re-deriving tier rules from a marker's shape, re-counting the affected rows by hand, or bulk-re-segmenting the store on the four-tier rule without the unresolved-class filter — that last one rewrites 2,210 rows on a guess · [FINDINGS §453](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md) · [H3948_four_tier_store_impact.json](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/reports/H3948_four_tier_store_impact.json) |
...
| P4 | Systema **grammar exercise widgets** — gender-sort (24) · noun→pronoun agreement (18) · verb-root match (11) · cloze verb-fill (11) + build-your-own generator | **64** fixed · 4 sets + tool | HTML/JS widgets | 🟢 Live | 07/26 | [public/lila/](https://github.com/gasyoun/Systema-Sanscriticum/tree/main/public/lila) |
