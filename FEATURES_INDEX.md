...
| 🟢 C20 | kosha `pwg_L_pc.tsv` (`pwg-print-anchors`) | PWG entry L → printed volume + column, the only page key Cologne's `servepdf.php` honours for a multi-volume dict (H839 `{vol}-{col:04d}`; a bare column serves volume 1 — every committed kosha PWG `scan_url` was bare, FINDINGS §581) | 122,730 rows · 1.5 MB | `119742	7	1737` (gam) | 08/26 | [kosha data/pwg_scan](https://github.com/gasyoun/kosha/blob/main/data/pwg_scan/pwg_L_pc.tsv) |
