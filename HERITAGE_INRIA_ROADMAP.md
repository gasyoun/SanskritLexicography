...
_Created: 03-07-2026 · Last updated: 03-07-2026 (H099 Phases 0–2 executed)_
...
| 4 | Morphology databanks | Current inflected-form XML banks → forms oracle for SanskritSpellCheck + third witness beside kosha's vidyut-built forms layer (426,410 pairs) — diff, never overwrite | SanskritSpellCheck, kosha | Phase 0 @DO download; @DECIDE for anything vendored | 🟡 **@DO cleared 03-07-2026** — `SL_morph.xml`/`WX_morph.xml` downloaded by MG, verified (1,286,615 forms / 32,837 stems, v3.81 2026-06-21), staged at `HeadwordLists/heritage_mirror/manual/` (gitignored); see [FINDINGS §51 update](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md). **Build not started** — needs its own H###; still gated on the license @DECIDE for anything vendored beyond local/derived use |
| 5 | Heritage dictionary (DICO) | French gloss layer + entry structure as an additional witness/gloss source for kosha entries | kosha | @DECIDE ruling + Huet outreach reply | ⬜ queued, gated |
