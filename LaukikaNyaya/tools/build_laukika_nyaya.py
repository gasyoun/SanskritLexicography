...
separately (PR #577 and PR #576):

  1. Broadened phrase-tier gloss-opener gate (ex-PR #577, already on
     master): the strict literal "The maxim of" gate found only 4/199
     non-nyaya candidates. looks_like_gloss_sentence() generalizes it to
     any real English topic sentence, raising phrase-tier recall to 93.
  2. Index-cross-referenced recovery pass (ex-PR #576): the OCR text's own
     ...
     else; nothing is synthesized.

Combining both techniques is a genuine sum, not a max(): each pass now
finds a different net-new set (broadened regex catches noise-free phrase
headwords the v1 regex skipped; index-crossref catches noise-DAMAGED
headword lines the regex could never match at all), and the index-crossref
pass is run AFTER the broadened regex pass and told which lines are already
used, so it only ever recovers genuinely new entries.
