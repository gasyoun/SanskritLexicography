...
> ([SIGNOFF_A33_author_pass.md](https://github.com/gasyoun/SanskritLexicography/blob/master/papers/SIGNOFF_A33_author_pass.md)).
> Remaining: venue decision (MG), AP90 Vedic-siglum recall hand-check, optional GRA/PW
> control columns.

## Abstract

It is widely assumed that the 19th-century European dictionaries of Sanskrit — the
...
literary-source siglum a date and a Renou language-state (I–V), I test this at corpus
...
the comparative-philology generation ordered senses historically. I test it.

**Relation to existing work.** The typology I test is the classical one: Zgusta (1971)
...
Rundell 2008) — exactly the split my Vedic-density discriminator recovers empirically
...
That PWG and MW order senses almost identically (§3.1) is expected rather than
surprising: MW's dependence on the Petersburg lexicon is philologically established
(Zgusta 1988; re-examined by Hanneder 2020), so their shared ordering regime is one more
inherited convention — measured here for the first time at corpus scale. My contribution
to this line is quantification: a per-entry, citation-dated test of *which* arrangement
principle a dictionary actually follows, rather than a preface-level characterisation.

## 2. Method

I use the CDSL editions (`csl-orig`) and an existing citation-dating resource built for the
...
For each entry I segment the printed senses (PWG/PW: `<div>`-numbered senses; MW:
...
citations *inside* a sense run oldest→newest. As a single cross-dictionary discriminator I
...
dictionary by re-sorting its senses oldest-first. The measurement above shows the cost: such a
