...
A naive corpus-wide statistic — average senses per entry, citation density, grammar-marking share — therefore reports the two largest indigenous dictionaries as the *emptiest* objects in the corpus. The question this paper answers is whether that emptiness is real (the kośas genuinely lack microstructure) or instrumental (the microstructure is present but encoded by conventions no European-keyed detector reads). The answer, demonstrated on three independent layers, is instrumental; the recovered microstructure is dense, disciplined and internally consistent.

The corpus's measurement doctrine already states the rule the demonstration licenses; it belongs at the outset, since everything below applies it:
