...
2. **DeepSeek read-only recheck (paired verifier family) — INCONCLUSIVE (missing evidence is never PASS, provider unfunded).** Both sanctioned DeepSeek surfaces — the councillor seat and the auto-launched [`class_verifier_launch.py`](https://github.com/gasyoun/Uprava/blob/main/tools/class_verifier_launch.py) mirror window (`openrouter/deepseek/deepseek-v4.1-flash`, brief + log under `Uprava-h5058-evi/tools/reports/class_verifier_H5058_17-09-2026.*`) — were refused by the OpenRouter key's weekly-credit ceiling («This request requires more credits… can only afford 4714» vs 32,000 max_tokens); the direct DeepSeek API is unfunded (MG 14-09 ruling) and `opencode run` exposes no per-call output-cap flag. Conventions/arithmetic/parity tiers therefore remain **executor-verified only** (tier 1 above); the independent DeepSeek round is owed after the weekly reset — GTD `@DO` 0CX (Uprava), handoff **H5058 stays open**.
