...
def main():
    ...
    sample_n = manifest_meta.get('sample_total', sum(
        p.get('n', 0) for p in manifest_meta.get('plan', [])))
    design_bar = manifest_meta.get('wilson_bar_design_target')
    handoff = manifest_meta.get('handoff', 'H1657')
    ...
    if votes:
        ...
        design_note = (
            f" Sample sized for a design target of Wilson LB "
            f"**{design_bar:.2f}** (min n = {manifest_meta.get('min_n_for_bar', '?')}; "
            f"{handoff})."
            if design_bar is not None else ""
        )
        head = (
            ...
            f"sample of **{sample_n:,}** cards CAN prove: the Wilson 95% lower "
            "bound each stratum reaches if its draw comes back perfect, and the "
            f"rows riding on it.{design_note}\n\n"
            "**Bar vote returns after the human votes this sample** (MG ruling "
            "30-07-2026 vote 4c / H1951): re-draw first so 0.95 is attainable; "
            "then set the promotion bar with measured precision in hand. "
            "Nothing is promoted in this handoff.\n"
        )

    with open(OUT_REPORT, 'w', encoding='utf-8', newline='\n') as f:
        f.write(head + "\n")
        f.write("_Created: 26-07-2026 · Last updated: 30-07-2026_\n\n")
        f.write("Generated by [`p2_precision_gate.py`](p2_precision_gate.py) "
                f"({handoff}; lineage H1657, Opus 5 1M `claude-opus-5[1m]`).\n\n")
        f.write("\n".join(body) + "\n")
    print(f"\nreport -> {OUT_REPORT}")
