...
**File analysed:** [`61267-Sanskrit-Catalan-Words-List.txt`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/61267-Sanskrit-Catalan-Words-List.txt) (mirrored here from the `sanskrit-lexicon/CORRECTIONS` repo)
**Source work:** *Diccionari Sànscrit–Català* (Òscar Pujol, Enciclopèdia Catalana, 2005 — the first Sanskrit→Catalan dictionary)
**Date:** 2026-06-25 · analysis scripts: [`coverage_by_dict.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/coverage_by_dict.py), [`match_rate.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/match_rate.py), [`make_uncovered_lists.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/make_uncovered_lists.py)
...
- **The remaining ~8 % are *not* in any CDSL dictionary.** Against the full set of **43** CDSL dictionaries (not just the 15 above), **4,670 unique keys / 4,680 headwords (7.9 %)** stay unmatched. The complete categorised list is in [`Catalan-uncovered-by-CDSL.txt`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/Catalan-uncovered-by-CDSL.txt); see §4 below for the breakdown.
...
Matching the 59,377 normalised keys against `<k1>` of **all 43** CDSL dictionaries leaves **4,680 headwords (7.9 %)** unaccounted for. The complete list lives **one file per category** under [`Catalan-uncovered/`](Catalan-uncovered/) (each a Markdown table of headword + normalised SLP1 key); a single combined dump is in [`Catalan-uncovered-by-CDSL.txt`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/Catalan-uncovered-by-CDSL.txt). All generated by [`make_uncovered_lists.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/make_uncovered_lists.py).

| Category | Count | Per-category file | What it is |
|---|---:|---|---|
| `compound(2)` | 2,397 | [`02-compound-2.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/Catalan-uncovered/02-compound-2.md) | Two-member compounds written solid (sandhi-joined), MW lists only under a parent or not at all |
| `simple` | 1,421 | [`01-simple.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/Catalan-uncovered/01-simple.md) | Single rare derivatives, variant spellings, and solid sandhi-forms |
| `long-compound(3+)` | 600 | [`03-long-compound-3plus.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/Catalan-uncovered/03-long-compound-3plus.md) | Three-plus-member compounds — heavily **Vedānta / philosophical** vocabulary |
| `suspect-char` | 126 | [`00-suspect-char.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/Catalan-uncovered/00-suspect-char.md) | File artifacts, **not real missing words** (see below) |
| `prefixed-root` | 107 | [`05-prefixed-root.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/Catalan-uncovered/05-prefixed-root.md) | Preverb + root combinations (`abhi-saṃ-ā-√gam`) not headworded as roots in CDSL |
| `root` | 29 | [`04-root.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/Catalan-uncovered/04-root.md) | **Denominative / causative roots** (`√nāthaya`, `√dolaya`, `√kuṇṭh`) Cologne/Whitney don't list as roots |
...
This corner is **not all convention** — it splits into convention/morphology vs genuine candidate gaps. The **complete categorised list of all 177** is in [`DCS-attested-no-CDSL.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/DCS-attested-no-CDSL.md) (band-sorted, reproducible via [`coverage_vs_dcs.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/coverage_vs_dcs.py)); summary triage:
...
2. **Candidate new headwords for CDSL — corpus-confirmed.** The genuine residue in [`DCS-attested-no-CDSL.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Catalan-Pujol/DCS-attested-no-CDSL.md) (rare lexemes, esp. plant/animal names like `alasāndra-`, `kustumburī-`) is attested in real texts *and* absent from all 43 CDSL dictionaries — the highest-value slice to propose to the dictionary makers, distinct from §4's dictionary-only list.
...
6. **Cross-resource interoperability key.** The normalisation recipe (§6 upshot) is the bridge that lets the Pujol list, the CDSL `<k1>` sets, and the DCS lemma index be joined on one accent-/compound-insensitive SLP1 key — reused for the [Huet/INRIA](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/Huet-INRIA-Wordlist-vs-Cologne.md) control too.
