...
## [1.144.171] - 2026-09-18

- **H4749 (OxAlpha `zai-coding-plan/glm-5.3-flash`) — the KEWA→PWG join applies the two morphological rungs H3169 had only used to size its residue: 476 of the 2,413 unmatched rows now match, 78.87 % → 83.04 % (9,005 → 9,481 of 11,418), landing exactly on the memo's ≈83 % ceiling.** [`join_kewa_pwg.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/etym/join_kewa_pwg.py) gains two lowest-ranked, witness-subordinate rule rungs — `present-stem->root` (strip `ati ate oti ute Ati Iti` → PWG root; `Kallate`→`Kall`) and `feminine->stem` (feminine `-ī` → masculine stem in H3169's priority: `-in` first `SIrI`→`SIrin`, then the bare `-vat`/`-ant` stem `gavatI`→`gavat`, then thematic `-a` `devI`→`deva`) — with every matched row flagged `rule-rung-applied` so the class stays reviewable and never inflates coverage silently. **Purely additive, proven by diff against the committed crosswalk:** exactly 476 rows changed, all 476 previously `unmatched`, all flagged, the other 10,942 byte-identical, every pre-existing class count unchanged (exact 2,023 · sandhi 5,670 · witnesses 634/632 · ambiguous 46 · `-as` fix 129 · routes-disagree 612). Selftest on the pure rung functions 10/10 (`--selftest`); spot adjudication of a seed-4749 sample 20/20 morphologically sound (same declared limitation as H3169: an AI adjudicator, not an editorial sign-off). [`sample_kewa_join.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/etym/sample_kewa_join.py) grows the two strata (sample 72 → 84, same seed 3169); crosswalk + summary + sample regenerated; the honest residue stays a reportable class at 1,937 (`no-nearby-pwg-headword` 1,921 + key-unusable 16). No store mutation, no `csl-orig` edit, no near-miss collapse, lane discipline untouched. Report: [KEWA_JOIN_RUNG_EXTENSION_H4749_15-09-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/docs/KEWA_JOIN_RUNG_EXTENSION_H4749_15-09-2026.md) · handoff: [H4749](https://github.com/gasyoun/Uprava/blob/main/handoffs/H4749-OxAlpha_SanskritLexicography_xwalk-s16-kewa-476-rung_14.09.26.md).

## [1.144.170] - 2026-09-15
...
- **H4837 adjudication shipped (OxAlpha/GLM) — the frozen MW-missing-Nachträge list + verdict workbook + Tier-A stubs + #119 draft reply.**
  New [`HeadwordLists/pw_nachtrag_vs_mw.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/pw_nachtrag_vs_mw.py)
  (builder, byte-identical rerun proven) →
  [`MW-missing-PW-Nachtrag-candidates-14-09-2026.{md,tsv}`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/MW-missing-PW-Nachtrag-candidates-14-09-2026.md):
  30,602 starred-k2 Nachträge headwords, **3,353 absent from MW99** (A 889 / B 1,335 / C 1,129; **273 MAHĀVY** = the mint's derived stat, exact; MW cleaned union 194,283 = recheck §0, exact).
  [`pw_nachtrag_adjudicate.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/pw_nachtrag_adjudicate.py) +
  [`MW-NACHTRAG-ADJUDICATION-14-09-2026.tsv`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/MW-NACHTRAG-ADJUDICATION-14-09-2026.tsv):
  2,743 confirmed-missing / 610 variant-form-in-MW, DCS-conllu lemma+form join (198/96 attested), risk flags (fold-twins, orthographic doublets, kAritra-class false friends, pratyāhāra).
  [`MW-NACHTRAG-TIERA-STUBS-14-09-2026.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/MW-NACHTRAG-TIERA-STUBS-14-09-2026.md) (889 stubs, 349 clean adds) +
  ...
  30-sample stratified spot-check 0/30 false positives. ⚠️ handoff-quoted 3,024/618/1,427/979 baseline was untracked-only (never landed); Δ329 + convention hunt documented in companion §3.
...
- **H4878 canonical adjudication shipped (OxAlpha/GLM) — every candidate of the deterministic `sup_7` pipeline verdicted + Tier-A stubs + #119 draft reply.**
  [`mw_pwk_nachtraege_adjudicate.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/mw_pwk_nachtraege_adjudicate.py) over the byte-identical re-derived candidate list (4,151 rows = master's recorded figure):
  [`MW-NACHTRAG-ADJUDICATION-14-09-2026.tsv`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/MW-NACHTRAG-ADJUDICATION-14-09-2026.tsv) —
  **1,751 confirmed-missing** (572 corroborated ≥2 dicts), 482 covered-by-MW-stem-form, 227 covered-by-fold-twin-flagged (`Akalita`/`akalita` SLP1-case trap), 1,691 near-form-needs-eyes;
  cross-dict corroboration over 36 dictionaries, MAHĀVY cites, DCS-conllu lemma/form attestation per row; trial rank A/B/C preserved.
  [`MW-NACHTRAG-TIERA-STUBS-14-09-2026.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/MW-NACHTRAG-TIERA-STUBS-14-09-2026.md) (572 stubs) +
  [`DRAFT_REPLY_csl-corrections-119-14-09-2026.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/DRAFT_REPLY_csl-corrections-119-14-09-2026.md) (posting = MG ruling).
  Fresh deterministic-seed 30-sample spot-check: **0/30 false positives** ([evidence](https://github.com/gasyoun/SanskritLexicography/blob/master/HeadwordLists/MW-NACHTRAG-SPOTCHECK-30-14-09-2026.tsv)).
  H4837's star-based sweep retained as the all-layer companion pass (artifacts renamed `MW-STARRED-NACHTRAG-*`).
...
## 1.144.169 — tagged and released without a master section (flow-doc mirror class) - 2026-09-14

> **No `## [1.144.169]` heading ever existed on `master` and none is created (18-09-2026).** [v1.144.169](https://github.com/gasyoun/SanskritLexicography/releases/tag/v1.144.169) was cut from a branch head — "release: 1.144.169 (RussianTranslation) — H4528 hang watchdog, H4529 width policy, H4209 shell quoting" ([PR #2202](https://github.com/gasyoun/SanskritLexicography/pull/2202)) — and tagged + released 14-09, while its CHANGELOG section never reached `master`: the [release-tag-flow](https://github.com/gasyoun/SanskritLexicography/blob/master/docs/agents/release-tag-flow.md) mirror class (tags with no heading — the 09-09-2026 census explicitly left it untouched). Tag and release are real and stand; retargeting a published tag is an outward-facing act and is not taken. The number is consumed — content lives in the release notes and the PR.
...
## [1.144.164] - 2026-09-09

- **Release tag backfill — 22 documented-but-untagged versions resolved, and the PR gate named as the cause.** `cut_release.py --verify-releases` reported 22 `## [x.y.z]` headings on `master` with no `refs/tags/vx.y.z`, the freshest being `1.144.161`, promoted the same morning by [PR #2156](https://github.com/gasyoun/SanskritLexicography/pull/2156). Every one was checked against the commit that introduced its heading: **none was a phantom** — all 22 headings landed on `master` on the date they claim. **21 were tagged and released** at their introducing commit (the convention already holding for `v1.144.7`/`v1.144.148`/`v1.144.157`/`v1.144.158`/`v1.144.100`/`v1.144.103`: `vX.Y.Z` points at the commit that introduced `## [X.Y.Z]`), all 21 confirmed ancestors of `origin/master` by `--verify-tag` (21/21 pass, 0 orphans — the [FINDINGS §646](https://github.com/gasyoun/Uprava/blob/main/FINDINGS.md) class), published with `--latest=false` so `v1.144.163` keeps the Latest badge. The 22nd, `1.144.102`, is **not a release and was not tagged**: it is residue of the duplicate-`[1.144.101]` repair recorded under 1.144.103 — the H2877 entry was renumbered into a number the root series never cut (root runs 1.144.101 → 1.144.103) and that pwg_ru had already spent, while its commit `a06059e6` already carries the published tag `v1.144.101`; the heading is demoted and annotated in place rather than left to report a phantom, and the published tag was **not** retargeted (still the human decision 1.144.103 flagged). **Systemic cause, now documented in [CLAUDE.md](https://github.com/gasyoun/SanskritLexicography/blob/master/CLAUDE.md):** `master`'s required check `RussianTranslation gates` forces every release through a PR, so `--apply --tag --push` would tag a branch head the squash-merge then replaces — the tag must be pushed against the **merge commit, after the merge**, and that step had no home in the repo's own docs. Two survey notes: the census reads `refs/tags/vX.Y.Z` only, so the older bare tags `1.144.6`/`1.144.7`/`1.144.84` were invisible to it (v-prefixed twins now exist); and the mirror class — 76 tags with no heading — is untouched here.
## [1.144.162] - 2026-09-09

- H4438: **`DHĀTUP.`→Palsule concordance — the reusable finding is that a coverage denominator can be a property of your regex set rather than of the source.** PWG writes its dhātupāṭha citations in three markup forms; the builder read two, so the published `1465 of 1751 (83.7%)` measured the reader, not the dictionary. Admitting `<ls n="DHĀTUP.">4,13</ls>` (320 occurrences, 270 coordinates, 141 cited no other way) gives `1573 of 1890 (83.2%)` — a *smaller* percentage over a *better* table, proven by the independent witness rather than asserted: agreement with Monier-Williams rose **77.7% → 80.1%** and `mw_only_coords` fell **124 → 33**. The two rates are not comparable and may not be printed side by side as a regression. Second reusable half: **a citation form that prints no source label beside its numbers cannot be trusted to certify its own coordinate space** — it carries no local redundancy against which a number carried over from the citation before it would show, and building the ceiling from all three forms would have let `27,71` (Monier-Williams's `xxvi, 71`, gaṇa carried forward from the `27,16` four words earlier) raise gaṇa 27's ceiling from an attested 33 to 71. The independent verifier refuted the first statement of this — «an attribute is inheritable» — because in `27,71` the gaṇa IS printed body text; what the attribute removes is the repeated `DHĀTUP.` that makes the other two forms read as a fresh citation rather than a continuation. `FEATURES_INDEX.md` L18 and `FINDINGS.md` refreshed; full adjudication of H4432's seven residues in [RussianTranslation/ABBREVIATIONS_RU.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/ABBREVIATIONS_RU.md).
## [1.144.161] - 2026-09-09
...
## 1.144.102 — never cut at repository root (renumbering artifact) - 2026-08-27

> **No `v1.144.102` tag exists at repository root and none was created (09-09-2026).** This heading is residue of the duplicate-`[1.144.101]` repair recorded in the 1.144.103 section above: two `## [1.144.101]` headings had landed, and the H2877 entry below was renumbered to 1.144.102 — a number the root series never cut (root runs 1.144.101 → 1.144.103, both tagged) and one already spent by the pwg_ru side of the shared namespace ([RussianTranslation/CHANGELOG.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/CHANGELOG.md) `## [1.144.102]`, cut alongside root 1.144.101). The entry below is **already tagged and released**: its commit `a06059e6` carries the published tag [v1.144.101](https://github.com/gasyoun/SanskritLexicography/releases/tag/v1.144.101), which is exactly the tag↔changelog disagreement the 1.144.103 entry left for a human — retargeting a published tag is an outward-facing act and was **not** taken here either. The heading is demoted from `## [1.144.102]` so `cut_release.py --verify-releases` stops reporting a release that was never cut; the other 21 documented-but-untagged headings were tagged and released on 09-09-2026 instead.
