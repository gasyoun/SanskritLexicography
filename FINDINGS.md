...
**Arm C is decisive:** the exact production schema (3 `$defs` — card/record/sense — nested arrays, enums, descriptions, `additionalProperties:false`, 1 573 B) under the exact production flag returned a well-formed `{"cards":[…]}` in two turns. Plan mode does not substitute a schema and does not break structured output; the schema shape is not the problem either. §606’s causal headline is struck. What survives it is only the design point: plan mode is used as a sandbox rather than for planning, and arms B/D confirm `--tools ""` works and is strictly stronger containment — a reasonable change, but **not** a fix for this failure, because there is now no evidence this failure is about the permission mode at all. **What actually separates pass from fail is scale, which no one varied.** The two failing production calls each ran **10 turns** with 368–496 K cache-read tokens against a 14.5 KB 5-card prompt; every passing canary ran **2–3 turns** with ~300 B. Permission mode and schema were identical across both groups. The live hypotheses are therefore batch size, prompt length, turn count, or the masked `{Tn}` content — and not one of them is what the model named. **Transferable rule:** *a model’s explanation of its own failure is generated from the same context that produced the failure, and it will confidently name a plausible mechanism it has no way to observe.* Treat it as a lead to test, never as a cause to record. Here it was recorded as cause twice — once in §604, once in §606 — and four calls costing **$0.963 in total** overturned both. The discriminating experiment was available the whole time and cost less than a thousandth of the window that produced the puzzle; **when a finding turns on a mechanism, price the experiment that would refute it before writing the finding down.**
