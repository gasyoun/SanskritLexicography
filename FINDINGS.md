...
(currently §633) whatever its section, so existing numbers never shift; when a finding is later
...
- 🟠 [§494. Homonym token-attribution residual is a 38-group single-lemma_id ceiling](#494-homonym-token-attribution-residual-is-a-38-group-single-lemmaid-ceiling)
...
- 🟠 [§520. UD `Tense=Past` is not the end of the aorist/perfect story — DCS's own `feat_formation` re-splits it, and the "too sparse to use" verdict was a denominator error](#520-ud-tensepast-is-not-the-end-of-the-aoristperfect-story-dcss-own-featformation-re-splits-it-and-the-too-sparse-to-use-verdict-was-a-denominator-error) — the seven Whitney aorist formations give Aorist 12,054 and `peri` gives Periphrastic Perfect 4,046 within the 93,329-token finite past indicative. Quote as bounds: aorist a LOWER bound, perfect an UPPER bound.
...
- 🔴 [§470. The Cologne scan-viewer page PDFs can carry an embedded digitized text layer](#470-the-cologne-scan-viewer-page-pdfs-can-carry-an-embedded-digitized-text-layer-check-gettext-before-declaring-no-e-text-exists-or-commissioning-ocr) — the ramayanagorr Google-sourced page PDFs held a clean Devanagari text layer; the full Gorresio e-text (10,225 vv) was extracted with zero new OCR the same day "no e-text exists" was concluded. Check `get_text()` per VOLUME before commissioning OCR; anchor ॥N॥ segmentation to an external per-page verse index (OCR drops digits).
...
- 🔴 [§467. corpus_lexicon.jsonl gets its first intrinsic BLI quality number](#467-corpuslexiconjsonl-gets-its-first-intrinsic-bli-quality-number-and-the-obvious-gold-source-the-corpuss-own-glossary-is-circular-so-the-fix-is-an-independent-dictionary-ranked-by-an-independent-frequency-source) — P@1 0.402 / MRR 0.539 / coverage 99.5% against an independent Kochergina+DCS gold set; the corpus's own 3-layer glossary was rejected as gold because it is built FROM this same file.
- 🔴 [§469. `to_slp1` is case-preserving, so a capitalised IAST headword transliterates into a DIFFERENT SLP1 letter](#469-toslp1-is-case-preserving-so-a-capitalised-iast-headword-transliterates-into-a-different-slp1-letter-60-of-ncc-match-keys-are-wrong-and-14379-exact-accncc-matches-were-never-proposed) — 60% of NCC match-keys were wrong, 93.3% of Tier D was an artefact, and 14,379 true exact matches were never proposed because the corrupted key changed the blocking letter. ✅ fixed + re-run 26-07-2026 (H1671), incl. the org-wide caller audit: the library and csl-atlas already defend silently, csl-apidev does not.
- 🔴 [§463. The pwg_ru store's `de` field is NOT a faithful copy of the csl-orig German](#463-the-pwgru-stores-de-field-is-not-a-faithful-copy-of-the-csl-orig-german-russian-connectives-have-been-substituted-into-the-source-of-truth-string) — 11 rows have `и`/`для`/`в`/`С` substituted for German connectives and do not round-trip against csl-orig; `sense_tag` (110 rows) and `h` carry Russian prose, so `h` is unusable as a homonym key.
- 🔴 [§464. The H1624 G1 `gloss_lang` classifier mislabels German as Latin/English about half the time it fires](#464-the-h1624-g1-glosslang-classifier-mislabels-german-as-latinenglish-about-half-the-time-it-fires-and-those-spans-are-then-withheld-from-translation) — 122 of 229 non-DE spans are German (77% FP on `english_content`), and `la`/`en` are marked `translate: False`, so those glosses never reach the model.
...
- 🔴 [§9. DCS OccId and sent_id are not unique keys](#9-dcs-occid-and-sentid-are-not-unique-keys) — PK collisions silently dropped tokens and 449 sentences before synthetic keys.
...
- 🟠 [§453. PWG's sense-closing glyph "〉" nests FOUR enumeration tiers, not two](#453-pwgs-sense-closing-glyph-nests-four-enumeration-tiers-not-two-greek-letters-and-roman-numeral-markers-are-unrecognised-by-the-ru-pipelines-splitter) — Greek letters (1,444 occurrences) and roman-numeral markers (30) unrecognised by `microstructure.py`'s `MARK` regex, the same bug class as §447 one tier deeper. **FIXED + MEASURED (H3948, 03-09-2026):** the splitter now recognises all four tiers; 3,212/11,462 store rows would change on the conservative key1-only join, only 12 provably so, 2,210 unresolved — measured, no row rewritten.
- 🟠 [§454. The pwg_ru RU store's `h` field has inconsistent semantics — not a reliable homograph-number join key](#454-the-pwgru-ru-stores-h-field-has-inconsistent-semantics-not-a-reliable-homograph-number-join-key) — holds a digit, an empty string, or a root-word string within the same file; 93.78% of store rows (10,881/11,603) touch a headword whose corrected `〉` segmentation changed sense count.
...
- 🔴 [§102. DCS `text_sandhied` is not reliably sandhied](#102-dcs-sentencetextsandhied-is-not-reliably-sandhied-some-rows-store-analyzed-word-forms-which-silently-downgrades-verbatim-quotations-to-partial-matches) — some rows store analyzed word forms; cross-check a real-surface corpus.
...
- 🟡 [§39. devanagari_to_slp1 mis-routes retroflex la](#39-devanagaritoslp1-mis-routes-retroflex-la) — ळ → x instead of L.
...
- 🟡 [§55. `gen_opt_harness2.py` output-budget: coarser wins on both knobs, in opposite directions](#55-genoptharness2py-output-budget-coarser-wins-on-both-knobs-in-opposite-directions)
...
- 🟠 §103. `10.5281/zenodo.15834721` is a false DOI, cited as genuine in two different repos — resolves to an unrelated topology preprint, not any Sanskrit-lexicon dataset; csl-observatory's own `CITATION.cff` carried it as OBS-T's "concept DOI".
...
- 🟡 [§70. pwg_ru TM composite grade: A is consensus-gated (5.7%), and a reference-free surface QE cannot detect wrong-sense](#70-pwgru-tm-composite-grade-a-is-consensus-gated-57-and-a-reference-free-surface-qe-cannot-detect-wrong-sense)
- 🟡 [§72. VedaWeb's `id_gra` token field IS the Grassmann `<L>` entry number — no fuzzy text-matching needed for a GRA↔VedaWeb crosswalk](#72-vedawebs-idgra-token-field-is-the-grassmann-l-entry-number-no-fuzzy-text-matching-needed-for-a-gravedaweb-crosswalk)
...
- 🔴 [§567. The memo's `<ls>L.</ls>` "lexicographers-only" marker does not exist in PWG — the real signal is `renou_register.py`'s own `ls`/`dcs` provenance tag](#567-the-memos-lslls-lexicographers-only-marker-does-not-exist-in-pwg-the-real-signal-is-renouregisterpys-own-lsdcs-provenance-tag) — a literal search for `<ls>L.</ls>` in `pwg.txt` finds nothing; the real lexicographers-only signal lives in `renou_register.py`'s own provenance fields.
- 🟠 [§568. Renou diachronic state V is never populated in `pwg_sense_stratum.jsonl`](#568-renou-diachronic-state-v-is-never-populated-in-pwgsensestratumjsonl) — `renou.py`'s canonical `STATES` tuple declares `(I, II, III, IV, V)`, but 0 of 64,296 per-sense rows ever carry state V.
...
- 🔴 [§611. The window audit's "units clean" line is NOT the promotable verdict, and `promote_final_cards.py --merge` is not dry-run by default](#611-the-window-audits-units-clean-line-is-not-the-promotable-verdict-and-promotefinalcardspy---merge-is-not-dry-run-by-default) — `audit_window`'s `PASS: n/n units clean` is markup fidelity only; the promoter guards on `requeue.defect.keys.txt`, and on H3663 the two disagreed 15-of-16 vs 7 clean / 8 defect / 1 transient. Multi-chunk windows need the UNION of the per-chunk requeue dirs; a plain `--merge` writes the store — H3663 (Opus 5 `claude-opus-5`, 29-08-2026).
- 🟠 [§614. `markup_wrapper_dropped` is two different defects, and only the count-match probe tells them apart](#614-markupwrapperdropped-is-two-different-defects-and-only-the-count-match-probe-tells-them-apart) — one flag name covers guillemet drift (repairable), wholesale omission (not — guessing gloss boundaries is what PR #789 refused), and not-a-wrapper-problem-at-all; count `{%…%}` in DE vs RU and `«…»` in RU before promising a repair class — H3663 residual (Opus 5 `claude-opus-5`, 29-08-2026).
- 🔴 [§615. The span-drop class is CLOSED on a real window — and the target-side repair's stated precondition was false on every card of it](#615-the-span-drop-class-is-closed-on-a-real-window-and-the-target-side-repairs-stated-precondition-was-false-on-every-card-of-it) — `hasita`, requeued as unfixable across four handoffs, is audit-clean; `target_anchor_repairs 5/5`, but `german_anchor` fired first on all five, so the repair anchored against a machine-repaired german — H3677 (Opus 5 `claude-opus-5`, 29-08-2026).
- 🔴 [§616. PWG-TM has a paid path outside the reservation kernel, while its green reconciliation ignores nested unresolved placeholders](#616-pwg-tm-has-a-paid-path-outside-the-reservation-kernel-while-its-green-reconciliation-ignores-nested-unresolved-placeholders) — direct xAI dispatch has no durable reservation/timeout/provider-call accounting; the 2,392-row canonical artifact has 79 rows / 609 nested `{Tn}` occurrences while reconciliation says `ok: true` (Codex Sol `gpt-5.6-sol`, 30-08-2026).
- 🔴 [§617. An index and a number that look alike: `~~h<N>` is an `enumerate` position, and the code that read it as a printed homonym reported success precisely when it guessed](#617-an-index-and-a-number-that-look-alike-hn-is-an-enumerate-position-and-the-code-that-read-it-as-a-printed-homonym-reported-success-precisely-when-it-guessed) — a positional index compared against PWG’s printed `<h>` (which starts at 1) put another homograph’s column on 1,278 of 5,205 mappable pwg_ru store rows, and the `matched` counter incremented on the guess path (Opus 4.8 `claude-opus-4-8`, 31-08-2026, H3751 / #1801).
- 🟡 [§618. A 939-row static I/O-in-loop census overstated coordinator.py's real hotspots by roughly two orders of magnitude — cProfile on a realistic fixture found exactly one](#618-a-939-row-static-io-in-loop-census-overstated-coordinatorpys-real-hotspots-by-roughly-two-orders-of-magnitude-cprofile-on-a-realistic-fixture-found-exactly-one) — most static hits were already-bounded or already-fixed by an earlier pass; the one real redundant-rescan in `save_state`/`write_dashboard` was found by profiling, not by reading the list — measured -21.5% calls / -11.1% pure-Python tottime (Opus 5 `claude-opus-5`, 31-08-2026, H3754 / PR #2001).
- 🔴 [§620. Splitting one overloaded field into two axes does not fix it unless the OLD field's value also changes — readers take the old name alone](#620-splitting-one-overloaded-field-into-two-axes-does-not-fix-it-unless-the-old-fields-value-also-changes-readers-take-the-old-name-alone) — 4,187 rows still read `restate` two weeks after the correct `placement` boolean shipped beside them, because every chip, rollup and headline takes the name on its own; derive the old field from the new one at one site and RED-pin the correspondence as a corpus STOP gate — H3752 (Opus 5 `claude-opus-5`, 31-08-2026).
- 🔴 [§623. A citation siglum can be a book the lexicographer himself printed: two of PWG's 45 dated sources date nothing, and one of them is Böhtlingk's own anthology](#623-a-citation-siglum-can-be-a-book-the-lexicographer-himself-printed-two-of-pwgs-45-dated-sources-date-nothing-and-one-of-them-is-böhtlingks-own-anthology) — `Spr.` is *Indische Sprüche*, Böhtlingk's own 1863–1873 anthology; `ŚKDR.` the Śabdakalpadruma, Calcutta 1821–1858. Both were given ordinary point dates and between them bound 11,771 of 43,990 attestation windows, setting `latest` on 9,082; 2,873 windows have no other dated work at all. A 19th-century apparatus mixes witnesses with authorities under one `<ls>` markup — H3790 (Opus 5 `claude-opus-5`, 01-09-2026).
- 🔴 [§624. A CDSL dictionary code need not name the dictionary its letters suggest — `md` is Macdonell, not Medinī — and the Sanskrit-to-Sanskrit kośas carry no `<ls>` at all](#624-a-cdsl-dictionary-code-need-not-name-the-dictionary-its-letters-suggest-md-is-macdonell-not-medinī-and-the-sanskrit-to-sanskrit-kośas-carry-no-ls-at-all) — Medinī and Amara are not in CDSL at all, and the `md` code that looks like Medinī is Macdonell 1893, so a loader resolving a name by letter-match ships the wrong dictionary under the right header with nothing able to catch it. ŚKDR and VCP carry 0 `<ls>` in 42,531 and 50,135 records, so the shared-witness bridge does not exist for them; the sound channel is the reverse one, PWG's own citations OF them (the §623 authorities), read as directional attributions and ranked below a real convergence. An open-vocabulary `iti X` reading of their prose was measured and rejected — H3862 (Opus 5 `claude-opus-5`, 02-09-2026).
- 🔴 [§621. A version string with no hash behind it is indistinguishable from a measurement — and the reconstruction that would repair it fails 8 of the 9 stamps that can test it](#621-a-version-string-with-no-hash-behind-it-is-indistinguishable-from-a-measurement-and-the-reconstruction-that-would-repair-it-fails-8-of-the-9-stamps-that-can-test-it) — 10,773 of 11,519 pwg_ru store rows (93.5 %) assert a pipeline provenance never measured; recomputing the component hashes from git reproduces 1 of 9 measured stamp classes (20 of 737 rows) because the pipeline runs against an uncommitted tree, so the era is unrecoverable — but its input identity is corroborated for 10,100 rows by the surviving sidecars — H3750 (Opus 5 `claude-opus-5`, 31-08-2026).
- 🔴 [§622. A coverage audit is meaningless until you measure how much of the generated side came from the attested side](#622-a-coverage-audit-is-meaningless-until-you-measure-how-much-of-the-generated-side-came-from-the-attested-side) — kosha's A3 reported "attested but never generated = 2" of 381,413, which recorded only that 93.30 % of its generated side was itself `source='dcs'`; the same question asked of an independent generator (`inflections`, 99.99 % cologne_mwinflect, no DCS input) answers 196,378 of 386,012 — and even that overstates the actionable set 35×, narrowing to 5,656 once verbs, sandhied-surface-only keys and bare stems are subtracted as scope facts rather than defects — H3782 (Opus 5 `claude-opus-5`, 01-09-2026). **Figures superseded, claim intact:** the join key was itself defective and the chain was rebuilt in H3925 — 164,236 of 352,745, overstating 29×, narrowing to 5,588. See [§626](#626-fixing-a-phonological-fold-at-one-position-leaves-the-same-defect-at-every-other-position-the-same-phonology-reaches-and-the-class-built-to-detect-it-goes-to-zero-which-reads-like-completeness).
- 🟠 [§625. pwg_ru has no "translate N pre-selected keys, report-only, never promote" entrypoint — the lease/promote machinery assumes every job was claimed and dispatched once](#625-pwgru-has-no-translate-n-pre-selected-keys-report-only-never-promote-entrypoint-the-leasepromote-machinery-assumes-every-job-was-claimed-and-dispatched-once) — `requeue_from_audit.py` and `coordinator.py prepare-requeue` are both keyed on a coordinator `lease_id`, so an ad hoc translate-only sample that never writes to the store has no lane; H3791 judged improvising one against the lease internals under a live receipt clock too risky and handed it off — H3791 (Sonnet 5 `claude-sonnet-5`, 02-09-2026).
- 🔴 [§626. Fixing a phonological fold at one position leaves the same defect at every other position the same phonology reaches — and the class built to detect it goes to zero, which reads like completeness](#626-fixing-a-phonological-fold-at-one-position-leaves-the-same-defect-at-every-other-position-the-same-phonology-reaches-and-the-class-built-to-detect-it-goes-to-zero-which-reads-like-completeness) — `form_key()`'s word-final anusvāra fix (0.11.0) moved kosha's A¬G 196,378 → 164,236, a third more than the −24,149 sampled on a filtered subset had predicted, and it is a re-partition not a gain (AG net −1,131; 33,267 "attested keys" were spellings of each other). The `orthographic-variant` detector then went 146 → 0, which read as completeness — but medial anusvāra before a labial is /m/ too and still folds to `n`, so 278 of 2,521 slot-conflicts (11.03 %) are `saṃbhavaḥ` vs `sambhavaḥ`, not disagreements. Enumerate every position the phonology reaches; make the rebuild driver refuse to start unless the dependency carries the fix — H3925 (Opus 5 `claude-opus-5`, 02-09-2026).
- 🔴 [§627. A morphological feature present on 16,100 tokens can be categorically absent from an entire mood — DCS's `Formation` never leaves `Mood=Ind`, and the propagation that would recover it reaches 296 of 8,726 tokens](#627-a-morphological-feature-present-on-16100-tokens-can-be-categorically-absent-from-an-entire-mood-dcss-formation-never-leaves-moodind-and-the-propagation-that-would-recover-it-reaches-296-of-8726-tokens) — DCS's `Formation` occurs 17,440 times in the pinned distribution and **never once outside `Mood=Ind`** (16,100 on `Tense=Past`, 1,340 on the periphrastic future), so H1486's `feat_mood='Ind'` predicate hides nothing: the 8,726 non-indicative past tokens (Jus 4,067 · Imp 1,700 · Sub 1,317 · Opt 1,065 · Prec 577) and the 9,112 past participles are unformationed **upstream**. Propagating from identical forms attested elsewhere reaches 296 (3.39 %) — the homography prior was falsified, the limit is that 96.55 % of these forms are never tagged anywhere — so the inference layer was measured and rejected, and `Mood=Jus` past tokens must be quoted by mood only — H3878 (Opus 5 `claude-opus-5`, 02-09-2026).
- 🔴 [§628. A sync ledger that counts only rows cannot see a content-only sync — so the refresh that carried a 75-row text sweep is byte-shaped like the no-op that carried nothing](#628-a-sync-ledger-that-counts-only-rows-cannot-see-a-content-only-sync-so-the-refresh-that-carried-a-75-row-text-sweep-is-byte-shaped-like-the-no-op-that-carried-nothing) — every counter `refresh_tm_mirror.py` writes to `mirror_refresh_ledger.jsonl` is derived from `rid()` = `(key1, subcard, sense_tag, de[:80])`, which excludes `ru` — the field the mirror exists to serve. H3947's refresh picked up the 75 rows H3969's German→Latin sweep changed, and its ledger row carries the same nine zero/unchanged values as the H3751 no-op; only the opaque sha pair differs, and the mirror is Git LFS so `git log` shows nothing either. `audit_store_gates.py` already measures `changed_ru` (75 → 0) one tool over. Carry the payload delta into the ledger; audit every consumer of a row identity that deliberately omits a field — H3947 (Opus 5 `claude-opus-5`, 02-09-2026).
- 🟡 [§629. A seed inventory counted across agent worktrees inflates its own denominator — the "61 IAST-bearing seeds" of GAPS §6 are 32, and the table built over them keys 534 names with zero character rules](#629-a-seed-inventory-counted-across-agent-worktrees-inflates-its-own-denominator-the-61-iast-bearing-seeds-of-gaps-6-are-32-and-the-table-built-over-them-keys-534-names-with-zero-character-rules) — content-hashing H1746's inventory collapses 61→32 seeds and 47→20 pure-Cyrillic; the witness-first Cyrillic→SLP1 table over those 32 yields **534 rows, `rule_derived_keys: 0`**, 83.5 % onomasticon-witnessed, covering 36.4 % / 33.1 % / 39.3 % of the three fully-Cyrillic indices — H3985 (Opus 5 `claude-opus-5`, 03-09-2026).
- 🔴 [§630. The dictionary's *entry ids*, not its headword list, are what separate one lexeme spelled twice from two lexemes — and a presence-only test gets the pinned `bhagavat`/`bhagavant` case exactly backwards](#630-the-dictionarys-entry-ids-not-its-headword-list-are-what-separate-one-lexeme-spelled-twice-from-two-lexemes-and-a-presence-only-test-gets-the-pinned-bhagavatbhagavant-case-exactly-backwards) — a lexicon's identity layer (entry ids, cross-reference targets) is machine-clean and states relations its prose only implies; "both spellings present" is evidence of nothing until the ids are compared, and it fails silently toward over-splitting. Reach reported as a fraction with the unreached rows named (243/279 NOUN lemma_ids, 87.1 %); the ī split needs no dictionary at all, because an NFC-normalised citation form's vowel-nucleus count IS its syllable count — H3984 (Opus 5 `claude-opus-5`, 03-09-2026).
- 🔴 [§631. A form→lemma map and a lemma inventory answer different questions — probing an inflected compound head against the *lemma set* loses 1,018 forms silently, and a fallback lexicon that is sound on whole forms is not sound on compound-internal ones](#631-a-formlemma-map-and-a-lemma-inventory-answer-different-questions-probing-an-inflected-compound-head-against-the-lemma-set-loses-1018-forms-silently-and-a-fallback-lexicon-that-is-sound-on-whole-forms-is-not-sound-on-compound-internal-ones) — the Sa→Ru "morpheme-marker residual" was recorded as a morphology gap ("rightmost element is itself inflected") but was a lookup against the wrong table: `BuvanAt` was a key in the 408,660-entry `dcs_form2lemma.tsv` all along, and one extra probe recovered 1,018 forms / 1,783 tokens (coverage 87.11 → 87.28 %). All 42 vidyut-sourced heads adjudicated exhaustively: 35/42, the failures non-words (`vart`→`varDi`, `sarp`→`sarb`), corroborating wave 2's independent 71.8 % — a blunt provenance fence beat the clever "lemma must be DCS-attested" test. Judge a coverage tier on whether it moves DOMINANT renderings (27 of 40,387 entries, 23 of them hapax ties), not on its item-level gloss score, which alone argued NO-GO — H3876 (Opus 5 `claude-opus-5`, 03-09-2026).
- 🔴 [§632. A contamination share measured over the corpus is not the share inside the stratum you are about to publish — 10.15 % corpus-wide was 29.22 % in the vedic slice, and the caveat that quoted the corpus number understated the affected slice threefold](#632-a-contamination-share-measured-over-the-corpus-is-not-the-share-inside-the-stratum-you-are-about-to-publish-1015--corpus-wide-was-2922--in-the-vedic-slice-and-the-caveat-that-quoted-the-corpus-number-understated-the-affected-slice-threefold) — the H3878 caveat published one damage figure (8,726 of 85,955 = 10.15 %) measured corpus-wide; the H3966 guarded re-run measured it per genre slice and the corpus figure described no slice at all: vedic −29.22 %, classical −3.03 %, epic −1.88 %, puranic −0.88 %. A 33× spread, because the augmentless injunctive is a Vedic category — a mood-shaped defect lands as genre-shaped damage, concentrated exactly where the study's only substantive signal lived. The unmeasured direction of bias was one predicate away and turned out to be strengthening (+1.5 → +2.5 pp); of 73 compared quantities exactly one was material, which is the evidence that "annotate and schedule the re-run" was a sound default — H3966 (Claude Code Opus 5 `claude-opus-5[1m]`, 06-09-2026).
- 🔴 [§633. The second position of the same fold was monotone where the first was a re-partition — and the only way to know which is to run the control arm, because the inputs moved underneath the fix](#633-the-second-position-of-the-same-fold-was-monotone-where-the-first-was-a-re-partition-and-the-only-way-to-know-which-is-to-run-the-control-arm-because-the-inputs-moved-underneath-the-fix) — closing [§626](#626-fixing-a-phonological-fold-at-one-position-leaves-the-same-defect-at-every-other-position-the-same-phonology-reaches-and-the-class-built-to-detect-it-goes-to-zero-which-reads-like-completeness): narrowing `form_key()`'s medial anusvāra fold before a labial (0.12.0) took kosha's medial-labial twins from **278 of 2,521 slot-conflicts to 0 of 2,212** and A¬G 164,236 → 163,058 — **1,178 keys left, 0 entered**, the opposite shape from the word-final fix, which broke 1,131 matches. Both directions were only knowable because `kosha.db` had been rebuilt in between, so the before/after was confounded and a same-inputs control arm had to be run — H3975 (Opus 5 `claude-opus-5`, 06-09-2026).
- 🟢 [§619. `pwg_tm_fragmentize.py`'s GLOSS_RE split 73 discontinuous glosses at `<is>` boundaries, orphaning half of each as an untranslatable fragment — GAPS §18 graduated and closed](#619-pwgtmfragmentizepys-glossre-split-73-discontinuous-glosses-at-is-boundaries-orphaning-half-of-each-as-an-untranslatable-fragment-gaps-18-graduated-and-closed) — census: 73 `<is>`-interrupted gloss events in 58 senses / 53 records; fixed by rejoining before fragmenting, RED-pinned on `viSveSa` 2 (Sonnet 5 `claude-sonnet-5`, 31-08-2026, H3753).
- 🔴 [§613. A planner that feeds production must pass the SAME binding as the coordinator — `no_pwg_scale_plan` stamped a v1 manifest production refuses, so every lease it prepared was unrunnable](#613-a-planner-that-feeds-production-must-pass-the-same-binding-as-the-coordinator-nopwgscaleplan-stamped-a-v1-manifest-production-refuses-so-every-lease-it-prepared-was-unrunnable) — the third of three gates in front of a paid no_pwg window, and the only one an agent can clear; fixed and pinned against `coordinator.py`'s own source — H3677 (Opus 5 `claude-opus-5`, 29-08-2026).
- 🔴 [§610. A dropped span can be re-anchored in the TRANSLATION without parsing prose — the same sense's `german` echo is a surviving parallel, and it carries the position](#610-a-dropped-span-can-be-re-anchored-in-the-translation-without-parsing-prose-the-same-senses-german-echo-is-a-surviving-parallel-and-it-carries-the-position) — `hasita~~h0_zz_pw`, requeued as unfixable across four handoffs, is repaired offline; H3675 (Opus 5 `claude-opus-5`, 29-08-2026).
- 🔴 [§612. The durable evidence root preserves a paid window’s OUTPUTS but not its INPUTS — once the session worktree dies, the window’s cards can never be audit-validated, so they can never be promoted](#612-the-durable-evidence-root-preserves-a-paid-windows-outputs-but-not-its-inputs-once-the-session-worktree-dies-the-windows-cards-can-never-be-audit-validated-so-they-can-never-be-promoted)
- 🔴 [§609. `classify_run` cannot tell a broken harness from a budget you set too small — `code-failure` is its fall-through, so an undersized `--max-calls` sends the next session hunting a phantom gate defect](#609-classifyrun-cannot-tell-a-broken-harness-from-a-budget-you-set-too-small-code-failure-is-its-fall-through-so-an-undersized---max-calls-sends-the-next-session-hunting-a-phantom-gate-defect)
- 🔴 [§608. A `translation-fidelity-reject` discards the card, and `german_anchor` would refuse it anyway — the counter now says `not_reached` instead of `0`](#608-a-translation-fidelity-reject-discards-the-card-and-germananchor-would-refuse-it-anyway-the-counter-now-says-notreached-instead-of-0) — `hasita` replayed offline returns `nothing-missing`, so moving the call site would fix nothing; H3665 (Opus 5 `claude-opus-5`, 29-08-2026).
- 🔴 [§607. A model's stated cause for its OWN failure is a hypothesis, not evidence — the paid lane's "Plan Mode broke StructuredOutput" self-diagnosis is refuted by four canary calls under one dollar](#607-a-models-stated-cause-for-its-own-failure-is-a-hypothesis-not-evidence-the-paid-lanes-plan-mode-broke-structuredoutput-self-diagnosis-is-refuted-by-four-canary-calls-under-one-dollar)
- 🔴 [§606. `--permission-mode plan` is used as a SANDBOX on the paid lane — that misuse, not a schema bug, is what breaks structured output, and `--tools ""` is the primitive the lane actually wants](#606---permission-mode-plan-is-used-as-a-sandbox-on-the-paid-lane-that-misuse-not-a-schema-bug-is-what-breaks-structured-output-and---tools-is-the-primitive-the-lane-actually-wants)
- 🔴 [§605. `german_anchor` CANNOT fire on a `translation-fidelity-reject` card — the two are mutually exclusive branches, so §604's `0` was the only value possible](#605-germananchor-cannot-fire-on-a-translation-fidelity-reject-card-the-two-are-mutually-exclusive-branches-so-604s-0-was-the-only-value-possible)
- 🔴 [§604. `german_anchor` reports 0 repairs on a window that DID drop a span — the `{#…#}` null class was assumed closed, never closed](#604-germananchor-reports-0-repairs-on-a-window-that-did-drop-a-span-the-null-class-was-assumed-closed-never-closed) — the first paid window since the Tier-1 fix shipped returns `hasita~~h0_zz_pw` at `translation-fidelity-reject` with `german_anchor_repairs: 0`, so predicate #2 **fails** and the repair is not merely idle — it is never reached — H3659 (Opus 5 `claude-opus-5`, 29-08-2026).
- 🔴 [§602. A fail-closed COST guard tests telemetry, not permission — so human spend authorization cannot start a paid window on an unpriced lane](#602-a-fail-closed-cost-guard-tests-telemetry-not-permission-so-human-spend-authorization-cannot-start-a-paid-window-on-an-unpriced-lane) — a clean `LIVE_GO` gate, a prepared lease and an explicit human authorization still produced **zero generation calls**: `bounded_supervisor` stops `cost_unevaluable` at the top of the drain loop whenever `--cost-ceiling` is set and Max-route telemetry is missing — H3659 (Opus 5 `claude-opus-5`, 29-08-2026).
- 🔴 [§603. The paid-run entry points resolve on-disk state the worktree cannot see — one refuses, one collides silently, one invents an empty roster](#603-the-paid-run-entry-points-resolve-on-disk-state-the-worktree-cannot-see-one-refuses-one-collides-silently-one-invents-an-empty-roster) — the probe refused (#1034), `no_pwg_scale_plan.py` proposed the already-completed `no_pwg_w02`, and `bounded_staged_run.py` created an empty accounts DB; the real `max_orchestrator.sqlite` is untracked **and** un-gitignored in the guarded main tree — H3659 (Opus 5 `claude-opus-5`, 29-08-2026).
- 🔴 [§601. A promoted pwg_ru fragment can carry untranslated German `{%…%}` or a rewritten `<ab>` and the gate never asked — until GAPS §17](#601-a-promoted-pwgru-fragment-can-carry-untranslated-german-or-a-rewritten-ab-and-the-gate-never-asked-until-gaps-17) — H2684 n=400 sample: **6 GLOSS-DE-RESIDUE** (`upakrama`/`AtmasAt`/`yad`/`yuj`/`sTA`/`aTa`) + **81 AB-MUTATED** (`taruRa` plus systematic `recurring_formula` expansions such as `<ab>demin.</ab>` → `<ab>уменьш.</ab>`); predicates now live in `pwg.tm.gate.v1` — H3644 (Grok 4.6 `grok-4.6`, 28-08-2026).
- 🔴 [§600. The pwg_ru store-touching tools resolve the store three different ways — only the promoter is canonical](#600-the-pwgru-store-touching-tools-resolve-the-store-three-different-ways-only-the-promoter-is-canonical) — `promote_final_cards.py` uses `canonical_store()`; `refresh_tm_mirror.py` and `audit_store_gates.py` default to the executing checkout's copy, so from a worktree they crash and from a checkout with a STALE local store they would silently mirror and audit the wrong file while the promoter wrote the right one — H3654 (Opus 5 `claude-opus-5`, 28-08-2026).
- 🔴 [§599. A paid window can die at Windows process creation (`0xC0000142`) when the host is out of RAM — and a working `claude --version` does not prove the host can carry one](#599-a-paid-window-can-die-at-windows-process-creation-0xc0000142-when-the-host-is-out-of-ram-and-a-working-claude---version-does-not-prove-the-host-can-carry-one) — `b6` returned `0xC0000142` after 53 ms with 1.7 GB free of 15.8 GB and 6 live `claude.exe`; free RAM is an unmeasured launch precondition. Same abort PROVED the §596 salvage fix: `cards_salvaged: 6`, `partial_output: true` where the pre-fix worker wrote nothing — H3654 (Opus 5 `claude-opus-5`, 28-08-2026).
...
- 🔴 [§597. `cost_evaluable: false` is a pipeline attribution gap, not absent price data — the CLI envelope carries real dollars](#597-costevaluable-false-is-a-pipeline-attribution-gap-not-absent-price-data-the-cli-envelope-carries-real-dollars) — a run reading `observed_cost_usd: 0.0, cost_evaluable: false` also read `usage_evaluable: true, priced_calls: 20`, and the failing call's envelope carried `total_cost_usd: 0.4131022`; scaled it corroborated the preflight at ~$5-6. "Unevaluable" is never "zero", and cache-creation (~20× read) is reported apart from cache-read — H3627 (Opus 5 `claude-opus-5`, 28-08-2026).
- 🔴 [§596. A pwg_ru window discards every paid success when one mid-window call fails schema validation](#596-a-pwgru-window-discards-every-paid-success-when-one-mid-window-call-fails-schema-validation) — 20 priced calls, 19 successes, then `structured_output_retry_exhausted` at `b13` and `out.json` never written, so 13 billed headwords were discarded; size a window on the variance of TOTAL LOSS, not mean cost per card — H3627 (Opus 5 `claude-opus-5`, 28-08-2026).
...
- 🔴 [§589. The R4.1 "any SAN-LOSS reaching the store" freeze trigger is a marker grep, not a gate — four real SAN-LOSS rows sit in the pwg_ru store unseen](#589-the-r41-any-san-loss-reaching-the-store-freeze-trigger-is-a-marker-grep-not-a-gate-four-real-san-loss-rows-sit-in-the-pwgru-store-unseen) — `spot_check_daily.store_san_loss_scan` greps `ru` for the literal `SAN-LOSS` string and never recomputes `{#…#}` spans; the real gate over all 11 620 rows finds 4 SAN-LOSS + 1 LS-LOSS rows (`dA` desid. head-line, `dA`+`anu`, `mA`, `pat`, `asvatantra`); the spot-check task is Disabled with 0 receipts ever — runner `audit_store_gates.py` — H3590 (Fable 5 `claude-fable-5`, 27-08-2026).
...
- 🟢 [§578. Accent digitization is three incompatible devices and svarita is essentially un-digitized — `/` lives in `<k2>` of exactly 9 dicts, `\` survives in 17 lemma marks total, and the German-school translit dicts carry accent as acute vowels that never reach the headword field](#578-accent-digitization-is-three-incompatible-devices-and-svarita-is-essentially-un-digitized-lives-in-k2-of-exactly-9-dicts-survives-in-17-lemma-marks-total-and-the-german-school-translit-dicts-carry-accent-as-acute-vowels-that-never-reach-the-headword-field) — `/` is headword-field accentuation in exactly 9 of the 44 dicts (mw 47 589, pw 21 543, pwg 20 876, cae 11 313, ccs 8 476, gra 10 699, lan 2 226, sch 1 124, pwkvn 2 108 lemmas), `\` svarita survives in 17 lemma marks total (pw 5, pwg 10, pwkvn 2), and stc/fri/bur/md-style transliteration carries accent as acute vowels that never reach `<k2>`; the pw-family homonym pair `agnihotra/` n. vs `agni/hotra` mfn. makes the slash itself a sense disambiguator any join must preserve.
...
[GasunsDhatu_2014/revision-2026/PALSULE_AUDIT.md](https://github.com/gasyoun/SanskritGrammar/blob/chore/GasunsDhatu_2014/revision-2026/PALSULE_AUDIT.md).
...
_(Build stamp added 01-09-2026, H3793: the 1,091,528 denominator is the **2026-06-26** corpus_lexicon build. The current post-H309 build holds **1,093,391** rows, so this coverage percentage is correct for its build and should be recomputed, not merely restated, against the newer one — [CONTRADICTIONS §13](https://github.com/gasyoun/SanskritLexicography/blob/master/CONTRADICTIONS.md) ✅.)_
...
Reproducible: [SanskritGrammar/GasunsDhatu_2014/revision-2026/varga_shares.py](https://github.com/gasyoun/SanskritGrammar/blob/chore/GasunsDhatu_2014/revision-2026/varga_shares.py)
...
cross-checks every one of the **1,091,528** `corpus_lexicon.jsonl` word-pairs (the
**2026-06-26** build; the current post-H309 build holds **1,093,391** — build stamp added
01-09-2026, H3793, [CONTRADICTIONS §13](https://github.com/gasyoun/SanskritLexicography/blob/master/CONTRADICTIONS.md) ✅) against
...
> against [`csl-atlas/data/parse-rules/pwg.json`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/data/parse-rules/pwg.json),
...
🟠 **§453 said "not yet applied to the production parser". H3948 applied it — and measured what it would cost, without writing a single store row.** `microstructure.py`'s `MARK` now recognises all four tiers through the *same* code path §447 built for the tier it already handled (one regex widened to `(?<![^\s—])(?:(?P<t>\d{1,2}|[a-z])[)〉]|(?P<g>[α-ωϑϰ]|[IV]+)〉)`, one `ADJACENT_MARKERS` widened in step, `sense_path()` extended with `div`/`sub2`; no parallel splitter was added). Note the asymmetry, and it is deliberate: the two old tiers accept either closer (`1)` or `1〉`), the two new ones accept **only** the `〉` glyph — a bare `a)`-style ASCII roman or greek marker is not attested as an enumeration closer in PWG, and admitting one would have invented a rule. The tier fields are `div` (roman) → `n` (digit) → `sub` (latin) → `sub2` (greek).

**Occurrence counts — three numbers, three methods, none of them "fixed" into agreement (the handoff forbade reconciling them).**

| Count | Method that produced it |
|---|---|
| **~1,474** (1,444 greek + 30 roman) | §453's own figure — H1350 W1.2 raw `〉`-glyph hits, no `MARK` lookbehind, no protected-span filtering |
| **1,546 occurrences over 393 records** | this session reproducing that same H1350 probe method on today's corpus |
| **1,455** (1,426 greek + 29 roman) | this session's genuine method — glyph-closed **and** `MARK` lookbehind **and** outside every `protected()` span (`<ls>`, `<is>`, any tag, `{#…#}`, `{%…%}`) |

The single roman marker that the genuine method drops and the probe method keeps is a phantom: the `U〉` inside `{#juhU〉ma/si#}`, i.e. a Sanskrit span, not an enumeration marker. `seen_by_old_parser = 0` — the two-tier parser matched **none** of the 1,455, confirming §453's under-split claim end to end.

**Store impact — the number this handoff exists to produce.** Script: [`pwg_four_tier_store_impact.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_four_tier_store_impact.py) (read-only; opens the store `'r'` and writes no row), report [`reports/H3948_four_tier_store_impact.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/reports/H3948_four_tier_store_impact.json). The query, stated plainly: segment all 123,366 PWG records twice — once with the pre-H3948 two-tier `MARK`/`ADJACENT_MARKERS`, once with the four-tier pair — collect the `key1` values whose leaf-sense **id list** differs; then join the `pwg_ru` store's rows onto that set on `key1` alone, because §454 already proved `(key1, h)` is not a usable join key.

- corpus: **331 of 106,082 distinct `key1` change segmentation**.
- store: **11,462 rows over 209 distinct `key1`**; **9** of those `key1` are in the changed set.
- **CONSERVATIVE would-change rows (key1-only join): 3,212 / 11,462 = 28.02 %** [95 % CI 27.21–28.85 %]. This is the headline number, and it is deliberately an upper bound — a `key1` match does not prove the row's own sense moved.
- Resolving each row's `sense_tag` against the pre-H3948 sense ids splits that 3,212 three ways: **12 rows (0.10 %,** [95 % CI 0.06–0.18 %]**) resolve to a pre-H3948 sense id that the fix changes** — the precise lower bound; **990** resolve to a sense the fix leaves alone; **2,210** resolve to **no** pre-H3948 sense id at all.
- most-affected `key1`: `gam`×673, `han`×596, `viS`×537, `vas`×405, `vad`×297, `hA`×247, `siD`×187, `yat`×159, `man`×111.

**The re-translation decision is therefore a range, not a figure: between 12 and 3,212 rows, and the 2,210-row unresolved class is what makes the range that wide.** Nothing was requeued, re-segmented or rewritten; the store's sha256 `19fcf5258e5ea384…` is byte-identical before and after the measuring pass.

**Unresolved classes, declared rather than guessed** (the handoff's ambiguity policy: leave an ambiguous marker unsplit and count it separately).

1. **2,210 store rows whose `sense_tag` resolves to no pre-H3948 sense id.** Their fate under the corrected segmentation cannot be decided from the tag alone; they are counted, not assigned.
2. **48 probe records where a bare `〉` survives with no four-tier marker in front of it.** These are the *glued-marker* class — two consumers pre-normalise differently: `leaf_senses()` un-glues marker chains via `ADJACENT_MARKERS`, while `portrait()`/`split_senses()` does not (pre-existing since H1456, which fixed only `leaf_senses`). No tier was invented for them.
3. Any marker whose printed tier is genuinely ambiguous stays unsplit by construction — the widened `MARK` matches only the four attested alphabets, so an unattested glyph falls through instead of being coerced into a tier.

**Two observations a future reader will otherwise mis-read as bugs.**

- **Repeated sense ids inside one record are legitimate.** PWG restarts digit (and greek) numbering after each `<div>` line, so one record can emit `I1a`, `II1a`, … Sense ids are also **not comparable across the two parsers**: the old parser's `1` and the new parser's `I1`/`II1` are different senses that happen to share a label, which is why [`pwg_four_tier_diff.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_four_tier_diff.py) prints both segmentations whole instead of a merged +/− diff.
- **`vas`, `hA` and `siD` appear in the store-impact changed set but show identical id lists in the diff tool.** Not a contradiction: the diff tool reports only the **first** record per `key1`, while the impact audit folds **all** homograph records of that `key1`. The change is in a later record.

**Before/after, three real entries, measured** ([`pwg_four_tier_diff.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_four_tier_diff.py), leaf senses): `hvA` 21 → 25 · `akzara` 17 → 22 (the old parser's single `4b` blob is really `4bα 4bβ 4bγ 4bδ 4bε`) · `ati` 6 → 12 · `Gana` 25 → 26. Pinned by [`microstructure_four_tier_selftest.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/microstructure_four_tier_selftest.py): 11/11 unit + 5/5 corpus over all 123,366 records and the 393 H1350 probe records in 9.6 s, and `--prove-revert` confirms **6 tests go red** when the parser is reverted to two tiers.

**Two schemas widened, no data migrated.** Both portrait schemas had to learn the new tiers: [`pwg_ru_lexicographic_portrait.schema.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/schemas/pwg_ru_lexicographic_portrait.schema.json) (base RU, checked by `validate_portrait_schema.py`) and [`pwg_portrait_structural.schema.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/schemas/pwg_portrait_structural.schema.json) (its additive superset, checked by `validate_pwg_portrait.py`). Both now declare `div` and `sub2`, and both allow `n: null` — because a **childless roman division is itself the leaf** and carries no digit of its own (a parent division's pre-sense head text still gets `n = '0'`, which `leaf_senses()` skips).

**The committed `pwg_portrait_validation.json` baseline was stale — read the 0 → 2 diff correctly.** The tracked report claims `passes: 123366` with every failure bucket at 0. A re-run of the **pre-H3948** parser against today's schema gives `pass: 123364, schema-violation: 2` (record_ids `19236`, `81366`). The committed 0 predates H1624 G2 adding `government` to portrait senses and nobody re-ran the validator since. Post-H3948 the numbers are **identical to that measured baseline** — `pass: 123364, schema-violation: 2, unclassified: 0, assertion OK` — so **H3948 introduces no portrait regression**; the 0 → 2 movement in the committed file is a stale-report artifact, not this change.

**A latent defect found in passing, left unfixed as out of scope.** Those 2 violations are an extractor/schema disagreement over non-government cases: `government_census.extract_government` documents that parenthesised `nom.`/`voc.` are excluded, but the paren branch only skips a hit when **all** its cases are non-government, and the `MIT_RE` branch applies no such filter at all — so `nom` reaches `sense.government.cases`, which the structural schema's enum `["acc","loc","instr","gen","dat","abl"]` forbids. Record `19236` (`key1=ko`) sense `n='1'` carries `['nom']` from `mit dem <ab>nom.</ab>`; record `81366` (`key1=yaTAvftta`) sense `n='1' sub='b'` carries `['nom','acc']` from `(<ab>nom.</ab> oder <ab>acc.</ab>)`. Both are byte-identical under the two-tier and four-tier parsers. Whether the extractor should filter or the schema should admit `nom`/`voc` is a separate decision.

**Fence held.** No store row was written, requeued or re-segmented; `pwg_tm_fragmentize.py` (§619) was not touched; the ~1,474 figure was reported alongside, not replaced by, this session's own counts.

> **Source:** H3948 (Opus 5, 🔴3 hard) — [`microstructure.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/microstructure.py) four-tier `MARK` + [`microstructure_four_tier_selftest.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/microstructure_four_tier_selftest.py) · [`pwg_enum_tier_census.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_enum_tier_census.py) · [`pwg_four_tier_diff.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_four_tier_diff.py) · [`pwg_four_tier_store_impact.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_four_tier_store_impact.py) → [`reports/H3948_four_tier_store_impact.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/reports/H3948_four_tier_store_impact.json). — csl-orig (pwg) / RussianTranslation · 03-09-2026 · Opus 5 (`claude-opus-5`)
...
> [H911](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H911-Fable_SanskritLexicography_h818-local-quality-economy-readiness-gate_14.07.26.md) ·
...
**Measured 16-07-2026** (Fable 5 `claude-fable-5`, [H1076](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1076-Fable_MWS_a18-citation-registers-draft_16.07.26.md),
...
**Consequence — fixed 17-07-2026** ([H1086](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1086-Sonnet_csl-atlas_mw-row-regenerate-ls-shapes_17.07.26.md),
...
**Measured 17-07-2026** (Opus 4.8 `claude-opus-4-8`, [H1065](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1065-Opus_WhitneyRoots_alternation-type-induction-nonpaninian_16.07.26.md);
...
> [H1134](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1134-Opus_SanskritGrammar_whitney-aorist-per-text-tagger_17.07.26.md) ·
...
> [H1228](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1228-Fable_SanskritGrammar_whitney-concordance-audit-sangram-kochergina_18.07.26.md) ·
...
> [H1110](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1110-Opus_SanskritLexicography_pwg-ru-post-h1080-audit-fix-skills-c4-restart_17.07.26.md) ·
...
> **Source:** [`test_gate4_all_ids_unique`](https://github.com/gasyoun/SamudraManthanam/blob/codex/web/tests/test_converter.py) +
> [manual corpus launcher](https://github.com/gasyoun/SamudraManthanam/blob/codex/web/scripts/run_corpus_tests.py) ·
...
> **Source:** [H1657](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1657-Opus_SanskritLexicography_acc-ncc-p2-agent-adjudication-49k_26.07.26.md)
...
_26-07-2026 · [H1656](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1656-Opus_SanskritLexicography_gorresio-southern-critical-concordances_26.07.26.md) · [PR #784](https://github.com/gasyoun/SanskritLexicography/pull/784) · Fable 5 `claude-fable-5`_
...
> **CORRECTED 03-09-2026 by [§629](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md) (H3985, Opus 5 `claude-opus-5`).** The 61/47 split is a **denominator artifact**: 10 of the 15 top seeds H1746 ranked were `.claude/worktrees/…` copies of files that exist once in the repo, and the probe's own report truncated `all_hits` at 80. Re-derived over distinct content, the inventory is **32 IAST-bearing seeds and 20 pure-Cyrillic**. The verdict (character rules unsafe, lookup table recoverable) stands; only the counts move.
...
_29-07-2026 · [H1847](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1847-Opus_SanskritLexicography_nws-tag-vocabulary-facets_29.07.26.md) · Opus 5 1M (`claude-opus-5[1m]`)_
...
([H1846](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1846-Opus_SanskritLexicography_h1210-arm-a-coverage-fill_29.07.26.md), 29-07-2026).
...
[`H1920`](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1920-Codex_SamudraManthanam_durable-reference-zero-orphan_30.07.26.md).
...
> Opus 5 (`claude-opus-5[1m]`) · 03-08-2026 · [H2249](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2249-Opus_SanskritLexicography_pwg-bare-cwd-ancestry-leak-fix_03.08.26.md) · [PR #1090](https://github.com/gasyoun/SanskritLexicography/pull/1090)
...
- **`timws.csv`** carries 42 tense/mood **codes** but only 30 distinct names. The name-keyed
  ...
  reported as 4,442 instead of 35,921 + 4,442 = 40,363) — [§520](#520-ud-tensepast-is-not-the-end-of-the-aoristperfect-story-dcss-own-featformation-re-splits-it-and-the-too-sparse-to-use-verdict-was-a-denominator-error)'s
...
Context: [§520](#520-ud-tensepast-is-not-the-end-of-the-aoristperfect-story-dcss-own-featformation-re-splits-it-and-the-too-sparse-to-use-verdict-was-a-denominator-error)
...
> Opus 5 (`claude-opus-5`) · 07-08-2026 · [H2361](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2361-Opus_SanskritLexicography_griffith-en-rv-mandala8-valakhilya-misalignment_07.08.26.md), from a code review of the shipped H2334 pilot ([v1.144.15](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/CHANGELOG.md), [PR #1182](https://github.com/gasyoun/SanskritLexicography/pull/1182)). Full evidence: [GRIFFITH_EN_RV_MANDALA8_VALAKHILYA_MISALIGNMENT_07-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h2361/GRIFFITH_EN_RV_MANDALA8_VALAKHILYA_MISALIGNMENT_07-08-2026.md).
...
> Opus 5 (`claude-opus-5`) · 13-08-2026 · [H2630 (Opus 5) — PREP compare Option A: 4 pairs on the 4 whole-card cards](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2630-Opus_SanskritLexicography_prep-compare-whole-card-4-pairs-option-a_13.08.26.md) · evidence: [h2630/README.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/h1210/h2630/README.md) · [PR #1670](https://github.com/gasyoun/SanskritLexicography/pull/1670). Sealed into the plan's `known_non_equivalences`, so a later reader of `plan.json` cannot re-make the inference.
...
Implication: keep `DEFAULT_MODEL` = Flash and keep canonical hashes unchanged. Do not AND the next Flash sitting to a Pro pair. The re-test shipped as [H2756 (Grok 4.6) — H2754 title-collision residual: Flash PREP one-shot vs incremental warm](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2756-Grok_SanskritLexicography_h2754-flash-oneshot-vs-warm-residual_14.08.26.md) ([§534](#534-h2756-re-test-of-flash-prep-incremental-save-on-a-fresh-50-is-inconclusive-at-02)). Process twin: [Uprava FINDINGS §401](https://github.com/gasyoun/Uprava/blob/main/FINDINGS.md).
...
> Grok 4.6 (`grok-4.6`) · 14-08-2026 · [H2756 REPORT](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/experiments/pwg_cache_economy/h2756_flash/REPORT.md) · [H2756 (Grok 4.6) — H2754 title-collision residual](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2756-Grok_SanskritLexicography_h2754-flash-oneshot-vs-warm-residual_14.08.26.md) · class A (recompute from `h2756/run/summary.json` via `cache_prep_h2756.paired_save_metrics`).
...
> Opus 5 (`claude-opus-5`) · 15-08-2026 · [H2787 (Opus 5) — Funded re-run of the H1210 DeepSeek-vs-Claude A/B](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2787-Opus_SanskritLexicography_h1210-ab-rerun-funded-both-arms_15.08.26.md) · class A (reproduce: run either form of the recipe and list `src/pilot/input/`).
...
> Opus 5 (`claude-opus-5`) · 15-08-2026 · [H2827 (Opus 5) — H180 re-glue vote v2: join Cologne ls-links, surface the glue typology, normalize NWS gloss separators](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2827-Opus_SanskritLexicography_reglue-vote-v2-ls-links-typology_15.08.26.md) · class A (reproduce: `python src/ls_coverage_probe.py`).
>
...
> Opus 5 (`claude-opus-5`) · 15-08-2026 · [H2835 (Opus 5) — Mine the mintable `<ls>` citation gaps: classify the 17 % unresolved into format-gap vs no-target, rank by yield](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2835-Opus_SanskritLexicography_ls-citation-gap-mine_15.08.26.md) · class A (reproduce: `python src/ls_gap_repair.py`).
...
> Opus 5 (`claude-opus-5`) · 16-08-2026 · [H2859 (Opus 5) — Give the re-glue vote real evidence: measure PWG-vs-supplement gloss/citation overlap, rank cards by chip-vs-measurement disagreement](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2859-Opus_SanskritLexicography_reglue-vote-evidence-overlap_15.08.26.md) · class A (reproduce: `python src/build_reglue_evidence_sheet.py`).
>
...
> Opus 5 (`claude-opus-5`) · 16-08-2026 · [H2879 (Opus 5) — Волна 1: развести ось привязки и метку типологии + нормализация тегов смыслов](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2879-Opus_SanskritLexicography_placement-axis-split-w1_16.08.26.md) · class A (reproduce: `python src/placement_axis_check.py`). Issue #1736 stays open — waves 2–4 (edits inside PWG, SCH, MW/AP) are untouched.
...
[H2850](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2850-Opus_SanskritLexicography_rv-citation-pada-alignment-elizarenkova-rvlinks_15.08.26.md).
...
"Nachtrag" — the §541 axis defect one layer down.
...
csl-whitroot/jpg/whit-023-kft2.jpg,
...
0 states `dcs`) for [H2856](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2856-OxAlpha_SanskritLexicography_ghost-headword-census-translation-drift-survival_15.08.26.md):
...
[H2856](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2856-OxAlpha_SanskritLexicography_ghost-headword-census-translation-drift-survival_15.08.26.md)
...
for the instrumental case. When [H2849](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2849-Sonnet_SanskritLexicography_german-case-abbreviations-to-latin-compliance-sweep_15.08.26.md)
...
> · [H3590](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3590-Fable_RussianTranslation_pwg-ru-translation-store-audit_27.08.26.md)
...
> · [H2878](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2878-Opus_RussianTranslation_pwg-c1-no-output-progress-watchdog_16.08.26.md)
...
### §599. A paid window can die at Windows process creation (`0xC0000142`) when the host is out of RAM — and a working `claude --version` does not prove the host can carry one

🔴 **`returncode 3221225794` (`0xC0000142`, `STATUS_DLL_INIT_FAILED`) with a sub-100 ms `elapsed_ms` is a host-memory failure, not an API, rate-limit, or schema failure — the CLI child never starts, so that batch costs nothing but the window ends.** **Evidence (H3654, 28-08-2026):** the H3627 23-key manifest was re-run on c1; batches `b0`-`b5` returned 0 (`_anana`, `anukamp_a`, `_apta`, `a_sru`, `_atura`, `havyav_aha`, 72 494-250 854 ms each), then `b6` (`h_uti`) returned `0xC0000142` after **53 ms** and was classified `process`. Measured on the host immediately after: **1.7 GB free of 15.8 GB RAM with 6 concurrent `claude.exe` processes**, each binary 217 MB. `claude --version` still succeeded afterwards — a trivial spawn fits where a full headless translate does not, so a green `--version` is **not** evidence the host can carry a window, and "the CLI works" is not a diagnosis. **Implication:** free physical memory is a launch precondition of the same kind as the gate's latency reading, and it is currently unmeasured; add it before the first paid call rather than discovering it at batch *n*. Distinguishing signature vs a model-side failure: returncode `0xC0000142` **and** elapsed under ~100 ms **and** zero usage on that reservation. **Same run, the good news:** this abort is what proved the 1.144.114 salvage fix from §596 — `summary.window_aborted = {classification: process, cards_salvaged: 6}` and `status.partial_output: true`, so `out.h3654.json` (57 773 B) was written **despite** the abort where the pre-fix worker wrote nothing and discarded 19 paid successes. `process` is correctly outside the infrastructure-boundary list (`rate_limit`/`auth`/`connection`/`timeout`/`budget_exceeded`) that must publish nothing per H2056 #944. The schema-park half of the same fix remains **unproven**: `_sr_avaka`, the key that killed H3627, sits at index 19 and was never reached.

> **Source:** [pwg_ru/h3654/H3654_C1_RERUN_28-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3654/H3654_C1_RERUN_28-08-2026.md)
> · H3654 (Opus 5 `claude-opus-5`). — SanskritLexicography · 2026-08-28

### §600. The pwg_ru store-touching tools resolve the store three different ways — only the promoter is canonical

🔴 **`promote_final_cards.py` resolves the store through `store_path.canonical_store()` (env → main-worktree → local), but `refresh_tm_mirror.py` and `audit_store_gates.py` default to `os.path.join(HERE, 'pwg_ru_translated.jsonl')` — the *executing checkout's* copy.** **Evidence (H3654, 28-08-2026):** run from a linked worktree after a promotion, the promoter correctly wrote the canonical `SanskritLexicography/RussianTranslation/src/pwg_ru_translated.jsonl` and announced it (`canonical/shared — not this checkout's …`), while both other tools died with `FileNotFoundError` on the worktree path that has no store. From the canonical main checkout all three agree **by accident**, which is why this went unnoticed. **Implication:** the crash is the *benign* outcome. The dangerous case is any checkout holding a **stale local** `pwg_ru_translated.jsonl` — there the mirror refresh and the store audit would silently operate on the wrong file and report PASS while the promoter wrote the right one, i.e. a green gate over an unexamined store. Port both to `canonical_store()`; until then pass `--src` / `--store` explicitly from anywhere but the main tree. Adjacent papercut found the same pass: `refresh_tm_mirror.py` hardcodes `--handoff` default `H3627`, so an unattended run mislabels its own ledger row. **Related:** the same class as §592 (a gitignored "regenerable" singleton with hardcoded consumers that cannot be rebuilt in a clean worktree).

> **Source:** [pwg_ru/h3654/H3654_C1_RERUN_28-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3654/H3654_C1_RERUN_28-08-2026.md)
> · H3654 (Opus 5 `claude-opus-5`). — SanskritLexicography · 2026-08-28

### §601. A promoted pwg_ru fragment can carry untranslated German `{%…%}` or a rewritten `<ab>` and the gate never asked — until GAPS §17

🔴 **`pwg.tm.gate.v1` used to pass a fragment whose `{%…%}` inner text was still German, and whose `<ab>…</ab>` tokens were not byte-identical to the source — both are one-line predicates on the target string.** `residue_flags` only scanned *stripped* prose for umlauts and a German-function-word list, so `{%Antritt, Anfang, Beginn%}` and `{%beginnt%}` (no umlaut, not in the list) were invisible; `markup_span_flags` counts `<ab>` *open tags*, so `<ab>v. a.</ab>` → `<ab>т. е.</ab>` kept the count and passed. **Evidence (H3644, 28-08-2026):** both predicates (`GLOSS-DE-RESIDUE`, `AB-MUTATED`) now live in [`RussianTranslation/src/pwg_tm_gates.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_tm_gates.py) and fail the H2684 / 27-08 sidecar canaries (`upakrama`, `AtmasAt`, `taruRa`). Census of the frozen n=400 sample ([`sample400.jsonl`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/release/pwg_tm_canonical/wave1_b_receipt/sample400.jsonl)): **6 GLOSS-DE-RESIDUE** (`upakrama`, `AtmasAt`, `yad`, `yuj`, `sTA`, `aTa`) and **81 AB-MUTATED**. The AB class is not 81 copies of the `taruRa` videlicet collision — it is dominated by `recurring_formula` expansions that the formula table already emits (`<ab>demin.</ab>` → `<ab>уменьш.</ab>` on `etad`). House convention is still copy-through; those 81 now fail promotion instead of waiting for a judge. The dump is unchanged. **Implication:** Wave-2 promotion must re-run this gate; a paid n=400 re-score stays a human `@DECIDE`. The three remaining store SAN-LOSS rows (`mA` / `pat` / `asvatantra`) are **not** this class: H3590 typed them as *minor* head-line omission vs the two *medium* `dA` rows that H3593 requeued `--no-tm`; they stay parked for G5.

> **Source:** [reports/H3644_GAPS17_DEFECT_REPAIR_28-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/reports/H3644_GAPS17_DEFECT_REPAIR_28-08-2026.md)
> · [GAPS §17](https://github.com/gasyoun/SanskritLexicography/blob/master/GAPS.md) closed
> · [H3644](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3644-Grok_SanskritLexicography_pwg-ru-gaps17-defect-repair_28.08.26.md)
> (Grok 4.6 `grok-4.6`). — SanskritLexicography · 2026-08-28


### §602. A fail-closed COST guard tests telemetry, not permission — so human spend authorization cannot start a paid window on an unpriced lane

🔴 **When a lane returns no usage telemetry, `bounded_supervisor` refuses to run at all, and no amount of operator authorization changes that — the guard is not asking whether it *may* spend, it is refusing to continue on a window it cannot *price*.** **Evidence (H3659, 29-08-2026):** a c1 live gate returned a clean `LIVE_GO` (health measured wall 23 252 ms < 80 000, route 7 296 < 45 000, schema-valid; canary `dq_canary_puregloss` 3/3 senses, 0 TNMASK, 0 SAN-LOSS), the `no_pwg_w09` lease was prepared and preflighted clean (`over_ceiling: false`, 5 cards, est \$0.15), a human had explicitly authorized one bounded window under unknown billing — and the run still ended `stop_reason: cost_unevaluable` with `windows_done: 0`, `clean_total: 0` and **zero generation calls**. [`bounded_supervisor.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/bounded_supervisor.py) checks `if self.budget_cap is not None and not self._durable_cost_evaluable` at the **top of the drain loop, before pulling any work**; `--cost-ceiling` sets `budget_cap`, the two Max-route fleet-probe calls landed in the reservation ledger unpriced, and the first iteration stopped. **AMENDED 29-08-2026 (same handoff, after the window actually ran):** the *cause* stated here was overclaimed. The CLI **does** report per-call cost — the raw envelope of a refused call in the H3659 window carries `"total_cost_usd": 0.30507280000000003` with a full `cache_creation.ephemeral_1h_input_tokens` breakdown, captured by the `write_failed_envelope` that [PR #1837](https://github.com/gasyoun/SanskritLexicography/pull/1837) shipped. [`gateway_route.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/gateway_route.py) sets `cost_evaluable = True` whenever `total_cost_usd` is a number, then **rule 3 deliberately demotes it to `False` on a content failure** so the ledger never prices a failed call. So a `cost_evaluable: false` window is often that demotion working as designed, **not** absent telemetry. What stands is the MECHANISM — `budget_cap` + `not _durable_cost_evaluable` stops the loop before any work is pulled, and no human authorization clears it. What does not stand is "this lane has no cost telemetry"; why the probe-only path is unevaluable is a separate, answerable question. **A second gate the original text missed:** `--allow-unbounded` is necessary but **not sufficient** — `no_pwg_scale_plan.py` emits a **v1** manifest and production requires v2, so a planner-prepared lease can never run no matter what the cost ceiling says (see §604). **Implication:** while Max-route credit accounting is dormant, a cost-bounded window on this lane is not merely unpriced, it is **unrunnable** — the same dormancy that closed [H2591](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H2591-Opus_SanskritLexicography_pwg-flash-prep-claude-bounded-context-compare_12.08.26.md) ❌ INCONCLUSIVE, expressed as a refusal *before* the fact rather than an unreadable receipt *after* it. A handoff whose only stated blocker is "a human must authorize the spend" is therefore mis-specified on this lane: name the telemetry gate as a second, independent precondition. `--allow-unbounded` (H2157) is the disclosed escape — it leaves `budget_cap = None` while `--max-calls`/`--max-windows` still bind — but swapping it in for an authorization that said *bounded* is the guard-tunnelling defect of H2851, so it is a human's call, not the agent's.

> **Source:** [pwg_ru/h3659/H3659_NO_PWG_W09_WINDOW_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3659/H3659_NO_PWG_W09_WINDOW_29-08-2026.md)
...
> (Opus 5 `claude-opus-5`). — SanskritLexicography · 2026-08-29

### §603. The paid-run entry points resolve on-disk state the worktree cannot see — one refuses, one collides silently, one invents an empty roster

🔴 **§600 found three store resolvers disagreeing; the same split runs through the *paid-run* entry points, and there the failure modes are not all benign.** **Evidence (H3659, 29-08-2026), running from a linked worktree:** (1) [`h963_c4_gate0_probe.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/h963_c4_gate0_probe.py) **refused before any call** — its append-only evidence series is checkout-relative and would die with the worktree (#1034); nothing spent, no attempt consumed; fixed by `PWG_EVIDENCE_DIR` pointing outside every checkout. (2) [`no_pwg_scale_plan.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/no_pwg_scale_plan.py) derives its window index by globbing the **gitignored** `output/` dir: the main tree has used `1, 02–05, 08`, the fresh worktree saw none, and the planner proposed **`no_pwg_w02`** — silently re-running a completed window. Only an explicit `--start-index 9` avoided it. (3) [`bounded_staged_run.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/bounded_staged_run.py) reads the validated-account roster from `max_orchestrator.sqlite` resolved against **cwd**, so it died `--only-profile is not a validated roster slot` *and silently created an empty schema-only DB at the worktree root*; the real roster is `src/pilot/max_orchestrator.sqlite` (one row: `c1`, `validated=1`). Meanwhile the store figure in the same run was **correct** (121 remaining headwords), because `no_pwg_scale_plan` does route the store through `store_path.canonical_store()`. **Implication:** the canonical-resolution pattern exists and is applied to exactly one artifact. Port the window-index glob and the sqlite roster to it, and until then pass `--start-index` and `--db` explicitly from any non-main checkout. **Separately:** `src/pilot/max_orchestrator.sqlite` is **untracked *and* un-gitignored** in a guarded main tree — it is the only record of which profile slots may bind a paid run, git can neither warn nor restore it, and one `git clean -fd` leaves the lane unable to start a window until it is rebuilt by hand.

> **Source:** [pwg_ru/h3659/H3659_NO_PWG_W09_WINDOW_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3659/H3659_NO_PWG_W09_WINDOW_29-08-2026.md)
> · [H3659](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3659-Opus_SanskritLexicography_h3157-residual-c1-paid-no-pwg-window_28.08.26.md)
> (Opus 5 `claude-opus-5`) · same class as §592 / §600. — SanskritLexicography · 2026-08-29

### §604. `german_anchor` reports 0 repairs on a window that DID drop a span — the `{#…#}` null class was assumed closed, never closed

🔴 **The first paid window run against the span-drop class since the Tier-1 fix shipped returns the class ALIVE, and the repair counter at zero — which means the repair never engaged, not that nothing dropped.** **Evidence (H3659, 29-08-2026, `no_pwg_w09` on c1, 8 priced calls):** of 5 eligible sub-cards, `darv_i` and `gl_ana` produced cards, **`hasita~~h0_zz_pw` returned `translation-fidelity-reject`** — the exact class signature, on current code, after [`german_anchor.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/german_anchor.py) shipped ([PR #725](https://github.com/gasyoun/SanskritLexicography/pull/725) / [v1.61.0](https://github.com/gasyoun/SanskritLexicography/releases/tag/v1.61.0)) — and `jaw_ayus` / `kast_ur_i` were never attempted (`budget_exceeded:max_calls`). `hasita` is one of the six keys naming the class in [RUN_LOG](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/RUN_LOG.md) line 937. **Predicate #2 (ruled 19-08-2026) therefore FAILS**: absence needs every key clean, presence needs one, and one is confirmed — the two unrun keys cannot rescue it. **The load-bearing number is `german_anchor_repairs: 0` with `german_anchor_detail: []` on that same window.** H3144's acceptance ANDed "class gone" with `repairs > 0` on the theory that `0` means *no card dropped a span*; here a card **did** drop one and the counter stayed `0`, so the repair is not idle-for-lack-of-work — **it is not reached on this failure path at all**, the card being rejected at `translation-fidelity-reject` before it fires. **Implication:** the class was never closed, only assumed closed by the presence of a shipped fix; a repair whose counter can read `0` both when there is nothing to repair *and* when it never ran is unfalsifiable as shipped, and needs a third state (`not_reached`) before any future window is spent on it. The next step is offline and unpaid — exercise the repair against `hasita`'s captured output. **Two more findings from the same window:** the §498 Plan-Mode refusal class returned in a NEW form on `gl_ana` (the model completes the task and cannot *emit* it — "`must have required property 'cards'` … tied to Plan Mode being active … which appears to substitute a different expected schema"), classified correctly first try by PR #1837's refusal/`malformed_output` split; and `write_failed_envelope`, the artifact that preserved that evidence, writes **checkout-relative**, so in a worktree it lands in the gitignored `output/` and dies with the tree (§603's trap hitting the very fix that makes diagnosis possible — derive its path from the resolved evidence root, as `_write_probe_raw` already does). Budget note: `translate_agents_spent 5 + heal_agents_spent 3 = --max-calls 8`, so a no_pwg window needs **≥ ~2.5 × card count**, not 1.6 ×.

> **Source:** [pwg_ru/h3659/H3659_NO_PWG_W09_RESULT_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3659/H3659_NO_PWG_W09_RESULT_29-08-2026.md)
> · [H3659](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3659-Opus_SanskritLexicography_h3157-residual-c1-paid-no-pwg-window_28.08.26.md)
> (Opus 5 `claude-opus-5`) · amends §602, extends §603, revives §498. — SanskritLexicography · 2026-08-29

### §610. A dropped span can be re-anchored in the TRANSLATION without parsing prose — the same sense's `german` echo is a surviving parallel, and it carries the position

🔴 **§608 left one fork open: the target-side half of the span-drop class had no repair, and the choice was between building one and ruling that such cards requeue. MG ruled `reanchor` (29-08-2026). The build turns on one observation the earlier handoffs never made — the evidence needed to place the span is already in the card.**

**Why this is not the [PR #789](https://github.com/gasyoun/SanskritLexicography/pull/789) refusal, which stands.** PR #789 declines to reconstruct a `{%…%}` gloss boundary the model merely omitted: there the target is prose, the span has no marker left in it, and finding its edges means guessing inside a translated sentence. A `{Tn}` token is **opaque and atomic** — there is no boundary to find, only a position — and the **same sense's `german` field still carries every token in order**, guaranteed, because reaching this guard at all requires the german-only count to have already matched (§605). Senses pair 1:1 by card schema, so there is no alignment step to get wrong. The translation is never read; a token is restored to the position its own parallel names. **Present evidence, not inference** — that is the whole distinction, and it is what makes a repair legitimate here and illegitimate there.

**The rule, and where it beats its own sibling.** Per sense, each missing token is re-injected next to its nearest surviving neighbour measured in that sense's german, with the **sense start AND sense end as virtual anchors** when nothing survives on that side. All three cases are load-bearing: a headword span at offset 0 goes to the head (`{T1} Feuer {T2}` / `огонь {T2}` → `{T1} огонь {T2}`); a leading-but-not-first span has prose ahead of it and anchors before its successor (`b {T3} {T4}` / `б {T4}` → `б {T3} {T4}`, never `{T3} б {T4}`); a trailing span goes to the tail (`{T1} unverwirrt {T2}` / `{T1} невозмутимый` → `{T1} невозмутимый {T2}`). `german_anchor` takes an unconditional head branch instead, and correctly so: it anchors against the card-wide SOURCE skeleton, whose first span really is the headword and which has no per-sense end to measure to. **A per-sense parallel offers both edges; using them is what keeps a citation on the right side of its own prose.**

**Measured.** `hasita~~h0_zz_pw` — requeued as unfixable across H858 → H3144 → H3157 → H3659, and the card a paid window was spent on — is **repaired offline**: `target_anchor.reanchor` re-injects `{T7}` (`<ls>GAUT.</ls>`) at the end of sense 2, exactly where the german parallel puts it, and every sense's token sequence then equals its anchor. `german_anchor` on the same card still returns `nothing-missing`.

**A trap under both lanes, found while wiring it.** `restore_card` / `restoreCard` unmask **in place**. The german branch was restoring `masked` itself, consuming the very `{Tn}` tokens a target repair needs as its anchor; both lanes now restore a **copy**. And because the target repair re-restores, a german repair already applied to the card must be **re-stamped** or `german_anchor`'s provenance vanishes silently on a card dropped on both sides. Pinned in both lanes.

**What the store may receive is unchanged** — only cards that count exactly right on both sides. The remedy changed, the guarantee did not. A target echo that is not a pure order-preserving drop still rejects, now as `translation-fidelity-reject: target-anchor <reason>`; the prefix is preserved deliberately so every existing reader still matches. Registered `target_anchor_repair_h3675`, verdict **SHARED**: the target field is a parameter, never a literal, and `test_h3675_target_anchor_selftest` asserts that mechanically rather than trusting it. **Nothing is promoted here** — re-running the window is paid, and §602's cost-telemetry gate and §604's v2-manifest gate both still stand in front of it.

> **Source:** [pwg_ru/h3675/H3675_TARGET_ANCHOR_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3675/H3675_TARGET_ANCHOR_29-08-2026.md)
> · [`src/target_anchor.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/target_anchor.py)
> · [H3675](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3675-Opus_SanskritLexicography_target-side-span-drop-reanchor_29.08.26.md)
> (Opus 5 `claude-opus-5`) · **closes §608's open fork**; zero paid calls. — SanskritLexicography · 2026-08-29

### §613. A planner that feeds production must pass the SAME binding as the coordinator — `no_pwg_scale_plan` stamped a v1 manifest production refuses, so every lease it prepared was unrunnable

🔴 **`gen_opt_harness2` stamps `pwg.headless_execution_manifest.v2` only when it is given a profile binding; unbound it stamps v1, and `bounded_staged_run` refuses v1 in production. [`no_pwg_scale_plan.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/no_pwg_scale_plan.py) never passed that binding, so for its whole life every window it prepared was born unrunnable — and nothing said so until a session tried to spend.** **Evidence (H3677, 29-08-2026):** `prepare_window` built its `gen_cmd` with `--nominal`, `--keys`, `--output-budget`, `--out`, `--refuse-oversize` and `--manifest-out`, while [`coordinator.prepare`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/coordinator.py) appends five more for the same generator — `--profile-slot`, `--config-dir`, `--execution-route=claude-cli-headless`, `--executor-lane`, `--validation-method`. `gen_opt_harness2` computes `bound = bool(profile_slot or config_dir)` and selects `SCHEMA_V2 if bound else …v1`, so the planner's `bound` was always false. H3659 hit the refusal live — `production requires pwg.headless_execution_manifest.v2 (got 'pwg.headless_execution_manifest.v1')` — and rebuilt the manifest **by hand** with the coordinator's exact flags to get its window; the planner itself was left unfixed, so the next session would have paid the same tax. **The failure is silent in the worst place.** Preparation prints `wrote … | execution manifest pwg.headless_execution_manifest.v1` and exits 0; the lease registers, the preflight cost gate passes, the harness is generated, and the artifact looks complete. The refusal arrives only at `bounded_staged_run`, i.e. at the moment a human has already cleared billing authorization and the cost guard — the two expensive gates — so the cheap mechanical gate is discovered last. **Fix:** the planner builds the identical list in `binding_args()` and forwards it, `coordinator.register_prepared_lease` records `profile_slot`/`config_dir`/`executor_lane` the way `prepare` already did, and `--profile-slot`/`--config-dir` are refused unless supplied together (the coordinator's own rule). Verified end-to-end, not asserted: the patched planner emits v2 carrying `config_dir_fingerprint 9321e2c138f0…`, **byte-identical to the binding H3659 hand-built** for `no_pwg_w09`. `windows100_selftest._test_planner_binding` checks the flag list against `coordinator.py`'s **source** rather than a copied literal, so the two paths cannot drift apart again — the copied-constant failure mode H2254 records one subsystem over. **The general rule:** when two entry points feed the same generator and only one of them is production-eligible, the eligibility difference is invisible until spend time. Either both pass the binding or the generator refuses to emit an artifact that names no route — a v1 manifest with a `--manifest-out` path is a foot-gun with a receipt. **Ordering note for anyone planning a paid window:** of the three gates in front of a `no_pwg` run — human billing authorization, the fail-closed cost guard (§602), and manifest schema — only this third one is an agent's to clear, and it is the one that was still open.

> **Source:** [pwg_ru/h3677/H3677_NO_PWG_PLANNER_V2_BINDING_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3677/H3677_NO_PWG_PLANNER_V2_BINDING_29-08-2026.md)
> · [H3677](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3677-Opus_SanskritLexicography_no-pwg-window-rerun-after-target-anchor_29.08.26.md)
> (Opus 5 `claude-opus-5`) · closes gate 3 of §602 / §604. Numbered §613 after landing concurrently with §611 (H3663) and §612 — three sessions appended on 29-08-2026 from branches cut before each other's merge, so the number was taken between this branch point and its merge. — SanskritLexicography · 2026-08-29

### §615. The span-drop class is CLOSED on a real window — and the target-side repair's stated precondition was false on every card of it

🔴 **`hasita~~h0_zz_pw`, requeued as unfixable across H858 / H3144 / H3157 / H3659, came back a clean card on a paid window — and the same window shows the justification H3675 shipped the repair on does not describe what actually happens.** **Evidence (H3677, 29-08-2026, `no_pwg_w10` on c1, 5 calls):** all five sub-cards of the class returned cards on the **first attempt** — `null_keys: []`, zero retries, zero heals, zero refusals — against H3659's 8 calls that produced 3 verdicts and one `translation-fidelity-reject`. `target_anchor_invocations: 5`, `target_anchor_repairs: 5`, every outcome `repaired`; audit gives **4 clean, 1 defect** (`kast_ur_i`). The repair is semantically right, not merely count-matching: `hasita` sense 2 carries `<ls>GAUT.</ls>` at the **tail** of the Russian, exactly where the German parallel puts it — the trailing-citation placement `german_anchor` structurally cannot do, which is why H3675 built a per-sense repair with the sense END as a virtual anchor. **Because `TARGET_ANCHOR_INVOCATIONS` increments BEFORE the attempt, this number is falsifiable** — `1 invocation / 0 repairs` would have read "reached and refused", the third state §604 demanded after H3659's unfalsifiable `0`. **The correction.** H3675 justified the target-side repair with *"the SAME sense's german still carries every token in order (guaranteed by the guard's own precondition)"* — the target branch being reachable only once the german-only count has passed. **That precondition held on ZERO of the five cards.** `german_anchor_invocations: 5`, `german_anchor_repairs: 5`, `german_anchor_not_reached: []`: the german echo was itself short on every card and was machine-repaired first, after which `target_anchor` anchored against the repaired german. The outcome is still sound, but for a different reason than the one on record — `german_anchor` reinjects from the **source skeleton** and verifies against the same counts, so the anchor is source-derived whether or not the model emitted it faithfully. What is false is that the german field's fidelity comes from the emission. **Both repairs reinjected the IDENTICAL token set on every key** (`darv_i` T1,T2,T3,T6,T7,T8 · `gl_ana` T1 · `hasita` T1,T8,T11 · `jaw_ayus` T1,T2,T3 · `kast_ur_i` T1,T2), and **every** key dropped `{T1}`, the masked headword+grammar token — so the drop is a property of the CARD, not of one field, and a repair that treats the two fields as independent is modelling it wrong even while it produces the right answer. **A second correction, to the offline replay.** H3675 reconstructed `hasita` as dropping `{T7}` and repaired that; the live card dropped **T1, T8, T11**. The reconstruction was explicitly "pinned by control flow", never observed. An offline replay can prove a repair WORKS; it cannot tell you what the repair will be asked to fix, so never carry a replay's input shape forward as a fact about production.

> **Source:** [pwg_ru/h3677/H3677_NO_PWG_W10_RESULT_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3677/H3677_NO_PWG_W10_RESULT_29-08-2026.md)
> · [H3677](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3677-Opus_SanskritLexicography_no-pwg-window-rerun-after-target-anchor_29.08.26.md)
> (Opus 5 `claude-opus-5`) · answers §604 positively, amends §610. — SanskritLexicography · 2026-08-29

### §608. A `translation-fidelity-reject` discards the card, and `german_anchor` would refuse it anyway — the counter now says `not_reached` instead of `0`

🔴 **§605 established that the repair cannot fire on this failure path. Two things it could not know, both found offline for zero paid calls: the rejected card is never persisted, and the repair would decline it even if reached.**

**(a) The artefact is thrown away.** [`normalize_batch`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/headless_worker.py) sets `card = None` on `translation-fidelity-reject`, and `write_failed_envelope` fires only on a **process-level** failure — which `hasita~~h0_zz_pw`'s batch was not (`returncode 0`). So the H3659 evidence root holds the verdict and never the card, and §604's prescribed "replay the captured output" was **not runnable as written**. A content-reject discards the only evidence that could diagnose it; capturing rejected cards under the resolved evidence root is open work.

**(b) The repair would refuse it.** The error string still pins the card by control flow — reaching the target-field guard requires the german-only count to have already matched, so `hasita`'s german echo was exactly `<ls> 2/2, {# 2/2` and only `russian` was short. Rebuilding that proven state from the manifest skeleton and running the real module ([`replay_hasita_german_anchor.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3665/replay_hasita_german_anchor.py)) returns **`german_anchor.plan -> ok=False, reason='nothing-missing'`**: every source span *is* present in `german`. So this is **not** a wiring defect that a moved call site would fix — §605's "cannot fire" becomes **"cannot fire, and would decline if it did"**. A future session that tries to close the class by re-gating the repair is chasing nothing.

**(c) The counter, against §605's objection.** §605 argued a `not_reached` state "would be permanently true here and still carry no information". Its code-shape half is right and is confirmed; this half is not. `german_anchor_repairs` is read from a **summary**, by a session that does not have `count_card`'s and `german_anchor`'s docstrings in front of it — which is exactly how §604 came to be written, at the cost of a paid window. Both lanes now emit `german_anchor_invocations` and `german_anchor_not_reached` (headless also `german_anchor_outcomes`: `repaired` · `verify-failed` · `refused:<reason>` · `repaired-then-rejected` · `not-reached`), so the three states are three distinct summaries: clean = `0/0/[]`, german-side drop = `1/1/[]`, target-side drop = `0/0/[key]`. Before this, rows 1 and 3 were byte-identical on every `german_anchor` field. `repaired-then-rejected` closes a fourth hole found while wiring it: a card the repair *did* fix but a later guard nulled loses its stamp with `card = None`, so the stamp-derived `german_anchor_repairs` forgets the repair happened. Pinned by `headless_worker_selftest.test_h3665_german_anchor_counter_is_falsifiable` — which asserts the **invariant**, not just three fixtures: any key failing `translation-fidelity-reject`/`unmapped-token-reject` must appear in `not_reached` or carry an outcome — and by `german_anchor_test.js` CONTROL 2b/2c through the real emitted harness.

**Still not fixed, deliberately:** the target-side span drop has no repair at all (§605 consequence 3). This changes only what the telemetry admits, never the reject.

> **Source:** [pwg_ru/h3665/H3665_GERMAN_ANCHOR_NOT_REACHED_29-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3665/H3665_GERMAN_ANCHOR_NOT_REACHED_29-08-2026.md)
> · [H3665](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3665-Opus_SanskritLexicography_german-anchor-never-reached-span-drop-repair_29.08.26.md)
> (Opus 5 `claude-opus-5`) · **extends §605, partly dissents from its consequence (1)**; zero paid calls. — SanskritLexicography · 2026-08-29

### §605. `german_anchor` CANNOT fire on a `translation-fidelity-reject` card — the two are mutually exclusive branches, so §604's `0` was the only value possible

🔴 **§604 asked why the repair "is not reached" on the span-drop null class and prescribed exercising it offline against `hasita`'s captured output. It can never be reached there, and that exercise would prove nothing — the answer is a code-shape fact, not a measurement.** The gate that invokes the repair and the gate that raises `translation-fidelity-reject` count **different things**, and the second is reachable ONLY when the first has already passed. In [`headless_worker.normalize_batch`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/headless_worker.py#L863) the repair branch is guarded by `count_card(card, '<ls') != inp['ls'] or count_card(card, '{#') != inp['sk']`, and [`count_card`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/headless_worker.py#L798) is german-only *by deliberate design* — its own docstring: *"DELIBERATELY still `german`-only … this count is compared against `inp['ls']`/`inp['sk']`, which are SOURCE-occurrence counts"*. Only when that count **matches** does control fall through to the second gate, [`count_card_field(card, field, …)`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/headless_worker.py#L818), whose docstring states the complement outright: *"count_card above proves only that the `german` SOURCE echo is faithful. A `{#..#}`/`<ls>` span can be dropped from the translation field alone (german 33/33, english 32/33 — the live H1070 r102 pattern) **with zero effect on the german count**"* — and that is what raises `error = 'translation-fidelity-reject'` (H1152 parity C1). **The two rejects are therefore disjoint by construction:** `fidelity-reject: german-anchor …` means the GERMAN echo dropped a span (repairable — re-inject it from the source skeleton); `translation-fidelity-reject` means the German echo was faithful and the RUSSIAN target dropped it (nothing for an anchor to re-inject — the source span is present and correct; the translation simply omits it). `hasita~~h0_zz_pw` returned the *second* string, so by the code path alone its drop is proven target-side. **Consequences.** (1) `german_anchor_repairs: 0` on `no_pwg_w09` is neither an unfalsifiable counter nor a missed repair path — for a window whose only span-drop was target-side, `0` is the sole possible value and is correct on both readings; §604's proposed third state `not_reached` would be permanently true here and still carry no information. (2) The real error is upstream, in the predicate itself: H3144 treated `german_anchor` ([PR #725](https://github.com/gasyoun/SanskritLexicography/pull/725)) as the fix for "the `{#…#}`-span-drop null class", but that fix addresses only the SOURCE-echo half of the class. The half that actually appears on `hasita` is target-side and **has never had a fix at all** — predicate #2 was testing a repair against a failure mode it was never built to catch, which is why it could only ever fail. (3) The remaining work is net-new, not diagnostic: either a target-side re-anchor (hard — re-inserting a `{#…#}` span at an unknown position inside a translated sense is exactly the boundary-guessing that [PR #789](https://github.com/gasyoun/SanskritLexicography/pull/789) refuses as a standing ruling) or an explicit ruling that target-side drops requeue rather than repair. **No paid window can inform any of this** — it was established by reading two docstrings, and a further window would re-buy the same negative.

> **Source:** [`headless_worker.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/headless_worker.py) `count_card` / `count_card_field` / `normalize_batch`
> · [H3659](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3659-Opus_SanskritLexicography_h3157-residual-c1-paid-no-pwg-window_28.08.26.md)
> (Opus 5 `claude-opus-5`) · **amends §604 and retires its prescribed offline exercise**; bears on §602's predicate framing. — SanskritLexicography · 2026-08-29

### §606. `--permission-mode plan` is used as a SANDBOX on the paid lane — that misuse, not a schema bug, is what breaks structured output, and `--tools ""` is the primitive the lane actually wants

⚠️ **PARTLY REFUTED the same day by §607 — read that first.** The causal half of this finding (that plan mode is what breaks structured output) is **struck**: four direct canary calls, including the real production schema under `--permission-mode plan`, all returned a valid `structured_output`. What still stands is the design point only — plan mode is used here as a sandbox rather than for planning, and `--tools ""` is the primitive for that posture. The mechanism paragraph below was labelled a hypothesis when written; it is now a *refuted* one, and the model self-diagnosis it rests on is the subject of §607.

🔴 **The paid spawn asks for plan mode to get a permission posture, and keeps paying for plan mode's actual semantics.** [`headless_worker.HeadlessWorker.call`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/headless_worker.py#L1172) spawns `-p --output-format json --json-schema <S> --model <M> --permission-mode plan`, and the reason on record is explicitly not planning — [`h2189_min_profile`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/h2189_min_profile.py) writes an EMPTY `settings.json` and notes that "`--permission-mode plan` supplies the only permission posture this lane needs". But plan mode's real contract is *propose a plan, then call `ExitPlanMode` for approval*, and this lane never wants a plan. **The bill so far is two separate failures six weeks apart, both charged at full window price, both rediscovered from scratch.** §498 (19-08-2026): the model **refused the task** on plan-mode grounds — repaired by reframing the PROMPT (option B / the `=== TASK SHAPE ===` preamble) while *deliberately keeping* plan mode. §604 (29-08-2026, `no_pwg_w09`): the model **completed the task and could not emit it** — `b1` died `error_max_structured_output_retries` after 5 attempts (`total_cost_usd` 0.2372) and `b1.retry1` came back `refusal` (0.3051) with the model naming the cause in prose: *"it fails with the same schema error as before (`must have required property 'cards'` / `must NOT have additional properties`) even on a minimal test object — … a tool-validation issue tied to Plan Mode being active in this session (which appears to substitute a different expected schema). I don't have a way to exit Plan Mode (no `ExitPlanMode` tool is exposed here)."* **The prompt is exonerated, which is what makes this structural.** The manifest prompt never names `StructuredOutput` or `structured_output` at all (0 occurrences), defers wholly to `--json-schema`, and its single shape instruction — *"return one object per headword in `cards`"* — agrees exactly with the schema's `required: ["cards"]`. Option B could buy the model's *willingness*; no prompt edit can change which tool the CLI registers, so §498's repair was always going to leave this half open. **The validator signature says the wrong object is being checked:** `must have required property 'cards'` AND `must NOT have additional properties` firing *together*, on a minimal object, is not a content error — it is a schema being applied to an envelope that is not the one the model submitted. **Mechanism — hypothesis, explicitly NOT proven:** in plan mode the structured emission is routed through the plan-submission path, whose wrapper is then validated against `--json-schema`, producing both errors at once. The CLI is a 217 MB native binary with no readable source, so nothing inside it can be confirmed from here; only the vendor or a canary can settle it, and this finding does not claim more. **What IS provable, and is the fix:** `claude --help` on CLI **2.1.251** documents `--tools <tools...>` — *"Use `""` to disable all tools"*. That is the designed primitive for the posture this lane wanted all along, it is **stronger** containment than plan mode (which still exposes read tools), and it carries no planning semantics whatsoever. The change is one line at [`headless_worker.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/headless_worker.py#L1172): drop `--permission-mode plan`, add `--tools ""`. **Verification owed before any window relies on it:** ONE canary call with `--tools ""` against this same schema — the only open question is whether the `--json-schema`-synthesized `StructuredOutput` survives `--tools ""`. It is not in the CLI's declared built-in set (`sdk-tools.d.ts` has 0 occurrences of `StructuredOutput`, while `ExitPlanMode` is declared there), which is the reason to expect it does survive — and the reason to check rather than assume. **Transferable rule:** *a flag borrowed for a side effect keeps its primary semantics, and those semantics keep charging rent.* Plan mode was adopted because it happened to suppress actions; it also suppresses emission — twice now, each time diagnosed only after a paid window died. When a flag is chosen for what it incidentally prevents rather than for what it means, write down the mechanism you actually wanted and check whether a primitive for it already exists.

> **Source:** preserved failure envelopes `failed_envelope_b1.txt` / `b1.retry1.txt` (durable evidence root, §603) · [`headless_worker.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/headless_worker.py) · `claude --help` on 2.1.251
> · [H3659](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3659-Opus_SanskritLexicography_h3157-residual-c1-paid-no-pwg-window_28.08.26.md)
> (Opus 5 `claude-opus-5`) · answers §604's owed item 2; closes the half §498's option-B repair could not reach. — SanskritLexicography · 2026-08-29

### §607. A model's stated cause for its OWN failure is a hypothesis, not evidence — the paid lane's "Plan Mode broke StructuredOutput" self-diagnosis is refuted by four canary calls under one dollar

🔴 **§604 recorded, and §606 then built on, the model’s own prose explanation for why `gl_ana` could not emit** — *"a tool-validation issue tied to Plan Mode being active in this session (which appears to substitute a different expected schema)"*. **Four direct canary calls on c1 refute it.** Each spawned the production CLI (2.1.251) as `-p --output-format json --json-schema <S> --model claude-sonnet-5`, varying only the containment flag and the schema, against a ~300 B prompt:

| arm | schema | flag | `structured_output` | turns | cost |
|---|---|---|---|---|---|
| A | toy (`$defs`+`$ref`, `required:["cards"]`) | `--permission-mode plan` | **PRESENT** | 2 | $0.336 |
| B | toy | `--tools ""` | **PRESENT** | 2 | $0.190 |
| C | **the real `w09v2` `output_schema`, verbatim** | `--permission-mode plan` | **PRESENT** | 2 | $0.232 |
| D | the real schema | `--tools ""` | **PRESENT** | 3 | $0.205 |

**Arm C is decisive:** the exact production schema (3 `$defs` — card/record/sense — nested arrays, enums, descriptions, `additionalProperties:false`, 1 573 B) under the exact production flag returned a well-formed `{"cards":[…]}` in two turns. Plan mode does not substitute a schema and does not break structured output; the schema shape is not the problem either. §606’s causal headline is struck. What survives it is only the design point: plan mode is used as a sandbox rather than for planning, and arms B/D confirm `--tools ""` works and is strictly stronger containment — a reasonable change, but **not** a fix for this failure, because there is now no evidence this failure is about the permission mode at all. **Round 3 — scale was the next named mechanism, and it fell too.** Two corrections to this finding as first written. (1) *Batch size was never 5.* `manifest["batches"]` is `[[darv_i],[gl_ana],[hasita],[jaw_ayus],[kast_ur_i]]` — **one key per batch** — so `b1` was a single-card call, not a 5-card one; the 14.5 KB figure was the whole `prompt` dict, not what was sent. (2) *Prompt scale does not reproduce the failure.* Feeding the pipeline's own `build_prompt_joined(manifest, ['gl_ana~~h0_zz_pw'])` — a byte-identical reconstruction of the production prompt, **15 836 B** — with the real schema:

| arm | prompt | flag | `structured_output` | turns | cache-read | cost |
|---|---|---|---|---|---|---|
| E | **the real `gl_ana` prompt, byte-identical** | `--permission-mode plan` | **PRESENT** | 3 | 114 748 | $0.335 |
| F | the real `gl_ana` prompt | `--tools ""` | **PRESENT** | 3 | 64 284 | $0.240 |

**Arm E is the exact production combination — same prompt bytes, same schema, same flag — and it passed.** So the failure does **not** reproduce, and scale joins plan mode and schema shape on the refuted list. What remains is a bare behavioural difference with no established cause: the two failing calls each ran **10 turns** (burning 5 structured-output retries, then refusing) where every passing canary ran **2–3**. One environmental difference is on record and is offered as an observation, not a claim: the CLI package was rewritten on disk at 07:25Z, about eleven minutes *after* the failing calls, so those ran against a different installation state than these canaries did. **The honest verdict is transient-until-shown-otherwise** — and that should have been the null hypothesis from the first envelope, because a single unreproduced failure is evidence of a failure, not of a mechanism. **Transferable rule:** *a model’s explanation of its own failure is generated from the same context that produced the failure, and it will confidently name a plausible mechanism it has no way to observe.* Treat it as a lead to test, never as a cause to record. Here a mechanism was named three times running — Plan Mode by the model in §604, plan-mode-as-sandbox by §606, prompt scale by this finding’s own first draft — and **six canary calls totalling $1.538** refuted all three. Each was plausible, each was written down before the experiment that could kill it, and each was wrong. The discriminating experiment was available the whole time and cost less than a thousandth of the window that produced the puzzle; **when a finding turns on a mechanism, price the experiment that would refute it before writing the finding down.**

> **Source:** four canary spawns on c1, 29-08-2026, against the CLI at 2.1.251 and the `w09v2` manifest’s own `output_schema` · [`headless_worker.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/headless_worker.py)
> · [H3659](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3659-Opus_SanskritLexicography_h3157-residual-c1-paid-no-pwg-window_28.08.26.md)
> (Opus 5 `claude-opus-5`) · **refutes the causal half of §606 and the mechanism §604 attributed to the model’s prose.** — SanskritLexicography · 2026-08-29

### §609. `classify_run` cannot tell a broken harness from a budget you set too small — `code-failure` is its fall-through, so an undersized `--max-calls` sends the next session hunting a phantom gate defect

🔴 **`no_pwg_w09` classifies `code-failure`, and two of its three "null cards" are simply keys the run never reached.** Running [`classify_run.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/classify_run.py) on the window output returns *"3 null card(s), 0 partial card(s), tripped=True, with NO infra signal … suspect the harness itself: kill gate, heal budget, schema, or key matching"*. But the three nulls are not one class: `hasita~~h0_zz_pw` is a genuine `translation-fidelity-reject` (the §604/§605 null class), while `jaw_ayus` and `kast_ur_i` are `budget_exceeded:max_calls` — **never attempted at all**, because `translate_agents_spent 5 + heal_agents_spent 3` consumed a `--max-calls 8` ceiling set for five cards. Nothing in the harness misbehaved on those two; the operator picked 1.6× card count where a no_pwg window needs ≈ 2.5×. **The cause is structural, not a tuning slip.** `classify()` reads `nulls` from `null_keys` with no regard for *why* a card is null, sets `tripped = bool(summary["budget_stops"])` — 16 here — and then makes `code-failure` the **fall-through branch**: `clean` requires no nulls, no partials and an untripped switch; `infra-confounded` requires `conn_errors >= 1` or `kill_timeouts >= max(3, 25 %% of agents)`; *everything else* lands on `code-failure` with the fixed advice "suspect the harness itself". A window that did exactly what it was told — stop at the ceiling — is therefore indistinguishable from one whose gate logic is broken. **Why it costs real time:** [`/pwg-window-close`](https://github.com/gasyoun/claude-config/blob/main/commands/pwg-window-close.md) Phase 2 turns the verdict into an instruction — *"`code-failure` → a real gate defect; fix the code, don’t paper over it with a requeue"* — so the reader is told to go debug a harness that is fine, and is simultaneously blocked from the requeue that would actually finish the two unrun keys. This is the same shape as §498’s `refusal`-vs-`malformed_output` conflation: **a classifier that folds two causes into one verdict does not merely lose information, it actively misdirects the next session**, and the fix is the same — split the class. `budget_exceeded:*` nulls are mechanically separable from gate-flagged nulls in the same `failures` map the summary already carries, so a `budget-truncated` verdict (“the run was cut short by its own ceiling; raise it and re-run, the harness is not implicated”) needs no new telemetry. **Transferable rule:** *a fall-through branch in a classifier is a claim, and it asserts its claim most confidently exactly where it knows least.* When the terminal branch of a decision tree carries a directive — here "suspect the harness" — every unmodelled cause inherits that directive. Enumerate what can reach the fall-through before trusting what it says.

> **Source:** `python src/pilot/classify_run.py out.w09.json` against the H3659 durable evidence root · [`classify_run.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/classify_run.py) `classify()`
> · [H3659](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3659-Opus_SanskritLexicography_h3157-residual-c1-paid-no-pwg-window_28.08.26.md)
> (Opus 5 `claude-opus-5`) · kin §498 (same conflation shape); bears on §604’s budget note. — SanskritLexicography · 2026-08-29

### §611. The window audit's "units clean" line is NOT the promotable verdict, and `promote_final_cards.py --merge` is not dry-run by default

Two independent traps in the same promotion, both of which silently produce a *plausible* result.

**1. `audit_window.py` prints two verdicts that can disagree.** The line `PASS: 4/4 units clean` (and its `FLAGGED_JSON: []` twin) is the **markup-fidelity unit gate** — the `ls s/o` / `san s/o` / `ru` columns only. The audit's own `requeue.defect.keys.txt` is the **promotable** verdict, and it is what `promote_final_cards.py`'s defect guard actually loads. On H3663's five chunks the first said 15 of 16 clean; the second said **7 clean, 8 defect, 1 transient**. Reading the unit gate as the verdict would have staged eight defect cards for promotion, and the guard — had it been handed only one chunk's defect file — would have refused on a *different* pair of keys than the unit gate flagged, which is exactly the confusing signal that surfaced this. Read `requeue.defect.keys.txt` (plus `requeue.transient.keys.txt`), never the summary line. Per-chunk runs write **per-chunk** requeue dirs (`output/<window-tag>/`), so a multi-chunk window needs the **union** of them — one chunk's file is not the batch's block list.

Corollary worth keeping: `high-confidence risks: 0` on every chunk while 8 keys still landed in the defect bucket. Defect classification is not the same axis as high-confidence risk, so "no high-confidence risks" is not evidence that a batch is promotable.

**2. `--merge` writes the store; `--apply` does not gate it.** `--apply`'s help reads "with `--ready-partial-report`: actually write the clean subset (default is dry-run only)" — it gates *that report path*, not promotion. A plain `--merge` invocation without `--apply` **promotes for real**. H3654's recipe happened to pass `--apply`, so the asymmetry never surfaced there and every downstream reader inherits the assumption that `--apply` is the write switch. It is not. The safety that does hold: the promoter takes an automatic timestamped `.premerge.*.bak` before writing, and without `--override-reviewed` human-touched rows stay protected — so the H3663 accident was recoverable and content-correct, but it was still a write nobody intended.

Both are why the H3663 promotion narrowed its **input** — whole clean chunks copied verbatim, the mixed chunk rewritten without its flagged key — rather than reaching for `--force`, which bypasses the defect guard for **every** key in the batch, not just the one being excused.

### §612. The durable evidence root preserves a paid window’s OUTPUTS but not its INPUTS — once the session worktree dies, the window’s cards can never be audit-validated, so they can never be promoted

🔴 **Two clean cards from a window that cost 8 priced calls are permanently unpromotable, and no amount of unpaid work recovers them.** Attempting the narrow promote of `no_pwg_w09`’s `darv_i` + `gl_ana` on 29-08-2026, [`audit_window.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/audit_window.py) refuses outright: *"current raw/portrait inputs missing: `darv_i~~h0_zz_pw.raw.txt`, `darv_i~~h0_zz_pw.portrait.json`, …"* — all ten sidecars for all five keys — then *"raw hash mismatch … refusing collect/gates/glue"*. The sidecars were searched for in all three places they could survive and are in **none**: the main checkout’s `src/pilot/input/` (935 sidecars present, **zero** for these five keys), the still-extant H3659 session worktree, and the durable evidence root itself — which holds `out` · `status` · `calls` · `execution_manifest` · `preflight` · `canary_receipt` · `failed_envelopes`: **every output the run produced, and nothing it consumed.** The sidecars are written at plan time by `_pilot_gen_merged.py` into the checkout-relative, gitignored `src/pilot/input/`, so they are §603’s class exactly — but where §603 cost a *diagnostic* artifact that was recovered by hand, this costs the **promotion**, and it is not recoverable. **Every workaround is closed, and each for a principled reason.** (1) The execution manifest does preserve `inputs[key].skeleton` and `inputs[key].portrait` — but as *strings*, while the audit compares a recorded hash against on-disk **files**, and `skeleton` is the masked, rule-prefixed generation form rather than `.raw.txt`; materialising a guess would write wrong sidecars into a directory 935 other windows share. (2) `--allow-stale` is documented as *"forensic mode: continue even if workflow/rootmap/input provenance is stale"* — an inspection switch, not a promotion path; using it to clear this gate is the H2851 guard-tunnelling shape, and the standing evidence rule is that missing evidence is INCONCLUSIVE, never PASS. (3) Re-running [`no_pwg_scale_plan.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/no_pwg_scale_plan.py) does regenerate sidecars, but it re-enters window **planning** — which §603 records as resolving on-disk state a worktree cannot see — and cannot guarantee byte-identity with what the paid window actually consumed, which is the only thing the hash check would accept. **The fix is small and mechanical:** capture the input sidecars into the durable evidence root alongside the outputs — `input/<key>.raw.txt` + `<key>.portrait.json` for every `selected_key`, resolved from the evidence root exactly as §604 already prescribes for `write_failed_envelope`. That is two small files per key, copied once at manifest-build time, and it makes every future window promotable after its worktree dies. Until then "durable evidence root" is a misnomer: it durably preserves what the run **produced** and silently drops what the run **consumed** — and it is the consumed half the promotion gate actually reads. **Transferable rule:** *an evidence policy written from the failure you last debugged captures outputs, because outputs are what you read when something looks wrong.* The gate that blocks you months later asks for inputs. Enumerate what every downstream **gate** reads, not what the last incident made you wish you had kept.
...
**Amendment (same day, after the fix landed): the sidecars are HALF-recoverable, and what that reveals is worse than losing them.** Regenerating all five keys through `_pilot_gen_merged.py` — deterministic, no model call — restores every `.raw.txt` **byte-exactly**: the audit’s raw-hash mismatches all clear. Four of five `.portrait.json` files still mismatch, and the entire diff is one field: the window recorded `"layers": ["pw"]`, regeneration now yields `"layers": ["nws", "pw"]`. The control is exact — `kast_ur_i`, the one headword `_pilot_gen_merged` renders with a single `[pw]` sub-card and no `nws`, reproduces byte-for-byte. **So the portraits did not rot; the SOURCE gained a layer.** Those four headwords acquired an `nws` (Nachträge) layer after the window was planned, and the manifest’s own recorded portrait is the evidence (`inputs[key].nws == 0` at plan time). **This flips the disposition of the window.** §612 as first written treated `no_pwg_w09`’s two clean cards as a loss — correct but incomplete. They are not merely unvalidatable: they were translated against a view of these headwords that was missing a whole supplement layer, so promoting them — even if the gate could somehow be satisfied — would write **knowingly incomplete entries** into the store. The refusal was protecting content, not just provenance. The right disposition is a re-run, and a re-run is now strictly BETTER than the original window rather than a consolation: it translates `nws + pw` where the paid window saw only `pw`. **Transferable rule:** *a stale-input gate is usually read as a provenance nuisance — "the hashes drifted, re-verify" — but the drift can mean the source itself grew, in which case the gate is the only thing standing between you and silently storing an entry that is missing a layer.* Before overriding one, diff the inputs and ask which side changed: a regenerable `.raw.txt` that still matches proves the text is stable, and isolates the mismatch to the field that actually moved.
...
### §614. `markup_wrapper_dropped` is two different defects, and only the count-match probe tells them apart

> H3663 residual — a predicted repair recovered 1 card of 8, not most of them. (Opus 5 `claude-opus-5`) — SanskritLexicography · 2026-08-29
...
### §616. PWG-TM has a paid path outside the reservation kernel, while its green reconciliation ignores nested unresolved placeholders

🔴 **Two independent control-plane omissions make a plausible green receipt weaker than it looks.** The live branch in [`pwg_tm_generate.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_tm_generate.py) calls `client.chat.completions.create(...)` directly inside `_live_complete_chunk`: there is no durable reserve-before-dispatch record, per-call dollar/call ceiling, or hard request timeout at that boundary. Its provider response is one paid chunk call, but usage is divided across returned fragments and later ledger accounting increments per fragment; a malformed paid response with zero returned rows can therefore leave no counted fragment call, while the run remains `cost_evaluable: false`. Route provenance then stamps generated, deterministic-reuse, and imported material through the same Grok-labelled surface.

The canonical-data check has the complementary blind spot. A read-only census on 30-08-2026 found [`canonical.v1.jsonl`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/release/pwg_tm_canonical/canonical.v1.jsonl) at **2,392 rows**, SHA-256 `b9ad8e9f…bbe82`, with **79 rows containing 609 literal `{T<number>}` occurrences** in nested card payloads. The sibling [`reconciliation.v1.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/release/pwg_tm_canonical/reconciliation.v1.json) reports the same count/hash and `ok: true`, because it validates cardinality, identity, and lost top-level fields but does not recursively reject unresolved placeholders.

**Apply:** every paid provider adapter must enter one reserve → bounded dispatch → returned-route/usage capture → seal → finalize kernel, with accounting at the provider-call level. Every promotion/reconciliation gate must recursively traverse all nested strings and report exact JSON paths for unresolved sentinels. Existing 79 rows are a fenced repair backlog, not permission to rewrite canonical data during the control-plane migration. Tracking: [issue #1987](https://github.com/gasyoun/SanskritLexicography/issues/1987). Execution design: [PLAN_RussianTranslation_PWG_CONTROL_PLANE_STRANGLER_2026H2.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/docs/PLAN_RussianTranslation_PWG_CONTROL_PLANE_STRANGLER_2026H2.md); [H3714](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3714-Codex_SanskritLexicography_pwg-control-plane-strangler-wave1_30.08.26.md) (Codex Sol `gpt-5.6-sol`, 30-08-2026).

### §617. An index and a number that look alike: `~~h<N>` is an `enumerate` position, and the code that read it as a printed homonym reported success precisely when it guessed

🔴 **A type confusion between "the N-th record" and "the record printed as №N" put another word's page number on 24.6 % of mappable pwg_ru store rows.** `_pilot_gen_merged.gen_root_split` builds sub-card keys with `for hom, ... in enumerate(segmented)` → `'%s~~h%d_00_%s'`. That `<N>` is a 0-based position in the headword's PWG record list. `pwg_page_index.annotate_cards` then compared it against the source's printed `<h>`, which starts at **1** — so `~~h0_` (9,666 of 11,519 rows) could never match, control fell to `sel = cand` (every record sharing the `key1`), and `starts[0]` took the **lowest `(vol, col)` across all homographs**. Measured against the source, `~~h0_` maps to printed `<h>1` 113×, `<h>2` 48×, `<h>3` 33× — and never to `<h>0`. `export_de_edition.py` allowlists exactly `volume`/`page`/`column` and emits them inside a per-sense subject that also carries `pwglex:homonym`, so the published graph paired a homonym label with a different homonym's locus.

**Two generalisable lessons, both cheap:**

1. **A positional index and a printed identifier must never share a name.** The two coincide exactly when the record list happens to be `1..n` with no gaps — which PWG violates constantly, because a headword's record list interleaves homonyms with volume-2/5/7 Nachträge that carry **no** `<h>` at all. `Ap` has four records and zero `<h>` markers; `As` has `<h>1`, `<h>2`, `<h>3` plus two more `<h>2` Nachträge. Neither list is addressable by homonym number. The fix is to resolve **positionally** — index `N` addresses record `N` of the same list the generator enumerated — and to carry the printed value explicitly (`hom_printed`, `pc`) rather than let each consumer re-derive it.
2. **A counter incremented on the fallback path measures nothing.** `annotate_cards` printed `matched/total`, and `matched` incremented at the end of the homograph-pooled guess — so the operator-facing statistic reported success *in the failing case*. Replaying the pre-fix code on the fixture returns `matched=7` for 7 rows including one whose index addressed no record at all. A success counter that cannot distinguish a resolution from a fallback is the [#1803](https://github.com/gasyoun/SanskritLexicography/issues/1803) disease in miniature; split it (`matched` / `ambiguous` / `unmatched`) or it will vouch for the bug.

**Apply:** when a join is ambiguous, prefer **omitting** the scalar assertion over guessing it — an absent `column` cannot ship a wrong locus into a published graph, a guessed one can, and the honest set-valued fields (`pc_all`/`page_all`) were never claiming to name one homograph anyway. Do not re-spell the key to "fix" the naming: `~~h<N>` is the identity of 11k already-promoted rows, so changing its meaning at the generator would fork the join instead of repairing it — fix the *interpretation*, in one shared module, and pin it.

Measured and repaired by [H3751](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3751-Opus_SanskritLexicography_pwg-homonym-index-remap_30.08.26.md) (31-08-2026, Opus 4.8 `claude-opus-4-8`), issue [#1801](https://github.com/gasyoun/SanskritLexicography/issues/1801): 1,278 of 5,205 mappable pwg-layer rows (24.6 %, ratio 1.00× against H2889's claim), repaired under a 1,969-row JSONL ledger with `delta == ledger` re-read from disk, `changed_ru=0` and hard flags unchanged. Shared reading: [`src/pwg_homonym.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_homonym.py). Pin: `python RussianTranslation/src/pwg_page_index.py --selftest`, verified RED against `7435178e0`.

### §618. A 939-row static I/O-in-loop census overstated coordinator.py's real hotspots by roughly two orders of magnitude — cProfile on a realistic fixture found exactly one

🟡 **Repowise scored `coordinator.py` at 1.0/10 health with 939 static "I/O in loop" findings; profiling it before touching anything (H3754 W7, 31-08-2026) found exactly one measured hotspot class, not hundreds.** Almost every static hit is a `for`/list-comprehension near an `open(`/`json.load`/`subprocess.run` token that is either (a) bounded to a handful of iterations per lease-operation (requeue's `source_paths` loop, `begin_run_leases`' lease-id lookups) or (b) already fixed by an earlier pass — the residual ledger's `append_from_audit_report`/`backfill_documented` carry an explicit H1940-Phase-2 comment: read the ledger once per batch, not once per key ([`no_pwg_residual_ledger.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pilot/no_pwg_residual_ledger.py)). The one real hit — `save_state()` → `prepare_state_for_save()` already scans+expires the full lease list, then `write_dashboard()` independently re-scanned it twice more via its own `running_translation_leases`/`reserved_translation_leases` calls, on a state that provably had not changed in the interim — was found by cProfile against a synthetic 480-lease fixture (incl. legacy manifest-derived nominal leases matching production's un-backfilled shape), not by reading the static list: 806011 → 632881 function calls (-21.5 %) and 1.100s → 0.978s pure-Python cProfile tottime (-11.1 %) after the fix, over 30 claim-shaped load/scan/save cycles. See [H3754](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3754-Opus_SanskritLexicography_pwg-perf-top10-measured_30.08.26.md) / [PR #2001](https://github.com/gasyoun/SanskritLexicography/pull/2001).

**Apply:** a static I/O-in-loop count is a lead to profile, never a work-item count. Before spending a session "fixing" a large static-analysis backlog on a long-lived module, build a realistic fixture (real record counts, real edge cases like legacy un-backfilled rows) and cProfile the actual call sequence a production operation makes — most of the census will turn out to be either already-bounded or already-fixed by an earlier pass whose fix comment the static scanner cannot see. Class A (auto-reproducible): re-run the fixture-driven cProfile comparison in the PR.

### §619. `pwg_tm_fragmentize.py`'s GLOSS_RE split 73 discontinuous glosses at `<is>` boundaries, orphaning half of each as an untranslatable fragment — GAPS §18 graduated and closed

🟢 **A census over the live 2,392-record / 11,129-sense TM publication ([`translation_memory.ru.publication.jsonl`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/release/translation_memory/translation_memory.ru.publication.jsonl)) found `{%...%}` gloss spans separated only by whitespace and a single `<is>…</is>` run in 73 places across 58 senses in 53 records** — the shape GAPS §18 named after the `viSveSa` 2 case (`{%die%} <is>Viśve Devāḥ</is> {%zur Gottheit habend%}`, one gloss, "having the Viśve Devāḥ as deity"). `GLOSS_RE = r'\{%.*?%\}'` in [`markup_fidelity_gates.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/markup_fidelity_gates.py) matches each `{%...%}` run independently, so `pwg_tm_fragmentize.py`'s `extract_from_sense` emitted `{%die%}` as a standalone `definition_gloss` fragment — a bare German definite article with no lexical content, permanently unpromotable (Russian has no articles).

Census script: [`h3753_is_interrupted_gloss_census.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/h3753_is_interrupted_gloss_census.py); dated snapshot: [`docs/CENSUS_H3753_IS_INTERRUPTED_GLOSSES_31-08-2026.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/docs/CENSUS_H3753_IS_INTERRUPTED_GLOSSES_31-08-2026.json).

**Fix (RED-pinned on the viSveSa case):** `_rejoin_is_interrupted_glosses()` in [`pwg_tm_fragmentize.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pwg_tm_fragmentize.py) merges gloss spans separated only by one `<is>…</is>` run into a single `{%...%}` span (chains handled by repeated passes) BEFORE `extract_from_sense` runs `GLOSS_FULL` extraction; the `<is>` content is kept inside the merged span so the gloss is fragmented and translated as one unit. Only the gloss extractor sees the rejoined text — `<ls>`/`<lex>`/`<ab>`/`{#...#}` extraction still runs against the untouched sense text. Live re-run of `fragmentize_rows` over the same publication: `definition_gloss` fragments fall from 15,781 → 15,708 and total fragments from 112,133 → 112,060 — a drop of exactly 73, one per interruption event, confirming no chain double-counted. Tests: `test_is_interrupted_gloss_rejoined_before_fragmenting` (RED-pin: `{%die%}` alone must never appear) and `test_rejoin_is_interrupted_glosses_chain_and_noop` in [`tests/test_pwg_tm_canonical.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/tests/test_pwg_tm_canonical.py); `pwg_tm_fragmentize.py --verify` stays green.

**Out of scope (Wave-2+):** existing store rows produced under the old (pre-fix) fragment identity keep that identity — re-fragmenting and re-promoting them is a separate act with its own review, not part of this fix. The 73-event / 58-sense population above is the future-facing count of what changes on the next re-fragmentize; it does not retroactively touch anything already shipped. [H3753](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3753-Sonnet_SanskritLexicography_pwg-fragmentizer-rejoin_30.08.26.md) (Sonnet 5 `claude-sonnet-5`, 31-08-2026), closing GAPS §18.

### §620. Splitting one overloaded field into two axes does not fix it unless the OLD field's value also changes — readers take the old name alone

🔴 **The half-fix that reads as a fix.** A field carrying two claims at once is normally repaired by moving the second claim into a new field. That is only half the repair, and the missing half is invisible in review because the new field is demonstrably correct. On 16-08-2026 H2879 split the pwg_ru re-glue's `subtype` — which conflated "what kind of relation this is" with "this supplement relates to *that* sense" — by adding a `placement` boolean, and **deliberately left `subtype` unchanged**, reasoning that duplicating one fact across two fields guarantees drift ([REGLUE_SPEC §10](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/REGLUE_SPEC.md)). Two weeks later the census for [#1736](https://github.com/gasyoun/SanskritLexicography/issues/1736) found **4,187 rows still labelled `restate`** ("PW paraphrases *this* PWG sense") whose own `target_sense` read `*new` — the pipeline's own marker for *no sense was identified*. The boolean was right and nobody read it: every consuming surface — UI chips, the rollup TSV, a published headline percentage, a vote sheet's question — takes the **name** on its own, because a name is what a name is for.

The drift objection is real, but it is an argument about *how* to derive the label, not a licence to leave it false. The repair that satisfies both: make the old field a **pure function of the new one**, computed in the same expression at the same site, so redundancy exists but drift is arithmetically impossible — then assert the correspondence as a corpus-wide **STOP gate in both directions** (the suffix is present ⟺ the sense-asserting relation has no target). Pinning it RED is what proves it: the new gate came up **RED at 5,066 rows** against the already-shipped artifact.

Two corollaries worth reusing. **(a)** Ship a `base_subtype()`-style resolver back to the old vocabulary in the same commit, or every lookup table silently falls through to its default — here that default would have repainted the exact defective rows as green "additions", a louder lie than the one being fixed. **(b)** Before assuming a label change implies a store rewrite, measure where the label actually *lives*: this one sat on 86 canonical rows (none affected) and 6,326 derived sidecar rows, so the store stayed byte-identical and the ledger belonged to the derived artifact — which does **not** make the ledger optional, since 5,000 rows changing meaning with no before/after record is exactly what an unledgered mutation is.

**Apply:** when splitting an overloaded field, treat "the old field's value is unchanged" as a defect to be justified per value, not as the conservative default; derive the old field from the new one at one site; ship a STOP gate asserting the correspondence over the whole corpus, RED-pinned against the pre-fix artifact; and ship the compatibility resolver with it. [H3752](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3752-Opus_SanskritLexicography_pwg-relation-label-revision_30.08.26.md) (Opus 5 `claude-opus-5`, 31-08-2026), [REGLUE_SPEC §13](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/REGLUE_SPEC.md), [reports/H3752_RELATION_LABEL_REVISION_31-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/reports/H3752_RELATION_LABEL_REVISION_31-08-2026.md).

### §621. A version string with no hash behind it is indistinguishable from a measurement — and the reconstruction that would repair it fails 8 of the 9 stamps that can test it

🔴 **93.5 % of the pwg_ru store asserts a pipeline provenance it never measured, and the loss is permanent.** [`pipeline_version.stamp()`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/pipeline_version.py) writes a `prompt_sha` / `glossary_sha` / `script_sha` triple computed from the component bytes at generation time. Before 2026-07-04T20:36Z it did not run: those rows were later stamped by `pipeline_version.py backfill`, which writes `prompt_version: "1.0.0"` and `backfilled: true` and nothing else. Census over the live 11,519-row store ([`provenance_census.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/provenance_census.py), 31-08-2026): **10,773 asserted (93.52 %) · 737 measured (6.40 %) · 9 with no input identity at all (0.08 %)** — confirming [#1804](https://github.com/gasyoun/SanskritLexicography/issues/1804)'s 93.0 % at a different store size, ratio 1.01. The eras do not interleave: every asserted row falls in 2026-06-29T11:40Z .. 2026-07-04T05:43Z, every measured row after it, so the boundary is a timestamp rather than a judgement. Every row in both eras carries `prompt_version: "1.0.0"`, which is why the assertion is invisible: **the version string is identical on the rows that measured and the rows that did not.**

**The reconstruction that looks obvious is measurably wrong.** "Recompute the component hash from the repository as it stood when the row was generated" can be *tested*, because the measured era is ground truth for it. Recomputing `component_sha` over the manifest's file sets at all 99 commits that touched a component in 2026-06-01..2026-09-01 reproduces **1 of 9 measured stamp classes — 20 of 737 rows (2.7 %)** — under both a strict test (newest component commit at or before `generated_at`) and a deliberately loose one (any commit in the window, including commits made after the row). The cause is in the data: the prompt set hashes to `dc0e1e2b65ca9731` on 66 rows across 25–29 August, a value that exists at no commit at all. **The pipeline runs against an uncommitted working tree; git records what was committed, the store records what was executed, and for this pipeline those are routinely different objects.** A method wrong about the era it can be checked against must not be applied to the era it cannot — a git-derived backfill of the 10,773 would replace one assertion with a larger, better-dressed one.

**What survives is the input half, and it is corroborated.** Each row names the workflow sidecar that recorded its batch's input hashes, and the pre-cutover sidecars survive in `archive/legacy_runtime_2026-07-04/`. For the asserted era: 10,773 of 10,773 name a surviving sidecar and **10,100 (93.8 %) have their `input_raw_sha256` independently confirmed** by a file the promotion path did not write. These rows do not know what code translated them; they do know, twice over, what text was translated. All 673 non-confirmations are one defect — rows whose `wf_file` is the generic `wf_output.json`, a default name with four physical copies on disk that every later unnamed run overwrites; 1,025 rows point at it, and for them the sidecar layer resolves to whatever ran last. **A pointer to evidence must be content-addressed, not name-addressed.**

**Apply:** never let a backfill write a field whose name ends in `_sha` — a hash means bytes were read, and a reconstruction goes in a separately-named block carrying its method, its validation score and its confidence. Test any provenance-reconstruction method against the rows that already carry the truth *before* applying it to the rows that do not; a method that scores 1 in 9 is a finding, not a repair. And make measurement recoverable rather than merely recorded: archive the component bytes content-addressed when the hash is computed, and stamp `source_commit` + an explicit `worktree_dirty` flag, so a dirty-tree run stays legal but stops being invisible. [H3750](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3750-Opus_SanskritLexicography_pwg-provenance-census_30.08.26.md) (Opus 5 `claude-opus-5`, 31-08-2026), census [reports/PWG_RU_PROVENANCE_CENSUS_31-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/reports/PWG_RU_PROVENANCE_CENSUS_31-08-2026.md), design [docs/SPEC_PWG_RU_PROVENANCE_BACKFILL_31-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/docs/SPEC_PWG_RU_PROVENANCE_BACKFILL_31-08-2026.md), open remainder [GAPS §19](https://github.com/gasyoun/SanskritLexicography/blob/master/GAPS.md).

### §622. A coverage audit is meaningless until you measure how much of the generated side came from the attested side

🔴 **kosha's A3 morphology audit reported that its generator misses 2 of 381,413 attested forms, and the number was an artefact of joining the corpus against itself.** It joined 426,410 generated inflected forms against 381,413 attested DCS surface forms and reported **attested-but-never-generated = 2**. Read at face value that is a spectacular generator. What it recorded is that **93.30 % of its generated side was itself `source='dcs'`** — DCS's own tokens, previously ingested into the `forms` table and then joined back against DCS. The join was a round-trip, and the direction the audit existed to measure was structurally unmeasurable on it.

**The same question, asked of an independent generator, answers five orders of magnitude apart.** Re-running with the same key function (`form_key()`), the same corpus and the same week's data against `kosha.db` `inflections` — 3,326,312 distinct forms, 99.99 % `source='cologne_mwinflect'`, generated from MW headwords with **no DCS input** — gives **A¬G = 196,378 of 386,012 attested keys (50.87 %)** and over-generation of 92.80 %, against the original **2**. To the first build's credit the circularity is stated plainly in its own report and manifest notes and the contradiction was escalated rather than resolved; it still propagated, because nobody owned the follow-up. The roadmap's exit check ("full 6.9M × 5.7M join complete") was carried as satisfied for six weeks by a build that joined 426,410 rows, and `A¬G = 2` sat inside a released dataset row. **An escalated-but-unresolved contradiction behaves exactly like a silent bug.**

**The corrected number is not the actionable number either, and the gap between them is 35×.** A¬G of 196,378 narrows to **5,656** genuine give-back candidates through three subtractions that are scope facts, not defects: DCS-tagged finite verbs (26,470 — the generator holds 680 verbal lemmas against 222,736 nominal, so verbs are territory it never claimed), forms attested only as a *sandhied* surface (61,330 — a paradigm generator emits unsandhied forms, so this is an artefact of the attested key), and bare stems whose key equals their own lemma's (562 — DCS tags bound compound members as tokens). What survives cross-validates: the head is pronoun dative/locative cells and irregular feminine/consonant stems, exactly where MW-inflect is weak, and the same database already carries a hand-made `curated-gita-pronoun` patch — independent evidence the gap is real. It also still leaks: allomorphic bound stems (`rājñ` < `rājan`, `ātma` < `ātman`) pass an exact-match stem filter and sit at ranks 2–3, so it ships as a candidate set needing human triage, never a defect list.

**Apply:** before quoting any coverage, gap or recall number over two sets, measure the provenance overlap of the two sides and publish it beside the number — if side A is partly derived from side B, the measurable gap is bounded by the un-derived remainder, so say what that remainder is. Treat a near-zero asymmetry as a smell rather than a success: "misses 2 of 381,413" should trigger a provenance check before it triggers a celebration, because the suspiciously good result is this defect's signature. A `source` column exists to be grouped by — partition every bucket by it and publish per-source subtotals, since a headline mixing an ingested source with a generated one measures neither. And measure an inventory's *declared scope* before counting its gaps, cross-tabbing the residue against the corpus's own POS tagging, so out-of-scope territory is never reported as an engine defect. [H3782](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3782-Opus_kosha_concordance-a3-generated-vs-attested-audit_31.08.26.md) (Opus 5 `claude-opus-5`, 01-09-2026), reports [MORPHOLOGY_ATTESTATION_INFLECTIONS_BUILD_REPORT.md](https://github.com/gasyoun/kosha/blob/main/data/concordance/MORPHOLOGY_ATTESTATION_INFLECTIONS_BUILD_REPORT.md) and [MORPHOLOGY_GIVEBACK_CANDIDATES_REPORT.md](https://github.com/gasyoun/kosha/blob/main/data/concordance/MORPHOLOGY_GIVEBACK_CANDIDATES_REPORT.md), roadmap [CONCORDANCE_ROADMAP.md](https://github.com/gasyoun/kosha/blob/main/CONCORDANCE_ROADMAP.md) Q3.

### §623. A citation siglum can be a book the lexicographer himself printed: two of PWG's 45 dated sources date nothing, and one of them is Böhtlingk's own anthology

🔴 **`Spr.` and `ŚKDR.` set a chronological bound on 11,771 of 43,990 attestation windows, and neither can carry one.** C2 phase 1 ([H3168](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3168-OxAlpha_SanskritLexicography_ceiling-c2p1-sense-attestation-window_19.08.26.md)) windowed every numbered PWG sense over the 45 works in [`ls_source_map.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/ls_source_map.json), each carrying a bare point date with no provenance. Curating those dates against the scholarly literature (H3790) found two that are not datable Sanskrit works at all. **`Spr.`** is *Indische Sprüche*, **Böhtlingk's own anthology** of gnomic verse (St. Petersburg 1863–1865, 2nd ed. 1870–1873) — the editor of the dictionary citing his own collection, whose verses are drawn from across the whole of Sanskrit literature; the map gave it 600 CE. **`ŚKDR.`** is the *Śabdakalpadruma*, printed at Calcutta 1821–1858, i.e. inside Böhtlingk–Roth's own working life; the map gave it 1830 CE. A citation of either says the editors found the word in a book of their own century, and nothing whatever about when the word or sense existed.

**Measured on the committed store, not asserted:** 11,771 of 43,990 windows (26.8 %) contain one of the two — 7,508 cite `ŚKDR.`, 4,653 cite `Spr.`; **9,082 have their `latest` bound set by one of them**, and **2,873 have no other dated work at all**, so excluding the pair makes those windows vanish rather than shrink. Phase 1's own published hand-check sample shows the mechanism without any further work: `kāYci / sense #0` cites `UṆĀDIK.` and `ŚKDR.` and emerges as a window of **1830…1830**; `ruc / sense #0` cites `Spr. (II) 6939` and `ṚV. 1,165,12` and emerges as **−1125…600**, where the 600 is an artefact of Böhtlingk's title page.

**The general shape.** A 19th-century dictionary's citation apparatus mixes *witnesses* (texts that attest the language) with *authorities* (reference books the editors consulted) under one `<ls>` mechanism, and nothing in the markup separates them. Any pipeline that assigns a date to every siglum will silently date the authorities too — and because the authorities are recent, they dominate the `latest` bound wherever they appear. Seven further sigla were found carrying map dates outside their sourced range (`AMAR` 450 CE against a 7th–8th-century Amaru; `GĪT. GOV` 1048 against Jayadeva at Lakṣmaṇasena's court c. 1170–1200; `RĀJAN` 1300 against Vogel's «flourished in Kashmir after 1375»; plus `KATHĀS`, `SĀH. D`, `VOP`, `Spr`) — evidence-decidable corrections, recorded in the curated table rather than written back into the committed map phase 1 consumed.

**Apply:** before dating a citation apparatus, split its sigla into witnesses and authorities and give the authorities a `dating_valid: false` flag rather than a plausible number — a number is the one thing a downstream consumer will not question. Grade every date by what stands behind it (`anchored` / `consensus` / `contested` / `invalid`), gate the store so a date with no citation is a hard failure, and route the datings that scholars genuinely dispute to a human instead of picking one. [H3790](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3790-Opus_SanskritLexicography_ceiling-c2-phase2-work-dating-table_31.08.26.md) (Opus 5 `claude-opus-5`, 01-09-2026), table [`RussianTranslation/src/work_dating_table.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/work_dating_table.json), memo [`research/C2P2_WORK_DATING_TABLE.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/research/C2P2_WORK_DATING_TABLE.md), gate [`research/c2p2_dating_table.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/research/c2p2_dating_table.py), the eleven open datings at [pwg_c2p2_work_dating_11](https://gasyoun.github.io/vote/sheets/pwg_c2p2_work_dating_11.html).

### §625. pwg_ru has no "translate N pre-selected keys, report-only, never promote" entrypoint — the lease/promote machinery assumes every job was claimed and dispatched once

🟠 [H3791](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3791-Sonnet_SanskritLexicography_pwg-datalayers-w3-requeue-contaminated-ru_31.08.26.md) needed to run the 01-08-2026 200-row `〉`-quarantine sample's own recommended next step — a 30-card nested human/paid quality read, translate-only, never write to the store — and found no tool built for it. `RussianTranslation/src/pilot/requeue_from_audit.py` and `coordinator.py prepare-requeue` are both keyed on a coordinator `lease_id`: they assume the work item was already claimed and dispatched once via the normal transient/defect-retry flow, then re-fetch it for a second attempt. There is no first-class notion of "translate an ad hoc list of keys nobody has leased, write the result to a report, and stop" anywhere in `bounded_staged_run.py`/`coordinator.py`/`headless_worker.py`. Building one from scratch against the lease/promote internals, live, under a fresh `/pwg-live-gate` receipt clock, was judged too risky to improvise in-session rather than hand off. Full record: [H3791_WAVE3_REMEASURE_LIVEGATE_02-09-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/pwg_ru/h3791/H3791_WAVE3_REMEASURE_LIVEGATE_02-09-2026.md). **Do not re-derive:** a future session wanting a bounded, non-lease, translate-only sample either needs a human ruling on which existing lane (nominal window? defect-repair with `--no-residual`?) actually fits, or a small new script — this gap is not obviously closed by any single existing flag.

### §624. A CDSL dictionary code need not name the dictionary its letters suggest — `md` is Macdonell, not Medinī — and the Sanskrit-to-Sanskrit kośas carry no `<ls>` at all

🔴 **Two absences that a plausible-looking code and a plausible-looking assumption would each have hidden.** [H3862](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3862-Opus_kosha_sense-recon-w2-slice2-sasa-columns_02.09.26.md) was to add ŚKDR, Medinī, VCP and Amara as columns on kosha's aligned-sense table, on the stated premise that the row model already carried the cells and that «PWG already cites ŚKDR and Medinī in `<ls>`, so the witness bridge points at them». Both halves of that premise were wrong in ways nothing downstream would have flagged.

**1. The code collision.** csl-orig and csl-sqlite both carry an `md` code. It is **Macdonell**, *A Practical Sanskrit English Dictionary* (London 1893) — not Medinīkośa. Medinī and Amara are **not in CDSL at all**: no asset in any csl-sqlite release, no directory in `csl-orig/v02`. A loader that resolved «Medinī» to `md` because the letters matched would have put a 19th-century Sanskrit-English dictionary into a Sanskrit-Sanskrit kośa's column, under the right header, with plausible content — and no schema check, test or gate in the pipeline can see that. The confirmed reachable set is **ŚKDR = `skd`** and **VCP = `vcp`**.

**2. The kośas have no citation markup.** Measured over the csl-sqlite releases: **0 of 42,531 ŚKDR records and 0 of 50,135 VCP records contain a single `<ls>` element.** Their bodies are `<s>` end to end, because their metalanguage *is* Sanskrit; they mark attribution in running prose (`ityamaraḥ`, `iti medinī`, `yathā suśrute`). So for a Sa→Sa kośa the shared-literary-witness bridge — in a German/English/Sanskrit table the only channel that crosses a language boundary — **does not exist by construction**, and the gloss-overlap channel is shut by the same language fence that shuts it across German.

**The bridge that does exist runs the other way, and §623 already named the mechanism.** PWG cites these kośas in its own `<ls>` — `ŚKDR.` 1,227× and `MED.` 1,824× across a 500-headword pilot, touching 479 of its 500 lemmas. §623 classifies exactly those sigla as **authorities** rather than witnesses (`ŚKDR.` is Calcutta 1821–1858, inside Böhtlingk–Roth's own working life) and warns against dating them. Read as *attributions* instead of as *witnesses*, the same sigla are sound and useful: a PWG sense whose `<ls>` names ŚKDR is saying which meaning Śabdakalpadruma records for the lemma. That is directional evidence — one dictionary pointing at another, not two converging on a third text — and must be ranked below a genuine shared witness wherever it is used.

**The channel that looks obvious and is not.** Reading the kośas' own `iti X` / `yathā X` attributions as witnesses was implemented and measured before being rejected. The attribution particle is followed by an ordinary word far more often than by a source name, so after SLP1→IAST normalisation it matched PWG's `PRAT.` to `pratyarthin` («counter-claimant») and `BUDDH.` to `buddhim` — re-inventing witnesses through the side door that a minimum-prefix-length rule exists to close. Yield 73 of 707 records, most of them false. Genuine names *are* frequent in the extraction (`amaraḥ`, `medinī`, `śabdaratnāvalī`, `hemacandraḥ`), so the honest version is a **closed, curated source vocabulary**, not an open-vocabulary regex.

**Apply.** Before adding a dictionary to any cross-dictionary derivation: (a) resolve the CDSL code to the actual title from the release itself, never from the abbreviation — a two-letter code is not a name; (b) count the markup you intend to join on (`SELECT COUNT(*) … WHERE data LIKE '%<ls%'`) before designing around it, because «this family has `<ls>`» is a per-dictionary fact, not a family-wide one; (c) when a source is genuinely absent, ship the ones that exist and record the absence with its reason — a recorded absence is a finding, a silent omission is a defect; and (d) make any signal that is a *pointer* rather than a *convergence* say so in its own method token, so a reader can discount it. Measured and shipped in [kosha PR #492](https://github.com/gasyoun/kosha/pull/492) (Opus 5 `claude-opus-5`, 02-09-2026), algorithm [`app/sense_align.py`](https://github.com/gasyoun/kosha/blob/main/app/sense_align.py), evidence [`data/concordance/SENSE_ALIGNMENT_BUILD_REPORT.md`](https://github.com/gasyoun/kosha/blob/main/data/concordance/SENSE_ALIGNMENT_BUILD_REPORT.md).

### §626. Fixing a phonological fold at one position leaves the same defect at every other position the same phonology reaches — and the class built to detect it goes to zero, which reads like completeness

🔴 **The regression test that proves a fix landed can be the same thing that hides the half you did not fix.** [§622](#622-a-coverage-audit-is-meaningless-until-you-measure-how-much-of-the-generated-side-came-from-the-attested-side) established kosha's A3 audit on an independent generator; [H3911](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3911-Opus_sanskrit-util_formkey-final-nasal-fold_02.09.26.md) then found that the join key could not collide the two standard spellings of a Sanskrit **word-final** nasal — `form_key()` folded anusvāra to `n` at every position while never touching a literal `m`, so `rasaṃ` → `rasan` but `rasam` → `rasam`, and every anusvāra-final attestation read as un-generated. Fixed in [sanskrit-util 0.11.0](https://github.com/sanskrit-lexicon/sanskrit-util/pull/72); the whole chain was rebuilt (H3925). Three things came out of the rebuild that the diagnosis had not predicted.

**1. The sampled estimate was a floor, not a forecast — by a third.** H3911 measured the defect on the give-back candidate set and projected A¬G 196,378 → ≈ 172,229 (−24,149). The rebuild measured **−32,142** (196,378 → **164,236**). The sample could only see twins already inside the candidate set, so it was structurally incapable of seeing the ones the narrowing had already subtracted. **A defect measured on a filtered subset bounds the true effect from below; quote it as a floor and say so, never as an expected value.**

**2. A fold fix is a re-partition, not a monotone gain.** Collapsing final `-ṃ` onto `-m` also *breaks* matches that existed only because final `-ṃ` and final `-n` were conflated under the old rule. Generated-side AG moved 239,443 → **238,312**: a net **−1,131**. Distinct attested keys fell 386,012 → **352,745**, so the headline "coverage 49.13% → 53.44%" is partly a denominator effect — 33,267 of the old keys were spellings of each other, not distinct words. **Report both directions and both terms of the ratio, or a merge-key fix will look like a discovery of new data.**

**3. The detector class emptied, and that was not completeness.** The triage carried an `orthographic-variant` verdict whose only job was catching anusvāra twins the broken key had split. It matched **146 rows before the rebuild and 0 after** — a clean pass, and exactly the shape of evidence that ends an investigation. But the fold was corrected only **word-finally**. Medial anusvāra before a **labial** is phonetically /m/ too, and it still folds to `n`: `vaiśaṃpāyana` keys as `vaiśanpāyana` and never meets the `vaiśampāyanaḥ` the generator already emits. Measured on the one class that reaches a human — the 2,521 `slot-conflict` rows — **278 (11.03 %, 11.58 % by corpus weight) are not disagreements at all**: `saṃbhavaḥ` vs `sambhavaḥ`, `saṃbandhaḥ` vs `sambandhaḥ`, `samyaksaṃbuddhaḥ` vs `samyaksambuddhaḥ`. A further 90 candidates collapse into their own lemma once refolded. The detector reported zero because it was written to test the *fixed* position, not the *rule*.

**Apply.** (a) When a normalisation rule is position-sensitive, enumerate every position the underlying phonology reaches and measure each one before declaring the rule fixed — homorganic nasal assimilation is not a word-final phenomenon, and the code that treats it as one will be wrong wherever else it applies. (b) Make the driver of any re-derivation **refuse to start** unless the dependency actually carries the fix: `rebuild_a3_chain.py` asserts `form_key("rasaṃ") == form_key("rasam")` *and* `form_key("rājan") != form_key("rājam")` before stage 1, which is what caught the shared sanskrit-util checkout still sitting at 0.6.0 — running against it would have reproduced the old keys as a convincing no-op with green output. (c) A verdict class that exists to catch a specific defect is evidence about that defect only; do not read its emptiness as evidence about the rule. (d) Rows a measurement has resolved must be **screened off a human review sheet by a named rule** rather than left for a reviewer to reject one at a time — the 278 were still on the plate, and one of them had already been annotated in [REVIEW_SHEETS_INDEX](https://github.com/gasyoun/Uprava/blob/main/REVIEW_SHEETS_INDEX.md) as "likely a reject", which is a human being asked to re-derive a fact the repo could compute. (e) Screening-banner counts must be measured at build time: four of that sheet's five rule counts were stale, and a stale count tells a reviewer the plate was filtered by rules that did not run.

> **Source:** measured in [kosha PR #499](https://github.com/gasyoun/kosha/pull/499) — [`scripts/measure_medial_anusvara_residual.py`](https://github.com/gasyoun/kosha/blob/main/scripts/measure_medial_anusvara_residual.py), [`scripts/rebuild_a3_chain.py`](https://github.com/gasyoun/kosha/blob/main/scripts/rebuild_a3_chain.py), rebuilt figures in [`CONCORDANCE_ROADMAP.md`](https://github.com/gasyoun/kosha/blob/main/CONCORDANCE_ROADMAP.md) Q3 and [`MORPHOLOGY_GIVEBACK_TRIAGE_REPORT.md`](https://github.com/gasyoun/kosha/blob/main/data/concordance/MORPHOLOGY_GIVEBACK_TRIAGE_REPORT.md). The medial fold is deliberately unfixed — it is a third edit to a library ~85 repos consume — and is queued in [GTD_NEXT_ACTIONS](https://github.com/gasyoun/Uprava/blob/main/GTD_NEXT_ACTIONS.md). — kosha · 02-09-2026 (Opus 5 `claude-opus-5`, H3925)

### §627. A morphological feature present on 16,100 tokens can be categorically absent from an entire mood — DCS's `Formation` never leaves `Mood=Ind`, and the propagation that would recover it reaches 296 of 8,726 tokens

🔴 **An absent feature *value* and an absent *feature* are different facts, and only a census over the whole distribution tells them apart.** UD has no Aorist tense, so Sanskrit aorist and perfect both surface as `Tense=Past`; [H1486](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1486-Opus_VisualDCS_past-tense-aorist-perfect-resplit_22.07.26.md) re-split them inside VisualDCS using DCS's own `Formation` key, but only under `feat_mood='Ind'`. That predicate read like a scope decision somebody had made, and the queue carried it as an open question: are the 8,726 `Tense=Past` Jussive/Imperative/Subjunctive/Optative/Precative tokens untagged because DCS never tags them, or because our query hides the tag? H3878 answered it by counting every `Formation` occurrence in the pinned 15,900-file distribution rather than by reasoning about the predicate.

**1. The feature never leaves the indicative — 17,440 occurrences, zero outside `Mood=Ind`.** 16,100 sit on `Tense=Past` and 1,340 on `Tense=Fut`, all on finite `VERB`. Not one Jus (4,067), Imp (1,700), Sub (1,317), Opt (1,065) or Prec (577) token carries it, and neither does any of the 9,112 past participles. The whole `Tense=Past` universe is 111,167 tokens at 14.48 % formation coverage, but that coverage is **17.25 % inside the indicative and exactly 0.0 % everywhere else** — a step function, not a gradient. So `feat_mood='Ind'` is not hiding data; it is a description of where the data ends. **A predicate that looks like an arbitrary narrowing may be the shape of the annotation, and one census distinguishes those two readings for the price of one pass over the corpus.**

**2. "The tag is absent" is not yet "the information is unavailable" — that needs its own measurement, and it came back 3.39 %.** These forms plainly *have* a stem formation (`kṛdhi`, `gahi`, `bodhi` are root-aorist imperatives; `mā bhūt` / `mā hiṃsīḥ` augmentless injunctives), so if the same surface forms were tagged elsewhere the honest answer would be *build an inference layer*, not *stop*. Pointing H1486's same-form instrument the other way: **301 of 8,726 (3.45 %) match a formation-tagged `Tense=Past` form, 296 (3.39 %) also agree on lemma.** The prior that homography would be the limiting factor is **falsified** — the lemma test removes 5 tokens, not a third. What actually limits propagation is that **8,425 tokens (96.55 %) carry a surface form never formation-tagged anywhere in the past corpus**. A layer built on the most generous instrument available would resolve 296 tokens, leave 8,430 (96.61 %) exactly as unresolved as they are now, and add a propagation step whose own error rate is unmeasured — hence an evidence-backed NO-GO with the count recorded, so the next session starts from 296 instead of re-deriving it.

**3. The tense-dependence trap sits inside the same key.** `peri` under `Tense=Past` is the periphrastic **perfect** (4,046); under `Tense=Fut` it is the periphrastic **future** (1,340) — a different feature of a different tense that happens to share a value string. Any `Formation` query that does not guard on tense sums them. (`red` is likewise the reduplicated **aorist**, not the reduplicated perfect, which DCS leaves unmarked — the basis of H1486's `NULL → Perfect` default.)

**Apply.** (a) Before treating a query predicate as a scope choice worth widening, census the feature across the whole distribution — the answer here is a two-row table and it retires the question permanently. (b) Record the measured NO-GO, not just the absence: "3.39 % recoverable, 96.55 % of forms never tagged anywhere" is a decision a future session can act on, while "we could not find the tags" invites the same audit again. (c) Test the homography assumption instead of applying it as a discount; strict-minus-loose was 5 tokens here, and a guessed discount would have been the reported number. (d) Quote these tokens **by mood only**: `Mood=Jus` on `Tense=Past` (4,067) is the augmentless injunctive, and whether a given one rests on an aorist or an imperfect stem is exactly what `Formation` would have said — resolving it needs an external lexical signal (stem class from a dictionary or a generator), the same shape of blocker as the `-ī`/`-ant` conflation. (e) Pin a verdict like this with a test over a committed fixture of **real** corpus sentences, not with a report nobody regenerates: the 1.2 GB corpus is on no CI box, so `test_past_nonindicative_formation.py` re-runs the census over 14 extracted branches and re-checks the committed JSON's own partition closure — while deliberately **not** asserting the recoverability percentages, which are a measurement over a corpus that may be re-pinned, not a contract.

> **Source:** measured in [VisualDCS PR #119](https://github.com/gasyoun/VisualDCS/pull/119) over pin `04e0778d3dc971030229179e25eea043d06ff397` (15,900 CoNLL-U files) — [`audit_past_nonindicative_formation.py`](https://github.com/gasyoun/VisualDCS/blob/main/src/DCS-data-2026/audit_past_nonindicative_formation.py), report [`past_nonindicative_formation_audit.md`](https://github.com/gasyoun/VisualDCS/blob/main/src/DCS-data-2026/reports/past_nonindicative_formation_audit.md), guard [`test_past_nonindicative_formation.py`](https://github.com/gasyoun/VisualDCS/blob/main/src/DCS-data-2026/test_past_nonindicative_formation.py); the abandoned propagation layer is [DEAD_ENDS §15](https://github.com/gasyoun/SanskritLexicography/blob/master/DEAD_ENDS.md). — VisualDCS · 02-09-2026 (Opus 5 `claude-opus-5`, H3878)

### §628. A sync ledger that counts only rows cannot see a content-only sync — so the refresh that carried a 75-row text sweep is byte-shaped like the no-op that carried nothing

🔴 **When the ledger for a copy operation counts only set membership, a copy whose entire delta is *changed content on rows present in both sets* records all zeros — and the audit trail says "nothing happened" about the one refresh that mattered.** [`refresh_tm_mirror.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/refresh_tm_mirror.py) appends a row to `pwg-ru-data/tm/mirror_refresh_ledger.jsonl` on every `--apply`. Every counter in that row is a row-set counter derived from `classify()`: `only_mirror_dropped`, `dropped_quarantined`, `dropped_id_churn`, `dropped_superseded_acked`, `dropped_unexplained`, `only_src_added`. All six are computed from `rid()` — `(key1, subcard, sense_tag, de[:80])` — which deliberately **excludes `ru`**, the field the mirror exists to serve.

**1. The two indistinguishable rows.** H3947 refreshed the mirror on 02-09-2026 to pick up the [H3969](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3969-Opus_SanskritLexicography_german-latin-store-sweep-outside-ab_02.09.26.md) German→Latin sweep — 75 rows whose `ru` text had changed (`Akk`→`Acc.`, `Instr`→`Ins.`, `Lok`→`Loc.`, `Präs`). Its ledger row reads `only_mirror_dropped: 0, dropped_quarantined: 0, dropped_id_churn: 0, dropped_superseded_acked: 0, dropped_unexplained: 0, only_src_added: 0, src_rows: 11519, mirror_rows_before: 11519, mirror_rows_after: 11519`. The H3751 row from 31-08 — a refresh that moved nothing anyone has identified — carries **exactly the same nine values**. The only field that separates them is the `mirror_sha_before`/`mirror_sha_after` pair, which is opaque: it proves *something* changed, never *what* or *how much*.

**2. The measurement exists, one tool over.** `audit_store_gates.py`'s mirror-diff line already prints `only_src=N only_mirror=N changed_ru=N` and lists the per-row opcodes. `changed_ru` was **75** before this refresh and **0** after — the exact number the ledger needed and the exact number it does not carry. Two tools measure the same mirror and only the one that writes no durable record knows the answer.

**3. Why this bites specifically here.** The `pwg-ru-data` mirror is Git LFS, so `git log` on it shows a pointer-oid change and nothing else; the ledger *is* the human-readable history. A session asking "did the mirror ever pick up the H3969 sweep?" — the natural question after any store content mutation — reads an all-zero row and concludes no. The failure is silent and reads as reassurance, which is the worst direction for an audit trail to fail in.

**4. This is a class, not one script.** Any sync ledger keyed on a row identity that omits the payload has the same blind spot: `restore_store_rows_from_mirror.py` shares `rid()` byte-for-byte by design (the three implementations are pinned to agree), so it inherits it. The rule generalises — **an audit record must count the thing the artifact exists to deliver, and a mirror of translations exists to deliver `ru` text, not row identities.**

**Apply.** (a) Carry `changed_ru` (or a payload-hash delta) into the ledger row so a content-only refresh is legible without re-running the audit; the number is already computed next door. (b) Where a ledger row can be all-zero yet non-trivial, treat the sha pair as the tell and say so in the tool's own docstring, so the next reader does not take zeros as a no-op. (c) When a row identity deliberately excludes a field (here `de[:80]` and no `ru`, so re-parses read as id churn rather than loss — a correct choice for the *guards*), audit every consumer of that identity for the ones that needed the excluded field: the guards were right and the ledger was wrong, from the same function. (d) Generally: before trusting a "nothing changed" ledger row, check whether the ledger is capable of representing the change you are asking about.

> **Source:** found while executing [H3947](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3947-Opus_SanskritLexicography_pwg-ru-tm-mirror-refresh-167_02.09.26.md); refresh published as [pwg-ru-data eaeb870](https://github.com/gasyoun/pwg-ru-data/commit/eaeb870), pre/post measured with [`audit_store_gates.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/audit_store_gates.py) (`changed_ru` 75 → 0, `only_src`/`only_mirror` 0 throughout, 11,519 rows unchanged). Related: [§600](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md) (the same tools' store-resolution split) and [§593](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md) (the 167-row mirror staleness this handoff was minted for, already cleared by H3663/H3751 before it ran). — SanskritLexicography · 02-09-2026 (Opus 5 `claude-opus-5`, H3947)

### §629. A seed inventory counted across agent worktrees inflates its own denominator — the "61 IAST-bearing seeds" of GAPS §6 are 32, and the table built over them keys 534 names with zero character rules

🟡 **Two findings in one execution: the count a downstream handoff was scoped against was wrong by ~2×, and the artifact that count was blocking is buildable anyway.**

**1. The denominator artifact.** [FINDINGS §495](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md) reported "61 files with inline IAST parentheses / 47 Cyrillic-heavy files with zero IAST hits" from H1746's [`gaps_s6_cyrillic_name_probe.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/tools/gaps_s6_cyrillic_name_probe.py). That probe `rglob`s two roots and counts **paths**, and the SanskritLexicography checkout carries `.claude/worktrees/` trees: **10 of the 15 top-ranked seeds are copies of files that exist exactly once in the repo.** Re-running the identical filters and regexes over content-hashed distinct files ([`h3985_seed_inventory.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/tools/h3985_seed_inventory.py)) gives **85 candidates scanned, 32 IAST-bearing, 20 pure-Cyrillic** — 0 duplicate groups remain. The probe's report also truncated `all_hits` at 80, so the list could not be audited from the artifact alone; it had to be re-derived. **Rule: any inventory that counts filesystem paths inside a repo that hosts agent worktrees must hash content before it publishes a number, and must publish the untruncated list.**

**2. The table is decidable where a witness exists.** [`cyrillic_proper_noun_slp1.tsv`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/data/cyrillic_proper_noun_slp1.tsv): **534 rows** (518 distinct Cyrillic spellings, 532 distinct SLP1 keys) harvested as headword-anchored `Кириллица (IAST)` pairs and transcoded IAST→SLP1 with `indic_transliteration.sanscript`. **`rule_derived_keys: 0`** — no key touches a Russian character rule, which is the refuted approach GAPS §6 exists to fence off (`Дхритараштри → DftarAzwrI`, `Читракута → citrakUwa`, `Шурпанакха → SUrpaRaKA` all carry the retroflexes a т/ш rule would flatten). Each row is graded: **`onomasticon` 446 (83.5 %)** — the SLP1 key is a headword of Sørensen's *Index to the Names in the Mahābhārata* (`inm`, 9,454) or the *Purāṇic Index* (`pui`, 11,730), both already SLP1 in csl-orig, so "validated against an onomasticon" is set membership, not review; **`lexicon` 50** — MW only; **`iast-witness-only` 38** — the honest weakest tier (`Kṛttikās`, `Mataṅgāśrama`, plus a few source typos).

**3. What the reject ladder buys.** 1,175 candidate pairs were dropped against 673 kept, and the reasons are the finding: **696 mixed-script** (OCR mojibake — Cyrillic homoglyphs *inside* the apparent IAST, `Akора`), **251 witness not capitalised** (running glosses, `dharmakṣetra` — common nouns, not names), **200 Cyrillic run is running prose** (`Сравните этот термин (Ātmanepada)`), **20 all-caps siglum** (`Рену → evp`, bibliography rendered as a name pair), **8 outside the IAST alphabet**. Every one of these would have entered the table as a plausible-looking row under a looser extractor. **A parenthetical next to a Cyrillic word is not a transliteration until something proves it is a name.**

**4. Coverage is a third, not a half.** Against the three fully-Cyrillic indices the gap named: Эрман–Тёмкин **174/478 = 36.4 %**, Гринцер/Кадамбари **130/393 = 33.1 %**, Потапова **128/326 = 39.3 %**. Storage format decided whether they were measurable at all — two of the three live as corpus-builder JSONL, one as TSV-ish text, and a plain line regex saw only the last (the Potapova HTML twin scores 2/77 against its JSONL's 39.3 %, which is a parser artifact, not a coverage difference). **The 20 pure-Cyrillic seeds stay explicitly unkeyed and are listed**, per the handoff's own fail condition.

**5. Ambiguity is disclosed, not resolved.** 16 Cyrillic spellings map to more than one SLP1 key across seeds — `Тара → tArA | tAra`, `Дандака → daRqaka | dandaka`, `Гаруда → gAruqa | garuqa`. These are real source-level vowel-length and retroflex disagreements between the Russian editions, and the table carries both with their witness counts rather than picking one.

**Apply.** (a) A consumer joining on this table reads the `validation` column and decides its own floor — `onomasticon` is the safe tier. (b) GAPS §6 is **half-closed**, not closed: ~65 % of each Cyrillic index still has no key and no rule may manufacture one. (c) Re-audit any other count that came out of a repo-wide `rglob` in a worktree-hosting checkout.

> **Source:** [H3985](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3985-Opus_SanskritLexicography_cyrillic-proper-noun-slp1-table_03.09.26.md) (Opus 5 `claude-opus-5`) · builder [`h3985_cyr_slp1_table.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/tools/h3985_cyr_slp1_table.py) · validation [`H3985_cyr_slp1_validation.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/reports/H3985_cyr_slp1_validation.json) · corrects [§495](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md) · measures [GAPS §6](https://github.com/gasyoun/SanskritLexicography/blob/master/GAPS.md) · [§60](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md#60-practical-russian-transcription-of-sanskrit-names-has-no-safe-reverse-transliteration) stands. — SanskritLexicography · 03-09-2026

### §630. The dictionary's *entry ids*, not its headword list, are what separate one lexeme spelled twice from two lexemes — and a presence-only test gets the pinned `bhagavat`/`bhagavant` case exactly backwards

🟠 **GAPS §14 asked for "a dictionary class tag (MW/PWG grammar field)". There is no such field in the csl-json exports — but the headword index answers the question better than a grammar tag would, provided the ids are compared and not merely the keys.** A csl-json export is `{data: {words: {devanagari_headword: "id,id,…"}, text: {id: […]}}}`; grammar lives inside gloss prose (`m.`, `mfn.`, `avy.`), never as a structured field. Pattern-matching that prose is exactly the plausible-looking rule [H1472](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1472-Opus_VisualDCS_nominal-paradigm-case-number-dashboard_22.07.26.md) refused to encode. The `words` index is different in kind: it is the lexicographer's own statement of **which spellings reach which entries**.

**1. The distinction the ids carry and membership does not.** PWG registers both भगवत् and भगवन्त् under the identical id pair `53806,80057` — the same for महत्/महन्त् (`58261,121517`) and भवत्/भवन्त् (`54294,121349`). Shared ids are the dictionary declaring *one lexeme under two spellings*; **disjoint** ids would be it keeping two headwords apart. A test that asks only "is each spelling present?" sees `both` in either case, and the first implementation of this split classified 191 lemmas — `bhagavant` included — as two headwords on that evidence, inverting the very pair GAPS §14 pins by name. Comparing id **sets** is a one-line change and it is the whole finding.

**2. The measured sizes.** Over Sangram G2's complete `upos='NOUN'` universe ([`lemma_cell_coverage.csv`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/data/declension_cell_coverage/lemma_cell_coverage.csv), H1048, same pinned master and same `lemma_id` keying):

| pool | subclass / verdict | lemma_ids | tokens |
|---|---|--:|--:|
| `ii` | `ii_monosyllabic` — śrī/strī/dhī type | 17 | 6,373 |
| `ii` | `ii_polysyllabic` — devī/nadī type | 4,477 | 58,295 |
| `ant` | `one_lexeme_two_spellings` | 191 | 8,851 |
| `ant` | `two_headwords` | 0 | 0 |
| `ant` | `at_only` (DCS `-ant` id carries a variant spelling) | 51 | 2,384 |
| `ant` | `ant_only` | 1 | 1 |
| `ant` | `unresolved` — no witness in either dictionary, stays pooled | 36 | 70 |

Signal-B reach is **243 of 279 NOUN lemma_ids (87.1 %)**. Both re-bucketings reconcile against G2 exactly (`reconciles: true`, six checks, no total moves) — the handoff's second fail condition was "a G2 total that silently moves", and the reconciliation report is what proves it did not.

**3. The ī split needs no dictionary at all, and that is a result about the question.** A Sanskrit syllable has exactly one vowel nucleus, so an NFC-normalised IAST citation form's nucleus count (`ai`/`au` counting once) **is** its syllable count. Monosyllabic long-ī stems take the strong-case endings the devī/nadī type does not — a phonological property of the word, not a spelling accident, which is why this is a lexical signal rather than the character rule H1472 fenced off. GAPS §14 proposed "a transliteration-aware segmenter"; no segmenter is needed, because a citation form is one word and the nucleus inventory is closed.

**4. The merge the pool hid is two different defects, and conflating them inflates it.** 6 stems carry two `lemma_id`s where the dictionary indexes one lexeme (410 tokens, 12 ids for 6 lexemes) — but only **one** of them, `saṃvat` (42263, 18 tok) + `saṃvant` (42311, 2 tok), is the `-at`/`-ant` spelling pair GAPS §14 describes. The other five (`bṛhat` 356, `dhṛtimat` 16, `gabhastimat` 11, `rājīmat` 5, `dyumat` 2) are the master holding **two ids under the same citation form** — a duplicate-lemma_id defect that has nothing to do with stem class. Reporting them in one table would have made the class merge look six times its size. Separately, **26 lemmas / 2,066 tokens** are in the `-ant` bucket only because the master's citation spelling differs from the dictionary's — `jagant` (164188, 1,826 tok) against जगत्, `arvant` (53553, 124) against अर्वत् — i.e. their bucket membership is an orthographic choice of the corpus, not a fact about the stem.

**5. What stays INCONCLUSIVE.** G2's universe is NOUN-only. By token mass the `ii` pool is **99.0 %** NOUN (64,668 / 664 ADJ / 65,332), so the split covers nearly all of it; the `ant` pool is only **23.5 %** NOUN (11,306 / 36,768 ADJ / 48,074), because most `-vant`/`-mant` stems are tagged ADJ. That ADJ surplus has no per-lemma inventory outside `dcs_full.sqlite`, absent on this box, and `paradigm_nominal.json`'s `topLemmas` is capped at 25 — so it is declared unreached, not estimated. The pinned pair inherits this: the DCS `bhagavat` `lemma_id` is ADJ and therefore outside G2, so the two-`lemma_id` merge **itself** cannot be counted from this substrate even though the dictionary verdict on it is unambiguous.

**Apply.** (a) When a lexicon is used as an external signal, read its *identity* layer (entry ids, cross-reference targets) before its *content* layer; the identity layer is machine-clean and states relations the prose only implies. (b) A "both spellings present" test is not evidence of two lexemes — it is evidence of nothing until the ids are compared, and it fails silently in the direction of over-splitting. (c) Report reach as a fraction with the unreached rows named: 36 lemmas left pooled with no witness is a usable result, "the dictionary covers the pool" would not have been. (d) Before scoping work against a proposed signal, check the export actually has the field the gap names — here the named field (a grammar tag) does not exist and a better one did.

**ADDENDUM 06-09-2026 — §5 is closed: the ADJ surplus is now MEASURED, and the presence-only inversion is confirmed at seven times the scale.** [H4011](https://github.com/gasyoun/Uprava/blob/main/handoffs/H4011-Opus_SanskritLexicography_nominal-ant-adj-half-dcs-master_03.09.26.md) (Opus 5 `claude-opus-5`) re-ran the same two signals with the per-lemma inventory taken **from `dcs_full.sqlite` directly** instead of the NOUN-only G2 CSV — read-only, streaming, never copied. Nothing below is estimated or scaled from the NOUN half.

| pool | subclass / verdict | lemma_ids | tokens |
|---|---|--:|--:|
| `ant` ADJ | `one_lexeme_two_spellings` | 1,015 | 34,809 |
| `ant` ADJ | `two_headwords` | **0** | **0** |
| `ant` ADJ | `at_only` | 193 | 661 |
| `ant` ADJ | `ant_only` | 13 | 95 |
| `ant` ADJ | `unresolved` — no witness in either dictionary, stays pooled | 659 | 1,203 |
| `ii` ADJ | `ii_monosyllabic` | 3 | 31 |
| `ii` ADJ | `ii_polysyllabic` | 51 | 633 |

**6. The `-ant` pool partitions cleanly, which is why the NOUN half could not have been extrapolated.** The class is 48,074 tokens over 2,159 `lemma_id`s = **ADJ 36,768 / 1,880** + **NOUN 11,306 / 279**, and **not one `lemma_id` carries both upos**. G2's 279 / 11,306 is re-derived here unmoved, and all eleven reconciliation rows pass. Note the shape: the ADJ half holds 76.5 % of the tokens but 87.1 % of the `lemma_id`s, so the two halves have different tokens-per-lemma profiles (19.6 vs 40.5) — a NOUN-half ratio applied to the ADJ mass would have been wrong in both directions at once.

**7. Across 1,880 ADJ `lemma_id`s, `two_headwords` is still empty.** The NOUN half saw 0 / 279 and that could have been a small-sample accident; at seven times the lemma count it is a result about the alternation itself. With entry-id sets, the `-at`/`-ant` variation in DCS is **uniformly one lexeme spelled two ways** — never a homonym pair. The presence-only test that called 191 NOUN lemmas `two_headwords` would have inverted every one of the 1,015 ADJ lemmas here too.

**8. The pinned pair is finally reachable from the inventory side, not only the dictionary side.** DCS `bhagavat` is `lemma_id` **48482**, tagged **ADJ**, 3,238 ADJ tokens — exactly why it sat outside G2 and why §5 had to leave it INCONCLUSIVE. Its verdict is `one_lexeme_two_spellings`, and MW alone would have said `at_only` (it indexes भगवत् under nine ids and has no भगवन्त् headword at all); PWG's identical `53806,80057` pair is what carries the finding. **One lexicon is not a signal** — the combined verdict is doing real work here, not decoration.

**9. The unresolved tail is a productivity artefact, not a lexicographic gap.** 659 `lemma_id`s / 1,203 tokens (3.3 % of the ADJ mass) have no witness in either dictionary. **528 of them carry the fully productive `-vat`/`-mat`/`-vant`/`-mant` possessive shape** and **449 are single-token hapax** (the largest row in the entire tail is 36 tokens). A productive suffix generates ad-hoc derivatives that no lexicon indexes as headwords, so a dictionary-based signal *must* leave a tail here — its size is a property of Sanskrit derivation, not a defect in the method. They stay pooled. (DCS tags every one of these `adj` in `lemma.grammar` and carries no VERB token for any of them, so the participle-looking rows — `akurvat`, `acintayat` — are a **shape** observation, not a corpus-attested part of speech; that distinction is why they are described rather than split.)

**10. Substrate lesson — verify a rebuild by re-deriving its published totals, not by comparing hashes.** The master used here is a *local rebuild* (imported 2026-09-03T22:47Z) from the same `source_commit 04e0778d`, so its SHA-256 does **not** match the `.sha256` sidecar or the `8f3b06bd…` pinned in `paradigm_nominal.json` (built from the 2026-06-06 import). A hash comparison alone would have condemned a perfectly good substrate. Re-deriving the published figures settled it in one query: `gridTokens` 2,263,192 and every `-ant`/`-ī` class total reproduce **to the token**. SQLite page layout is not content.

**Apply (additional).** (e) A signal validated on a filtered universe is not thereby validated on the excluded one — measure the excluded half before assuming it behaves the same; here it did, but the differing tokens-per-lemma profile shows the assumption was not free. (f) When two lexicons disagree, say which one carried the verdict: `bhagavat` is `one_lexeme_two_spellings` on PWG evidence while MW would have said `at_only`. (g) A residual that tracks a **productive morphological process** is expected output, not unfinished work — size it, characterise it and stop, rather than reaching for a character rule to close it.

> **Source:** [H3984](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3984-Opus_SanskritLexicography_nominal-i-ant-stem-class-split_03.09.26.md) (Opus 5 `claude-opus-5`) · builder [`split_pooled_nominal_classes.py`](https://github.com/gasyoun/VisualDCS/blob/main/src/DCS-data-2026/split_pooled_nominal_classes.py) · report [`nominal_pooled_class_split.md`](https://github.com/gasyoun/VisualDCS/blob/main/reports/nominal_pooled_class_split.md) · reconciliation [`nominal_g2_reconciliation_split.md`](https://github.com/gasyoun/VisualDCS/blob/main/reports/nominal_g2_reconciliation_split.md) · payload [`paradigm_nominal_class_split.json`](https://github.com/gasyoun/VisualDCS/blob/main/visual/paradigm_nominal_class_split.json) · [VisualDCS PR #123](https://github.com/gasyoun/VisualDCS/pull/123) · signal source [`mw.json`](https://raw.githubusercontent.com/sanskrit-lexicon/csl-json/main/ashtadhyayi.com/mw.json) (195,313 headwords) + [`pwg.json`](https://raw.githubusercontent.com/sanskrit-lexicon/csl-json/main/ashtadhyayi.com/pwg.json) (108,952) · graduates [GAPS §14](https://github.com/gasyoun/SanskritLexicography/blob/master/GAPS.md) for the NOUN universe. — SanskritLexicography · 03-09-2026
>
> **Addendum source:** [H4011](https://github.com/gasyoun/Uprava/blob/main/handoffs/H4011-Opus_SanskritLexicography_nominal-ant-adj-half-dcs-master_03.09.26.md) (Opus 5 `claude-opus-5`) · builder [`split_pooled_nominal_classes_adj.py`](https://github.com/gasyoun/VisualDCS/blob/main/src/DCS-data-2026/split_pooled_nominal_classes_adj.py) · report [`nominal_pooled_class_split_adj.md`](https://github.com/gasyoun/VisualDCS/blob/main/reports/nominal_pooled_class_split_adj.md) · payload [`paradigm_nominal_class_split_adj.json`](https://github.com/gasyoun/VisualDCS/blob/main/visual/paradigm_nominal_class_split_adj.json) · [VisualDCS PR #126](https://github.com/gasyoun/VisualDCS/pull/126) · closes §5 above and graduates [GAPS §14](https://github.com/gasyoun/SanskritLexicography/blob/master/GAPS.md) in full. — SanskritLexicography · 06-09-2026

### §631. A form→lemma map and a lemma inventory answer different questions — probing an inflected compound head against the *lemma set* loses 1,018 forms silently, and a fallback lexicon that is sound on whole forms is not sound on compound-internal ones

**The 1,389-form "morpheme-marker residual" of the Sa→Ru gloss layer was never a morphology gap.** Corpus forms carry `+`/`-` boundary marks (`A+gam` = ā+√gam), and tier 3 of [`build_rollup_glossaries.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/src/build_rollup_glossaries.py) split on them and probed the rightmost element against `roots_set` and `lemmas_set`. `A-brahma-BuvanAt` fell through — and the published typology recorded the reason as *"rightmost element is itself inflected, not a bare root"*, which reads like a morphological limitation. It is not. `BuvanAt` is an ablative of `Buvana`, and it was a key in `dcs_form2lemma.tsv` the whole time. The pipeline held a 408,660-entry **form→lemma** map and asked the question against a **lemma inventory** instead. One extra probe recovered 1,018 forms / 1,783 tokens (73.3 % / 77.1 % of the residual) with no new morphology, no new lexicon, and nothing segmented.

**Apply.** When a lookup "needs light morphology", check first whether an inflected-form index already in the process answers it. A lemma/root inventory contains citation forms only and will refuse every inflected string by construction — that refusal looks like a linguistic finding and is a plumbing one. The tell here: the typology row named the *linguistic* property ("is itself inflected") and nobody re-read it as a *lookup-table* property.

**The second half is the one that costs precision.** The same map carries a vidyut fallback, merged in for forms DCS missed. On whole corpus forms it is legitimate — that is what it was built and (later) measured for. Used on a compound-**internal** element out of context it degrades: all 42 vidyut-sourced heads in this residual were adjudicated exhaustively, and **35/42** carry a defensible lemma. The seven failures are not near-misses, they are non-words — `vart`→`varDi`, `sarp`→`sarb`, `Bar`→`Barv`, `ar`→`arv`, `saNketa`→`saNketi`, `avaca`→`avaci`, `upAyatA`→`upaAi` — and five more are garbled stems (`saMkalpi`, `saMBAvi`) or over-reductions (`cittas`→`cit`). This corroborates the wave-2 panel independently: it put the whole **vidyut tier at 71.8 %** lemma precision against dcs 94.9 % and marker 93.3 % ([gold/saru_gloss_precision_report.md](https://github.com/gasyoun/SanskritLexicography/blob/master/RussianTranslation/gold/saru_gloss_precision_report.md)). The head probe is therefore fenced to DCS form keys — 42 forms / 59 tokens, 3.2 % of the available recovery, given up to keep the tier at the DCS precision level.

**A sharper-looking gate was tested and does not work.** "Accept a vidyut head whose lemma is itself a DCS-attested lemma" keeps `Barv` and `avaci` while dropping five correct heads (`yuktAtman`, `vimUQAtman`, `tfptAtman`, `nityatva`, `aBisaMtapta`). Being in a lexicon's lemma inventory is not evidence that *this* lemmatization is right. A blunt provenance fence beat a clever content test.

**And a minimum-evidence floor is worth its cost when the floor is measured.** Every 1–2 character SLP1 head in the residual resolved wrong: `aSva-zA` / `nf-zA` / `vAja-zA` → `zA` → **`tad`**, the demonstrative pronoun, where the real head is the root-noun `-ṣā` "winning" (√san). One syllable carries too little signal to beat a high-frequency function word in a count-ranked homograph list. Cost of `MIN_HEAD_LEN = 3`: four forms, all four of them errors — no correct recovery lost.

**What the recovery does *not* fix, stated because the numbers flatter it.** Lemmatization is near-perfect (25/25 on the canonical tier × frequency sample), but the *gloss* axis is 15 correct / 8 partial / 2 wrong of 25, well under the wave-2 `marker` baseline of 90.0 %. The reason is structural and pre-existing: these forms **are** compounds, so their aggregated Russian belongs to the whole compound, not to the head lemma the tier attributes it to. `vigata-BIr` "бесстрашный" lands on `BI` ("страх") — a bahuvrīhi whose sense *negates* its head; roughly 12 % of the nominal-head recoveries are of that shape. This is the wave-2 panel's own systematic lemma defect #3 ("compound tokens lemmatized to their final member only"), now extended to 1,018 more forms rather than solved.

**The layer-level number is the one that decides GO, and it is the reassuring one.** Because the glossary ranks each entry's renderings by count and the recovered forms are overwhelmingly hapax, only **27 of 40,387 lemma entries** (0.067 %) and 2 of 1,856 root entries change their dominant Russian gloss — and 23 of those 27 are ties between two one-occurrence glosses, i.e. entries with no meaningful "top" to disturb. Four are real: two improvements (`diva` gains "день за днем" on 26 occurrences, `yAjin` gains "Мне жертвующим"), one orthographic wash, one mild degradation (`saMsiD`). **Judge a coverage tier on whether it moves dominant renderings, not on its item-level gloss score** — the two numbers answer different questions and the item-level one alone would have argued for a NO-GO on a change that is measurably safe.
...
### §632. A contamination share measured over the corpus is not the share inside the stratum you are about to publish — 10.15 % corpus-wide was 29.22 % in the vedic slice, and the caveat that quoted the corpus number understated the affected slice threefold

🔴 **When a known defect is annotated rather than fixed, the annotation must carry the contamination *per analysed stratum*, because a corpus-wide average is exactly the statistic a stratified study never uses.** [§627](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md) established that DCS never assigns `Formation` outside `Mood=Ind`, so a finite past-tense bucket defined as "`Tense=Past` with no `Formation`" silently swallows the whole non-indicative preterite. The HK-15 imperfect-switching study had exactly that bucket, and the ruling was **annotate now, re-run later** — the right call, because silently renumbering a pre-registered study destroys the pre-registration. The caveat published one number for the damage: **8,726 of 85,955 (10.15 %)**, measured corpus-wide. The guarded re-run then measured it per genre slice, and the corpus figure turned out to describe no slice at all.

**1. The contamination was a vedic phenomenon, not a background.** Per-slice `PERF` loss under `feat_mood='Ind'`: **vedic −29.22 %** (19,543 → 13,833), classical −3.03 %, epic −1.88 %, puranic **−0.88 %**. A 33× spread across the strata the study actually reports on. The reason is linguistic and was predictable in hindsight: the augmentless injunctive (`Mood=Jus`, 4,067 of the 8,726) is a Vedic category, so the contamination concentrates precisely where the study's only substantive signal lived. Anyone citing a vedic row of the annotated report was standing on a bucket roughly one token in four of which was not a perfect — while the caveat above that row said 10 %.

**2. The unmeasured direction of bias was recoverable cheaply, and it was *strengthening*.** The caveat honestly recorded that it did not know which way the numbers would move. They moved in favour of the finding: vedic lexical turnover at imperfect insertions rose **+0.0151 → +0.0252** (+1.5 → +2.5 pp). The mechanism is legible in the two component means — the background level fell 1.4 pp (0.9016 → 0.8880) while the level at imperfects fell only 0.4 pp — so the discarded injunctive background points had **above-average** turnover. The injunctives were not behaving like the perfects they had been pooled with; the contamination was diluting the contrast, not manufacturing it. **A caveat that says "direction unknown" is a promise to measure, and the measurement is usually one predicate away.**

**3. The re-run vindicated annotating rather than retracting — measure that too.** Of 73 compared quantities (12 bucket sizes, 36 Markov cells, 21 runs-tests, 4 turnover slices) exactly **one** was material, and it was an input rather than a conclusion. No Markov `lift` crossed 1.0, no runs-test flipped direction or crossed p = 0.001, no turnover delta changed sign, and the pre-registered decision criterion still passed. The published-with-a-caveat numbers had misled nobody about a conclusion. **That verdict is itself worth recording: it is the evidence that "annotate and schedule the re-run" is a sound default, and without the delta nobody could have said so.**

**4. State the substitution when the original published no intervals.** The re-run was asked whether any headline number moved past its stated confidence interval. The v0.48.0 report published **decision thresholds**, not intervals, so materiality was judged against those thresholds (lift crossing 1.0; runs-test crossing p = 0.001 or flipping; turnover changing sign or crossing the pre-registered p = 0.01; a bucket moving further than the caveat predicted) — and the substitution was written into both the delta tool and the report rather than quietly assumed. **A correction that silently redefines the acceptance test is a second defect on top of the first.**

**Apply.** (a) When you annotate a defect instead of fixing it, report the contamination **per stratum the study reports on**, not once over the corpus — the average is the one number guaranteed not to describe any published row. (b) Suspect concentration whenever the contaminant is a *linguistic category*: moods, genres and registers co-vary, so a mood-shaped defect will be genre-shaped damage. (c) Write the corrected run to its **own** output path and make the instrument refuse to overwrite the pre-registration record; the guard is three lines and it is what makes "not retracted, not renumbered" checkable rather than promised. (d) Ship the before/after as a **committed diff tool with its materiality rule written down**, not as prose — the rule is then reviewable, and the count of material changes is a number instead of an impression. (e) Put the guard predicate in a regression test that fails on *partial* restoration too: here `IMPF` and `AOR` are 100 % indicative in this snapshot, so guarding `PERF` alone would pass every observable check today and silently make the claim depend on a property of one pinned corpus.

> **Source:** measured in [SanskritGrammar PR #912](https://github.com/gasyoun/SanskritGrammar/pull/912) over pin `04e0778` (seed 20260717, 1000 permutations, unchanged from the pre-registered design) — instrument [`imperfect_switching_stats.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/ZalizniakOcherk_1978/imperfect_switching_stats.py), diff tool [`imperfect_switching_delta.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/ZalizniakOcherk_1978/imperfect_switching_delta.py), guard [`tests/test_imperfect_switching_mood_guard.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/tests/test_imperfect_switching_mood_guard.py); corrected report [v0.49](https://github.com/gasyoun/SanskritGrammar/blob/main/ZalizniakOcherk_1978/IMPERFECT_SWITCHING_HK15_REPORT_V049.md), superseded-but-unaltered [v0.48.0](https://github.com/gasyoun/SanskritGrammar/blob/main/ZalizniakOcherk_1978/IMPERFECT_SWITCHING_HK15_REPORT.md); the defect itself is [§627](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md). — SanskritGrammar · 06-09-2026 (Claude Code Opus 5 `claude-opus-5[1m]`, [H3966](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3966-Opus_SanskritGrammar_t2607-26-mood-guarded-rerun-v049_02.09.26.md))

### §633. The second position of the same fold was monotone where the first was a re-partition — and the only way to know which is to run the control arm, because the inputs moved underneath the fix

🔴 **[§626](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md) said to enumerate every position the phonology reaches and predicted the medial one would be next; it did not say what the fix would *do*, and the answer turned out to be the opposite of the first position's.** `form_key()` 0.11.0 folded word-final anusvāra to `m`. 0.12.0 (H3975) narrows the medial fold the same way but only before a **labial** — `p ph b bh m`, because *homorganic* is a place of articulation and the nasal at the labial place is /m/. `saṃskṛta == sanskṛta`, `saṃvatsara → sanvatsara` (`v` is a semivowel, not a labial stop) and the real letters `ṅ/ñ/ṇ` are all untouched. `ph`/`bh` need no separate character class: the IAST digraphs start with `p`/`b`.

**1. The two positions behave differently, and the difference is predictable from the orthography.** The word-final fix was a genuine re-partition: it *broke* 1,131 generated matches, because merging final `-ṃ` onto `-m` removed matches that had existed only through the old conflation of final `-ṃ` with final `-n`, and `rājan` vs `rājam` is a real contrast at that position. Before a labial there is no such contrast to lose — an attested form would have to be spelled with a literal `-nb-`, `-np-` or `-nm-`, and Sanskrit orthography does not write those: the nasal there is `m` or anusvāra, never `n`. Measured over kosha's A3 join, exactly that: **1,178 attested keys left A¬G, 0 entered.** It reconciles without slack — 633 of the leavers stopped being distinct keys at all (they merged into their `m`-spelled twin, which is the denominator change) and 545 became matched. **A fold fix is not intrinsically a re-partition; whether it is depends on whether the position carries contrast, and that is checkable from the writing system before the rebuild is run.**

**2. What it moved where a human sees it.** The one class that reaches a reviewer — the give-back slot-conflicts — went **2,521 rows with 278 medial-labial twins (11.03%, 11.58% by corpus weight) to 2,212 rows with 0**. The 90 candidates that collapse into their own lemma once refolded are also 0. §626's own detector, the `orthographic-variant` verdict, had already emptied 146 → 0 after 0.11.0 and could never have seen these — which is the completeness illusion §626 named, now measured to its end at this position.

**3. The confound is the real lesson, and it nearly ate the finding.** kosha's `kosha.db` was itself rebuilt on 04-09-2026 (1,732.3 → 1,767.5 MB; distinct generated forms 3,326,312 → 3,333,034) between the 0.11.0 rebuild and this one, so the natural before/after mixes a key change with an input change and **cannot** be quoted as the key's effect. Isolating it needed a control arm: the identical audit, over the identical inputs, with sanskrit-util pinned to 0.11.0. The generated side was where the confound actually bit — the attested side was clean, DCS's raw distinct values being identical on both dates (381,413 sandhied / 303,859 unsandhied). **Rule: before quoting a delta as the effect of a fix, check whether anything else moved between the two measurements; in a repo whose inputs are rebuilt on their own schedule, "the last run's numbers" is not a control.**

**4. A pinned dependency must be verified as loaded, not as configured.** The first control arm was invalid and read as a null result: both arms returned identical figures to the byte, because the consuming module ran its own `sys.path.insert(0, …)` at import time and shadowed the pin. Two arms of an A/B agreeing exactly is a broken manipulation until proven otherwise. The check costs one line — probe the behaviour (`form_key('saṃbhavaḥ')` → `sanbhava` vs `sambhava`) in each arm before spending the run.

**Apply.** (a) When a fold is fixed at one position, ask of every remaining position whether the writing system carries contrast there — that predicts re-partition vs monotone gain before any rebuild. (b) Report both directions as a set diff of the two arms' key lists, never as a difference of totals; the totals hide compensating movement and cannot separate "became matched" from "stopped being a distinct key". (c) Treat a rebuilt input as invalidating the previous run as a control. (d) Verify a version pin by probing behaviour, not by trusting the env var. (e) Make the rebuild driver refuse to start unless the dependency carries the fix — kosha's `rebuild_a3_chain.py --check` now asserts six `form_key` invariants (both folds **and** their deliberate limits) so no consumer can rebuild against a stale checkout.

> **Source:** [sanskrit-util 0.12.0](https://github.com/sanskrit-lexicon/sanskrit-util/pull/75) with its [WhitneyRoots donor](https://github.com/gasyoun/WhitneyRoots/pull/86); control-arm method and reconciliation in [`data/concordance/evidence/H3975_CONTROL_ARM_KEY_ISOLATION.md`](https://github.com/gasyoun/kosha/blob/main/data/concordance/evidence/H3975_CONTROL_ARM_KEY_ISOLATION.md), re-partition table in [`CONCORDANCE_ROADMAP.md`](https://github.com/gasyoun/kosha/blob/main/CONCORDANCE_ROADMAP.md). The gate-blindness half — both cross-language gates went green on this edit — is [Uprava §715](https://github.com/gasyoun/Uprava/blob/main/FINDINGS.md). Predecessor: [§626](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md). — kosha · 06-09-2026 (Claude Code Opus 5 `claude-opus-5`, [H3975](https://github.com/gasyoun/Uprava/blob/main/handoffs/H3975-Opus_sanskrit-util_formkey-medial-anusvara-labial-fold_03.09.26.md))
