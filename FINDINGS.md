...
- 🟠 [§625. pwg_ru has no "translate N pre-selected keys, report-only, never promote" entrypoint — the lease/promote machinery assumes every job was claimed and dispatched once](#625-pwg_ru-has-no-translate-n-pre-selected-keys-report-only-never-promote-entrypoint--the-leasepromote-machinery-assumes-every-job-was-claimed-and-dispatched-once) — `requeue_from_audit.py` and `coordinator.py prepare-requeue` are both keyed on a coordinator `lease_id`, so an ad hoc translate-only sample that never writes to the store has no lane; H3791 judged improvising one against the lease internals under a live receipt clock too risky and handed it off — H3791 (Sonnet 5 `claude-sonnet-5`, 02-09-2026).
...
- 🔴 [§609. `classify_run` cannot tell a broken harness from a budget you set too small — `code-failure` is its fall-through, so an undersized `--max-calls` sends the next session hunting a phantom gate defect](#609-classify_run-cannot-tell-a-broken-harness-from-a-budget-you-set-too-small--code-failure-is-its-fall-through-so-an-undersized---max-calls-sends-the-next-session-hunting-a-phantom-gate-defect)
- 🔴 [§608. A `translation-fidelity-reject` discards the card, and `german_anchor` would refuse it anyway — the counter now says `not_reached` instead of `0`](#608-a-translation-fidelity-reject-discards-the-card-and-german_anchor-would-refuse-it-anyway--the-counter-now-says-not_reached-instead-of-0) — `hasita` replayed offline returns `nothing-missing`, so moving the call site would fix nothing; H3665 (Opus 5 `claude-opus-5`, 29-08-2026).
...
- 🔴 [§605. `german_anchor` CANNOT fire on a `translation-fidelity-reject` card — the two are mutually exclusive branches, so §604's `0` was the only value possible](#605-german_anchor-cannot-fire-on-a-translation-fidelity-reject-card--the-two-are-mutually-exclusive-branches-so-604s-0-was-the-only-value-possible)
