...
(currently §612) whatever its section, so existing numbers never shift; when a finding is later
...
- 🔴 [§611. The durable evidence root preserves a paid window’s OUTPUTS but not its INPUTS — once the session worktree dies, the window’s cards can never be audit-validated, so they can never be promoted](#611-the-durable-evidence-root-preserves-a-paid-windows-outputs-but-not-its-inputs--once-the-session-worktree-dies-the-windows-cards-can-never-be-audit-validated-so-they-can-never-be-promoted)
...
### §611. The durable evidence root preserves a paid window’s OUTPUTS but not its INPUTS — once the session worktree dies, the window’s cards can never be audit-validated, so they can never be promoted
...
> **Source:** `python src/pilot/audit_window.py src/pilot/output/wf_output.no_pwg_w09.json --root no_pwg_w09 --write-requeue --window-tag no_pwg_w09` · directory searches of the checkout, the H3659 worktree and `\.pwg_ru_evidence\c1\h3659\`
> · [H3659](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H3659-Opus_SanskritLexicography_h3157-residual-c1-paid-no-pwg-window_28.08.26.md)
> (Opus 5 `claude-opus-5`) · third recurrence of §603’s checkout-relative class, first one that is unrecoverable. — SanskritLexicography · 2026-08-29
