...
Написано в рамках H479 (Fable 5 `claude-fable-5`).

## 1. Что это за репозиторий

Это **рабочее пространство данных и исследований**, а не программный продукт.
Здесь живут: экспортированные списки заглавных слов 15+ словарей Кёльнской
коллекции, русский перевод Монье-Вильямса (mw_ru, 287 358 карточек, завершён),
...
- **[RussianTranslation/](https://github.com/gasyoun/SanskritLexicography/tree/master/RussianTranslation)** —
  два независимых конвейера: **mw_ru** (Монье-Вильямс → русский, завершён,
...
**Двигатель построен и прошёл полный аудит (H188: вердикт GO, 0 блокеров).**
В хранилище `src/pwg_ru_translated.jsonl` (не в git) — **11 275 строк**
переведённых под-карточек при цели ~120 173 карточек всего словаря (~9 %).

**Массовый прогон ещё не начат.** Что его держит:

1. **Номинальная линия (плотные band-4 карточки) остановлена** — каскад
   kill-gate/self-heal бюджета воспроизведён трижды (H317 → H437 → H442);
...
4. **Разделение пула heal и пула перевода** снимает связывание `MAX_AGENTS` —
   сейчас heal тратит тот же счётчик, и бюджет окна срабатывает всегда
   (доказано трижды).
5. Исследовательское: прокси-QE не ловит неверный смысл (AUC 0,58) —
   семантический подъём даёт консенсус + COMET; доля грейда A (5,3 %) должна
   измеримо вырасти с настоящим выравниванием.

## 7. Четыре аккаунта Claude Max — как гнать массовый PWG→RU

Протокол уже спроектирован (аудит H335 §W1) и укреплён кодом (H336:
claim-файлы промоушена, пер-оконные имена, гигиена append). Расширение с 3 до
4 аккаунтов — тот же протокол:

1. **Каждому аккаунту — свой git worktree** (облачным сессиям — своя ветка);
   общий клон запрещён.
2. **Шардировать по корням до старта**: список работ режется на 4
   непересекающихся среза; шардинг по корню гарантирует непересечение
   rootmap'ов, входов, ключей TM и очередей requeue.
3. **Правило единственного промоутера**: только один аккаунт (или финальная
   сессия) запускает `promote_final_cards.py --merge` и пересборку TM;
   остальные останавливаются на «прошло аудит, `wf_output.<окно>.json`
   сохранён». Облачные/веб-сессии возвращают свои `wf_output.<окно>.json`
   коммитом в свою ветку (каталог `RussianTranslation/incoming/` —
   см. агентское руководство §6), промоутер забирает, промоутит и чистит.
4. **Потолок ≤3 одновременных линий перевода — ГЛОБАЛЬНЫЙ, на всю
   организацию, а не на аккаунт.** Обвал Slice-D показал: обрыв серверный
   (~18 одновременных workflow). 4 аккаунта × 1 корень, максимум 3 в полёте.
...
- **Интерактивные сессии Claude Code**: предела одновременности с ошибками
  ...
  окно.
- **Workflow-развёртки (вот что реально падает)**: по нашим же измерениям
...
Безопасная форма на аккаунт: **1 линия генерации + 1–2 лёгкие сессии**
