...
Env/runtime: bulk corpus pulls go through the public [VedaWebProject/vedaweb-data](https://github.com/VedaWebProject/vedaweb-data) GitHub mirror (`rigveda/versions/zurich.xlsx`, CC BY 4.0 — the same Zurich glossed corpus the API serves as resource `66695e4a14f6d337f7788740`; working path proven 26-08-2026, [FINDINGS §588](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md)). The VedaWeb API itself (`vedaweb.uni-koeln.de/api`) remains WAF-blocked — HTTP 418 re-probed 26-08-2026; if trying it anyway, check liveness first (`curl -sI --max-time 15 …/api/openapi.json`); the host has an extended outage history (§48, [SERVER_OUTAGES](https://github.com/gasyoun/Uprava/blob/main/SERVER_OUTAGES.md)).
↔ Interlinks: [CONTRADICTIONS §1](https://github.com/gasyoun/SanskritLexicography/blob/master/CONTRADICTIONS.md) (Whitney gen.pl accent) was the accent disagreement this validation adjudicates — **ruled 26-08-2026 (H3555, Tier 1)** via the full-corpus gen.pl census in [docs/D3_GENPL_ACCENT_PROBE_26-08-2026.md](https://github.com/gasyoun/SanskritLexicography/blob/master/docs/D3_GENPL_ACCENT_PROBE_26-08-2026.md) · the former GAPS §1 (ī/ū split, n=2) graduated to [FINDINGS §587](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md) — n grew 2→71 Section-A tokens and the D3 cell is now a rule, not a variant · [DEAD_ENDS §2](https://github.com/gasyoun/SanskritLexicography/blob/master/DEAD_ENDS.md) is the failed corpus-only route that lacked the accent signal this recipe supplies.
...
### §5. Cross-dict union headword index (323,422 / PWG∩MW=94,753) → reproduce
...
Expected: 323,422 union headwords; PWG-bearing 106,054, MW-bearing 193,852, both 94,753 — ties back to FINDINGS §29 / manifest `union-headwords` (323,422 rows, 12.4 MB). CONSUME, don't rebuild (a new pairwise-overlap script is reinvention).
...
- **Every recipe ties a *number* back to a *runnable path*** — 81.4% linked (§2), 6.6% ungrounded (§3), Cramér's V = 0.037 (§4), 323,422 union headwords (§5), 39,540 correction loci (§6). A FINDINGS row that cannot be re-run this way is trust without reproduction; this file is the antidote.
