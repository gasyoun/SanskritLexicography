...
| 30-07-2026 | **§506–§509 appended, and a self-inflicted collision fixed in the same pass (H1910).** Four print-OCR extraction findings added: a complete-coverage count cannot see commentary leaking into an extracted layer (🔴), do not find hymn headings by matching roman numerals (🔴), archive.org `/download/` vs `/stream/` (🟠), and J–B declining to translate RV 10.106.5–8 (🟠). **The collision is the lesson worth keeping:** the numbers were first picked as §463–§466 by reading the repo's *main checkout*, which is frozen/diverged and showed a highest §462 — on `origin/master` the file already had 166 findings and those four numbers were taken, so four numbers briefly carried two claims each. Renumbered to §506–§509 against the marker instead. The marker itself was ALSO stale (`(currently §505)` while §505 was already used by the concurrent SamudraManthanam finding), so it is now bumped to §510 and the post-condition asserts the marker sits above every used number. Index parity backfilled (4 entries, anchors generated by a slugger validated against 138 of this file's existing 163 anchors — the 25 misses are pre-existing rot: 20 from an older hyphen-collapsing slugger, 5 stale after their own heading was edited). Both dashboards regenerated; [`tools/epistemic_integrity_check.py`](https://github.com/gasyoun/SanskritLexicography/blob/master/tools/epistemic_integrity_check.py) green. | Opus 5 1M (`claude-opus-5[1m]`), H1910 |
| 20-07-2026 | **Verifiability axis + §448–451 collision fix (H1362).** Every finding classed A/B/C/D (94/12/4/3 of **113**) in [`epistemic_dashboard/FINDINGS_VERIFIABILITY_RULING_2026.md`](https://github.com/gasyoun/SanskritLexicography/blob/master/epistemic_dashboard/FINDINGS_VERIFIABILITY_RULING_2026.md) + [`verifiability.json`](https://github.com/gasyoun/SanskritLexicography/blob/master/epistemic_dashboard/verifiability.json); schema gained the class-D citation rule; D rows marked in place. Resolved the concurrent H1350×H1361 §448–451 duplicate (CI-red integrity gate): H1350 PWG block → §452–455, marker → §456. STALENESS regenerated (113 rows, recipe column filled); 3 new RECIPES rows; dashboard verifiability block. | Opus 4.8 (`claude-opus-4-8`), H1362 |
